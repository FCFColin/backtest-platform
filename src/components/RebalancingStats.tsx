...
function RebalancingStatsHeader() {
  ...
  return (
    <tr style={{ backgroundColor: 'var(--bg-subtle)' }}>
      <th {...thStyle} className="text-[12px] font-semibold text-left py-2.5 px-3" style={{ color: 'var(--text-muted)', borderBottom: '2px solid var(--border-soft)' }}>组合</th>
      <th className="text-[12px] font-semibold text-left py-2.5 px-3" style={{ color: 'var(--text-muted)', borderBottom: '2px solid var(--border-soft)' }}>调仓频率</th>
      <th className="text-[12px] font-semibold text-right py-2.5 px-3" style={{ color: 'var(--text-muted)', borderBottom: '2px solid var(--border-soft)' }}>偏移(日)</th>
      <th className="text-[12px] font-semibold text-right py-2.5 px-3" style={{ color: 'var(--text-muted)', borderBottom: '2px solid var(--border-soft)' }}>偏离阈值</th>
      <th className="text-[12px] font-semibold text-left py-2.5 px-3" style={{ color: 'var(--text-muted)', borderBottom: '2px solid var(--border-soft)' }}>再平衡带</th>
    </tr>
  );
}
...
function RebalancingStatsRow({ portfolio, idx }: { portfolio: Pick<Portfolio, 'name' | 'rebalanceFrequency' | 'rebalanceThreshold' | 'rebalanceOffset' | 'rebalanceBands'>; idx: number }) {
  ...
  const bandsText = bands?.enabled
    ...
    : '未启用';
  const tdBase = { className: 'text-[13px] py-2 px-3', style: { borderBottom: '1px solid var(--border-soft)' } };
  return (
    <tr key={portfolio.name} style={{ backgroundColor: isAlt ? 'var(--bg-subtle)' : 'transparent' }}>
      ...
      <td className="text-[13px] text-right py-2 px-3 font-mono" style={{ color: 'var(--text-body)', borderBottom: '1px solid var(--border-soft)' }}>
        {portfolio.rebalanceFrequency === 'threshold' ? `${portfolio.rebalanceThreshold ?? 5}%` : '-'}
      </td>
      <td {...tdBase} style={{ ...tdBase.style, color: bands?.enabled ? 'var(--text-body)' : 'var(--text-muted)' }}>
        {bandsText}
      </td>
    </tr>
  );
