  ...
  if (data.length === 0) {
    return (
      <div className="chart-card">
        <div className="text-[13px]" style={{ color: 'var(--text-muted)' }}>No monthly return data available</div>
      </div>
    );
  }

  // 转为 year + 12 月列，用于 CSV 导出
  ...
  return (
    <div className="chart-card">
      <div className="flex items-center justify-between mb-3">
        <div className="chart-card-title mb-0">月度收益 — {portfolio.name}</div>
        <ChartExporter data={exportData} filename={`monthly-return-${portfolio.name}`} />
      </div>
        ...
        <table className="border-collapse">
          <thead>
            <tr>
              ...
              {MONTH_LABELS.map((m) => (
                <th key={m} className="px-1 py-1 text-[11px] font-medium text-center min-w-[36px]" style={{ color: 'var(--text-muted)' }}>
                  {m}
                </th>
              ))}
            </tr>
