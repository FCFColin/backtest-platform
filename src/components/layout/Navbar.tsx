  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      >
        ...
        <ChevronDown
          className="w-3 h-3"
          style={{ transition: 'transform .15s', transform: isOpen ? 'rotate(180deg)' : 'rotate(0)' }}
        />
      </button>
      {isOpen && (
        <NavDropdownItems group={group} isActive={isActive} onToggle={onToggle} t={t} />
      )}
    </div>
  );
}
...
function NavToolbar({
  ...
  return (
    <div className="ml-auto flex items-center gap-1">
      ...
      <NavIconButton onClick={() => i18n.changeLanguage(i18n.language === 'zh-CN' ? 'en' : 'zh-CN')} title={t('lang.switchLang')}>
        <Globe className="w-4 h-4" />
        <span style={{ ...labelStyle, minWidth: 28 }}>{i18n.language === 'zh-CN' ? 'EN' : '中文'}</span>
      </NavIconButton>
      <NavIconButton
        onClick={toggleTheme}
