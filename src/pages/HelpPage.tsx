...
function MethodologySection() {
  return (
    <div>
      ...
      >
        <MetricCard name="CAGR" fullName="年化复合增长率" formula="CAGR = (V_end / V_start)^(1/years) - 1" desc="组合在整个回测期间的年化几何平均收益率，是最核心的长期收益指标。" />
        <MetricCard name="Sharpe Ratio" fullName="夏普比率" formula="Sharpe = (R_p - R_f) / σ_p" desc="单位总风险下的超额收益。R_f 为无风险利率（默认 0），σ_p 为收益率序列的标准差（年化）。" />
        <MetricCard name="Sortino Ratio" fullName="索提诺比率" formula="Sortino = (R_p - R_f) / σ_downside" desc="与夏普类似，但仅以下行波动率作为风险度量，对上行波动不惩罚，更符合投资者直觉。" />
        <MetricCard name="Max Drawdown" fullName="最大回撤" formula="MDD = min((V_t - max(V_0..t)) / max(V_0..t))" desc="从历史最高点到后续最低点的最大跌幅，衡量组合最坏情况下的亏损幅度。" />
        <MetricCard name="Volatility" fullName="年化波动率" formula="σ_annual = σ_daily × √252" desc="日收益率标准差按 252 个交易日年化，衡量组合收益的波动程度。" />
        <MetricCard name="Calmar Ratio" fullName="卡玛比率" formula="Calmar = CAGR / |MDD|" desc="单位最大回撤下的年化收益，反映承担回撤风险所获得的回报。" />
        <MetricCard name="Beta" fullName="贝塔系数" formula="β = Cov(R_p, R_m) / Var(R_m)" desc="组合相对基准指数的系统性风险暴露。β=1 表示与基准同幅波动，β>1 更激进。" />
        <MetricCard name="Alpha" fullName="阿尔法" formula="α = R_p - [R_f + β(R_m - R_f)]" desc="扣除系统性风险后的超额收益，衡量主动管理能力。" />
      </div>
