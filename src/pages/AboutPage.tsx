    ...
    <div className="bt-page">
      <div className="bt-page-header">
        <h1 className="bt-page-title">
          {activeSection === 'limits' ? '使用限额' : activeSection === 'upgrade' ? '升级方案' : '关于平台'}
        </h1>
      </div>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>v1.0.0 · 本地部署 · 数据自主可控</div>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16 }}>
              <FeatureCard icon={<Shield className="w-5 h-5" />} title="本地部署" desc="所有数据存储在本地，无需上传到云端，隐私安全" />
              <FeatureCard icon={<Globe className="w-5 h-5" />} title="多市场支持" desc="美股、A股、港股、日股、欧股等全球主要市场" />
              <FeatureCard icon={<Clock className="w-5 h-5" />} title="高性能引擎" desc="Rust回测引擎 + Go数据服务，毫秒级响应" />
              <FeatureCard icon={<Database className="w-5 h-5" />} title="丰富数据" desc="支持12000+标的，含价格、财报、因子数据" />
            </div>

            <div style={{ marginTop: 24, padding: 16, background: 'var(--bg-subtle)', borderRadius: 'var(--radius-control)', fontSize: 13, color: 'var(--text-muted)' }}>
        ...
        {activeSection === 'limits' && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <LimitCard label="并发回测" value="受CPU限制" desc="Rust引擎使用rayon并行" />
            </div>

            <div style={{ marginTop: 24, padding: 16, background: 'var(--warning-soft, #fef3c7)', borderRadius: 'var(--radius-control)', fontSize: 13, color: 'var(--text-body)' }}>
...
function LimitCard({ label, value, desc }: { label: string; value: string; desc: string }) {
  return (
    <div style={{ padding: 16, background: 'var(--bg-subtle)', borderRadius: 'var(--radius-control)' }}>
      ...
      <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-strong)', marginBottom: 2 }}>{value}</div>
      <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{desc}</div>
    </div>
  );
}
...
function PlanCard({ title, price, features, current }: { title: string; price: string; features: string[]; current?: boolean }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-strong)', marginBottom: 4 }}>{title}</div>
      <div style={{ fontSize: 24, fontWeight: 700, color: 'var(--brand)', marginBottom: 16 }}>{price}</div>
      {features.map((f, i) => (
        <div key={i} style={{ fontSize: 13, color: 'var(--text-body)', padding: '4px 0', paddingLeft: 16, position: 'relative' }}>
          <span style={{ position: 'absolute', left: 0, color: 'var(--success)' }}>✓</span>
          {f}
        </div>
      ))}
      {current && (
        <div style={{ marginTop: 16, padding: '8px 16px', background: 'var(--brand)', color: 'white', borderRadius: 8, textAlign: 'center', fontSize: 13, fontWeight: 600 }}>
          当前方案
        </div>
      )}
    </div>
