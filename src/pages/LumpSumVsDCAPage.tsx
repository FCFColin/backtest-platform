...
function useLumpSumVsDCAState() {
  ...
  const runComparison = () => {
    ...
    if (validAssets.length === 0) { setError('请至少添加一个标的'); return; }
    if (Math.abs(totalWeight - 100) > 0.01) { setError('权重合计必须为 100%'); return; }
    setResults([]);
    run(() => doComparison(validAssets));
  };

  async function doComparison(validAssets: typeof assets) {
    const baseParams = { startDate, endDate, startingValue, baseCurrency, adjustForInflation, rollingWindowMonths: 12, benchmarkTicker: '', extendedWithdrawalStats: false, cashflowLegs: [], oneTimeCashflows: [] };
    const portfolioDef = { name: '组合', assets: validAssets, rebalanceFrequency: 'quarterly' as const, rebalanceOffset: 0, drag: 0, totalReturn: true };
    const lumpSumBody = { portfolios: [{ ...portfolioDef, name: '一次性投资' }], parameters: { ...baseParams, startingValue } };
    const contributionAmount = Math.round(startingValue / dcaPeriods);
    const dcaCashflowLegs = [{ id: `dca-${Date.now()}`, amount: contributionAmount, type: 'contribution' as const, frequency: dcaFrequency === 'monthly' ? ('monthly' as const) : ('quarterly' as const), offset: 0 }];
    const dcaBody = { portfolios: [{ ...portfolioDef, name: '定投' }], parameters: { ...baseParams, startingValue: 0, cashflowLegs: dcaCashflowLegs } };
  ...
  return {
    startDate, setStartDate, endDate, setEndDate, startingValue, setStartingValue,
    baseCurrency, setBaseCurrency, adjustForInflation, setAdjustForInflation,
    dcaFrequency, setDcaFrequency, dcaPeriods, setDcaPeriods, investTbill, setInvestTbill,
    assets, addAsset, removeAsset, updateAsset, totalWeight,
    isLoading, error, results, runComparison,
  };
}
    ...
    <div className="bt-seo-card card">
      ...
      <div className="bt-seo-features">
        <div className="bt-seo-feature">
          ...
          <div className="bt-seo-feature-desc">一次性投入 vs 按月/按季度定投的增长曲线、终值、CAGR、波动率、最大回撤、夏普比率等。</div>
        </div>
        <div className="bt-seo-feature">
          <div className="bt-seo-feature-title">策略说明</div>
          <div className="bt-seo-feature-desc">定投将初始资金均分为若干期，每期等额投入；未投入资金可选择投入短期国债（T-Bill）获取无风险收益。</div>
        </div>
      </div>
      <div className="bt-seo-related">
        ...
        <Link to="/rebalancing-sensitivity" className="link-blue" style={{ fontWeight: 700 }}>调仓敏感性</Link>
        <span style={{ color: 'var(--text-muted)' }}> · </span>
...
function ParamsSection1({
  ...
  startingValue: number; setStartingValue: (v: number) => void; baseCurrency: 'usd' | 'cny'; setBaseCurrency: (v: 'usd' | 'cny') => void;
  adjustForInflation: boolean; setAdjustForInflation: (v: boolean) => void;
  dcaFrequency: DcaFrequency; setDcaFrequency: (v: DcaFrequency) => void; dcaPeriods: number; setDcaPeriods: (v: number) => void;
  investTbill: boolean; setInvestTbill: (v: boolean) => void;
}) {
  return (
    <div className="params-section">
      <div className="params-title">参数设置</div>
      <div className="params-row">
        <div className="param-field"><label className="param-label">开始日期</label><input type="date" className="param-input" value={startDate} onChange={(e) => setStartDate(e.target.value)} /></div>
        <div className="param-field"><label className="param-label">结束日期</label><input type="date" className="param-input" value={endDate} onChange={(e) => setEndDate(e.target.value)} /></div>
      ...
      <div style={{ marginTop: 12 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 6 }}>定投参数</div>
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div className="param-field" style={{ width: 120 }}>
            <label className="param-label">DCA节奏</label>
            <select className="param-input" value={dcaFrequency} onChange={(e) => setDcaFrequency(e.target.value as DcaFrequency)}>
              <option value="monthly">每月</option><option value="quarterly">每季度</option>
            </select>
          </div>
          <div className="param-field" style={{ width: 100 }}>
            <label className="param-label">DCA期数</label>
            <div className="param-input-suffix-wrap">
              <input type="number" className="param-input param-input-with-suffix" value={dcaPeriods} onChange={(e) => setDcaPeriods(Number(e.target.value) || 1)} min={1} max={360} />
          ...
          <div className="param-field" style={{ width: 140 }}>
            ...
            <div className="param-input-prefix-wrap">
              <span className="param-input-prefix">{baseCurrency === 'usd' ? '$' : '¥'}</span>
              <input type="text" className="param-input param-input-with-prefix" value={Math.round(startingValue / dcaPeriods).toLocaleString()} readOnly style={{ opacity: 0.7 }} />
            </div>
          </div>
          <label className="param-check">
            <input type="checkbox" checked={investTbill} onChange={(e) => setInvestTbill(e.target.checked)} />
...
function PortfolioEditor({ assets, addAsset, removeAsset, updateAsset, totalWeight }: {
  assets: Array<{ ticker: string; weight: number }>;
  addAsset: () => void; removeAsset: (i: number) => void;
        ...
        <div className="portfolio-card">
          {assets.map((a, i) => (
            <div key={i} className="ticker-row">
              <input type="text" value={a.ticker} onChange={(e) => updateAsset(i, 'ticker', e.target.value)} placeholder="输入代码，如 VTI" className="ticker-input" />
              <div className="weight-cell">
                <input type="number" value={a.weight || ''} onChange={(e) => updateAsset(i, 'weight', Number(e.target.value))} min={0} max={100} className="weight-input" placeholder="%" />
                <span className="weight-suffix">%</span>
              </div>
              <button onClick={() => removeAsset(i)} className="row-remove-btn" title="删除"><X className="w-4 h-4" /></button>
            </div>
          ))}
          <div className="portfolio-card-toolbar">
            <button className="toolbar-btn" onClick={addAsset}><Plus className="w-4 h-4" />添加标的</button>
          </div>
...
function StatsTable({ results, fmtPct, fmtNum, fmtMoney }: {
  results: CompareResult[];
  fmtPct: (v: number) => string; fmtNum: (v: number) => string; fmtMoney: (v: number) => string;
}) {
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr style={{ backgroundColor: 'var(--bg-subtle)' }}>
            <th className="text-[12px] font-semibold text-left py-2.5 px-3" style={{ color: 'var(--text-muted)', borderBottom: '2px solid var(--border-soft)' }}>指标</th>
            {results.map((r, idx) => (
              <th key={r.label} className="text-[12px] font-semibold text-right py-2.5 px-3" style={{ color: 'var(--text-muted)', borderBottom: '2px solid var(--border-soft)' }}>
        ...
        <tbody>
          {STATS_ROWS.map((row, rowIdx) => {
            const hasAnyValue = results.some((r) => r[row.key] !== undefined && r[row.key] !== null);
            ...
            return (
              <tr key={row.key} style={{ backgroundColor: rowIdx % 2 === 1 ? 'var(--bg-subtle)' : 'transparent' }}>
                ...
                {results.map((r) => {
                  const val = r[row.key];
                  return (
                    <td key={r.label} className="text-[13px] font-medium text-right py-2 px-3 font-mono" style={{ color: 'var(--text-strong)', borderBottom: '1px solid var(--border-soft)' }}>
                      {val !== undefined && val !== null ? fmtVal(row.key, val as number) : '\u2014'}
                    </td>
                  );
                })}
              </tr>
...
function ConclusionAnalysis({ ls, dca, fmtPct, fmtMoney }: {
  ls: CompareResult; dca: CompareResult; fmtPct: (v: number) => string; fmtMoney: (v: number) => string;
}) {
  ...
  return (
    <div style={{ padding: 16, backgroundColor: 'var(--bg-subtle)', borderRadius: 'var(--radius-control)', marginBottom: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        {lsWins ? <TrendingUp className="w-5 h-5" style={{ color: 'var(--success)' }} /> : <TrendingDown className="w-5 h-5" style={{ color: 'var(--brand)' }} />}
        <span style={{ fontWeight: 600, fontSize: 14, color: 'var(--text-strong)' }}>结论分析</span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 12 }}>
        <div style={{ padding: 12, backgroundColor: 'var(--bg-elevated)', borderRadius: 'var(--radius-control)' }}>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>胜出策略</div>
          <div style={{ fontSize: 15, fontWeight: 600, fontFamily: 'monospace', color: lsWins ? CHART_COLORS[0] : CHART_COLORS[1] }}>{lsWins ? '一次性投资' : '定投'}</div>
        </div>
        <div style={{ padding: 12, backgroundColor: 'var(--bg-elevated)', borderRadius: 'var(--radius-control)' }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 600, fontFamily: 'monospace', color: 'var(--text-body)' }}>
            {fmtMoney(finalValueDiff)} <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>({finalValueDiffPct.toFixed(1)}%)</span>
          </div>
        </div>
        <div style={{ padding: 12, backgroundColor: 'var(--bg-elevated)', borderRadius: 'var(--radius-control)' }}>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>最大回撤差异</div>
          <div style={{ fontSize: 15, fontWeight: 600, fontFamily: 'monospace', color: 'var(--text-body)' }}>{fmtPct(mddDiff)}</div>
        </div>
      </div>
      <div style={{ fontSize: 13, color: 'var(--text-body)', lineHeight: 1.6 }}>
        {lsWins ? (
          <>在选定的时间范围内，<strong style={{ color: CHART_COLORS[0] }}>一次性投资</strong>的终值更高（{fmtMoney(ls.finalValue)} vs {fmtMoney(dca.finalValue)}），高出{finalValueDiffPct.toFixed(1)}%。但一次性投资的最大回撤（{fmtPct(ls.maxDrawdown)}）通常大于定投（{fmtPct(dca.maxDrawdown)}），在下跌市场中承受更大的心理压力。</>
        ) : (
          <>在选定的时间范围内，<strong style={{ color: CHART_COLORS[1] }}>定投</strong>的终值更高（{fmtMoney(dca.finalValue)} vs {fmtMoney(ls.finalValue)}），高出{finalValueDiffPct.toFixed(1)}%。定投通过分批买入降低了平均成本，在下跌市场中获得了更好的回报。</>
        )}
      </div>
    </div>
  );
...
function RiskWarning({ lsWins }: { lsWins: boolean }) {
  return (
    <div style={{ marginTop: 16, padding: '12px 16px', backgroundColor: 'var(--bg-subtle)', borderRadius: 'var(--radius-control)', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
      <AlertTriangle className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--warning)', marginTop: 2 }} />
...
export default function LumpSumVsDCAPage() {
  ...
  const fmtMoney = (v: number) => s.baseCurrency === 'usd' ? `$${v.toLocaleString(undefined, { maximumFractionDigits: 0 })}` : `¥${v.toLocaleString(undefined, { maximumFractionDigits: 0 })}`;
  ...
  return (
    <div className="bt-page">
      ...
      <div className="bt-main-card card">
        <ParamsSection1
          startDate={s.startDate} setStartDate={s.setStartDate} endDate={s.endDate} setEndDate={s.setEndDate}
          startingValue={s.startingValue} setStartingValue={s.setStartingValue} baseCurrency={s.baseCurrency} setBaseCurrency={s.setBaseCurrency}
          adjustForInflation={s.adjustForInflation} setAdjustForInflation={s.setAdjustForInflation}
          dcaFrequency={s.dcaFrequency} setDcaFrequency={s.setDcaFrequency} dcaPeriods={s.dcaPeriods} setDcaPeriods={s.setDcaPeriods}
          investTbill={s.investTbill} setInvestTbill={s.setInvestTbill}
        />
        <PortfolioEditor assets={s.assets} addAsset={s.addAsset} removeAsset={s.removeAsset} updateAsset={s.updateAsset} totalWeight={s.totalWeight} />
        <div className="bt-action-row">
          <LoadingButton isLoading={s.isLoading} onClick={s.runComparison} loadingText="对比中..." style={{ width: '100%' }}>
            <Play className="w-4 h-4" />开始对比
          </LoadingButton>
        </div>
      </div>
      {s.error && (
        <div className="bt-results-card card" style={{ color: 'var(--error)', textAlign: 'center', padding: 24 }}>对比失败：{s.error}</div>
      )}
      {s.results.length === 2 && (
        <div className="bt-results-card card">
          <ConclusionAnalysis ls={s.results[0]} dca={s.results[1]} fmtPct={fmtPct} fmtMoney={fmtMoney} />
          <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--text-strong)', marginBottom: 12 }}>增长曲线对比</div>
          <GrowthCurveChart results={s.results} />
          <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--text-strong)', marginBottom: 12, marginTop: 24 }}>统计对比</div>
