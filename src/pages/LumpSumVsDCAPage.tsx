...
export default function LumpSumVsDCAPage() {
  ...
  const runComparison = () => {
    ...
    run(async () => {
      ...
      const dcaBody = {
        ...
        parameters: {
          ...baseParams,
          startingValue: investTbill ? 0 : 0,
          cashflowLegs: dcaCashflowLegs,
        },
      };
      ...
      const [lumpSumRes, dcaRes] = await Promise.all([
        fetch('/api/backtest/portfolio', {
          ...
          body: JSON.stringify(lumpSumBody),
        }),
        fetch('/api/backtest/portfolio', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      const lumpSumData = lumpSumJson.data ?? lumpSumJson;
      const dcaData = dcaJson.data ?? dcaJson;

      const lumpSumP = lumpSumData.portfolios?.[0];
      const dcaP = dcaData.portfolios?.[0];
      ...
      const toResult = (p: any, label: string): CompareResult => {
        const stats = p.statistics as Statistics;
        return {
          label,
          cagr: stats?.cagr ?? 0,
          stdev: stats?.stdev ?? 0,
          ...
          finalValue: p.growthCurve?.length > 0 ? p.growthCurve[p.growthCurve.length - 1].value : 0,
          growthCurve: p.growthCurve ?? [],
        };
      };

      setResults([
        toResult(lumpSumP, '一次性投资'),
        toResult(dcaP, '定投'),
      ]);
    });
  };
  ...
  const fmtMoney = (v: number) =>
    baseCurrency === 'usd' ? `$${v.toLocaleString(undefined, { maximumFractionDigits: 0 })}` : `¥${v.toLocaleString(undefined, { maximumFractionDigits: 0 })}`;

  // 结论分析
  const ls = results[0]; // 一次性投资
  const dca = results[1]; // 定投
  const lsWins = ls && dca && ls.finalValue > dca.finalValue;
  const finalValueDiff = ls && dca ? Math.abs(ls.finalValue - dca.finalValue) : 0;
  const finalValueDiffPct = ls && dca && ls.finalValue > 0 ? (finalValueDiff / ls.finalValue * 100) : 0;
  const mddDiff = ls && dca ? Math.abs(ls.maxDrawdown - dca.maxDrawdown) : 0;
    ...
    <div className="bt-page">
      ...
      <div className="bt-main-card card">
        ...
        <div className="bt-action-row">
          <LoadingButton isLoading={isLoading} onClick={runComparison} loadingText="对比中..." style={{ width: '100%' }}>
        ...
        <div className="bt-results-card card">
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 12 }}>
              <div style={{ padding: 12, backgroundColor: 'var(--bg-elevated)', borderRadius: 'var(--radius-control)' }}>
                ...
                <div style={{ fontSize: 15, fontWeight: 600, fontFamily: 'monospace', color: lsWins ? CHART_COLORS[0] : CHART_COLORS[1] }}>
                  {lsWins ? '一次性投资' : '定投'}
                </div>
              </div>
              <div style={{ padding: 12, backgroundColor: 'var(--bg-elevated)', borderRadius: 'var(--radius-control)' }}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              ...
              <tbody>
                {[
                  { key: 'finalValue' as const, label: '终值', fmt: fmtMoney },
                  { key: 'cagr' as const, label: 'CAGR', fmt: fmtPct },
                  { key: 'stdev' as const, label: '波动率', fmt: fmtPct },
                  { key: 'maxDrawdown' as const, label: '最大回撤', fmt: fmtPct },
                  { key: 'sharpe' as const, label: '夏普比率', fmt: fmtNum },
                  { key: 'sortino' as const, label: 'Sortino', fmt: fmtNum },
                  { key: 'calmar' as const, label: 'Calmar', fmt: (v: number) => v !== undefined ? fmtNum(v) : '\u2014' },
                  { key: 'maxDrawdownDuration' as const, label: '最长回撤期', fmt: (v: number) => v !== undefined ? `${v} 天` : '\u2014' },
                  { key: 'ulcerIndex' as const, label: 'Ulcer Index', fmt: (v: number) => v !== undefined ? fmtNum(v) : '\u2014' },
                ].map((row, rowIdx) => {
                  const hasAnyValue = results.some((r) => r[row.key] !== undefined && r[row.key] !== null);
                  if (!hasAnyValue && row.key !== 'finalValue' && row.key !== 'cagr' && row.key !== 'stdev' && row.key !== 'maxDrawdown' && row.key !== 'sharpe' && row.key !== 'sortino') return null;
                  return (
                      ...
                      {results.map((r) => {
                        ...
                        return (
                          <td key={r.label} className="text-[13px] font-medium text-right py-2 px-3 font-mono" style={{ color: 'var(--text-strong)', borderBottom: '1px solid var(--border-soft)' }}>
                            {val !== undefined && val !== null ? row.fmt(val as number) : '\u2014'}
                          </td>
                        );
                      })}
                    </tr>
