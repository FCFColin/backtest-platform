...
const ALERT_TRIGGER_OPTIONS: Array<{ value: EmailAlertConfig['triggers'][number]; label: string; desc: string }> = [
...
function SignalEditor({ signal, index, onChange, onRemove, canRemove }: SignalEditorProps) {
  ...
  return (
    ...
    >
      ...
      {/* 条件列表 */}
      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 4, fontWeight: 600 }}>触发条件（全部满足）</div>
      {signal.conditions.map((cond, ci) => (
        <div key={ci} className="ticker-row" style={{ flexWrap: 'wrap', marginBottom: 4 }}>
          <select
            className="param-input"
            style={{ width: 130, height: 32, fontSize: 12, padding: '2px 6px' }}
            value={cond.indicator}
            onChange={(e) => updateCondition(ci, { indicator: e.target.value as TechnicalIndicator })}
          >
          ...
          <div className="param-input-suffix-wrap" style={{ width: 80 }}>
            <input
              ...
              value={cond.period}
              onChange={(e) => updateCondition(ci, { period: Number(e.target.value) })}
            />
          ...
          <select
            ...
            value={cond.operator}
            onChange={(e) => updateCondition(ci, { operator: e.target.value as SignalCondition['operator'] })}
          >
          ...
          <div className="param-input-prefix-wrap" style={{ width: 90 }}>
            <input
              ...
              onChange={(e) => updateCondition(ci, { threshold: Number(e.target.value) })}
            />
            <span className="param-input-prefix">阈值</span>
          </div>
          {signal.conditions.length > 1 && (
            <button className="row-remove-btn" onClick={() => removeCondition(ci)} title="删除条件">
      ...
      {/* 目标权重 */}
      <div style={{ fontSize: 11, color: 'var(--text-muted)', margin: '10px 0 4px', fontWeight: 600 }}>目标权重（激活时切换）</div>
      {signal.targetWeights.map((w, wi) => (
        <div key={wi} className="ticker-row" style={{ marginBottom: 4 }}>
          <input
            type="text"
            className="ticker-input"
            style={{ flex: 1, height: 32, fontSize: 12 }}
            value={w.ticker}
            onChange={(e) => updateWeight(wi, { ticker: e.target.value.toUpperCase() })}
            placeholder="标的代码"
          />
          <div className="param-input-suffix-wrap" style={{ width: 100 }}>
            <input
              ...
              style={{ height: 32, fontSize: 12, padding: '2px 30px 2px 6px' }}
              value={w.weight}
              onChange={(e) => updateWeight(wi, { weight: Number(e.target.value) })}
            />
            <span className="param-input-suffix">%</span>
          </div>
          {signal.targetWeights.length > 1 && (
            <button className="row-remove-btn" onClick={() => removeWeight(wi)} title="删除">
              <X className="w-3.5 h-3.5" />
            </button>
          )}
        </div>
...
function BacktestResultTab({ results }: { results: BacktestResponse }) {
  ...
  const statRows = useMemo(() => {
    ...
    return metrics.map((m) => ({
      metric: m.label,
      tactical: m.fmt === 'pct' ? fmtPct(portfolio.statistics[m.key]) : fmtRatio(portfolio.statistics[m.key]),
      benchmark: m.fmt === 'pct' ? fmtPct(benchmark.statistics[m.key]) : fmtRatio(benchmark.statistics[m.key]),
      _sortTactical: portfolio.statistics[m.key] ?? 0,
    }));
  }, [portfolio, benchmark]);

  const statColumns: Column<typeof statRows[number]>[] = [
    { key: 'metric', label: '指标' },
      ...
      <div className="chart-card">
        ...
        <ResponsiveContainer width="100%" height={380}>
          <LineChart data={growthData} margin={{ top: 5, right: 20, bottom: 5, left: 10 }}>
            ...
            <Line type="monotone" dataKey="战术分配" stroke={CHART_COLORS[0]} strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
            <Line type="monotone" dataKey="等权基准" stroke={CHART_COLORS[1]} strokeWidth={2} dot={false} activeDot={{ r: 4 }} strokeDasharray="6 3" />
          </LineChart>
        </ResponsiveContainer>
      </div>
            ...
            <table className="w-full border-collapse">
              <thead style={{ position: 'sticky', top: 0 }}>
                <tr style={{ backgroundColor: 'var(--bg-elevated)' }}>
                  <th className="text-[12px] font-semibold text-left py-2 px-3" style={{ color: 'var(--text-muted)', borderBottom: '2px solid var(--border-soft)' }}>日期</th>
                  <th className="text-[12px] font-semibold text-left py-2 px-3" style={{ color: 'var(--text-muted)', borderBottom: '2px solid var(--border-soft)' }}>激活信号</th>
                  <th className="text-[12px] font-semibold text-right py-2 px-3" style={{ color: 'var(--text-muted)', borderBottom: '2px solid var(--border-soft)' }}>目标权重</th>
                </tr>
              </thead>
              <tbody>
                {signalHistory.map((h, idx) => (
                  <tr key={idx} style={{ backgroundColor: idx % 2 === 1 ? 'var(--bg-subtle)' : 'transparent' }}>
                    <td className="text-[13px] py-2 px-3 font-mono" style={{ color: 'var(--text-body)', borderBottom: '1px solid var(--border-soft)' }}>{h.date}</td>
                    <td className="text-[13px] py-2 px-3" style={{ color: 'var(--text-body)', borderBottom: '1px solid var(--border-soft)' }}>
                      {h.activeSignals.length > 0 ? h.activeSignals.join(', ') : <span style={{ color: 'var(--text-muted)' }}>无（等权）</span>}
                    </td>
                    <td className="text-[13px] py-2 px-3 text-right font-mono" style={{ color: 'var(--text-strong)', borderBottom: '1px solid var(--border-soft)' }}>
...
function WhatIfTab({
  ...
  const signalColor = (t: WhatIfResult['signalType']) =>
    t === 'buy' ? 'var(--success)' : t === 'sell' ? 'var(--danger)' : 'var(--text-muted)';
  const signalLabel = (t: WhatIfResult['signalType']) =>
    t === 'buy' ? '买入' : t === 'sell' ? '卖出' : '持有';

  const columns: Column<WhatIfResult>[] = [
    { key: 'ticker', label: '标的', sortValue: (r) => r.ticker },
    { key: 'currentPrice', label: '最新价格', sortValue: (r) => r.currentPrice, render: (r) => <span className="font-mono">{fmtPrice(r.currentPrice)}</span> },
    { key: 'signalDate', label: '信号日期', sortValue: (r) => r.signalDate },
    { key: 'signalType', label: '信号状态', sortValue: (r) => r.signalType, render: (r) => <span style={{ color: signalColor(r.signalType), fontWeight: 600 }}>{signalLabel(r.signalType)}</span> },
  ];
    ...
    <div className="space-y-4">
      <div className="chart-card">
        ...
        <div className="ticker-row" style={{ marginBottom: 12 }}>
          ...
          />
          <LoadingButton isLoading={isLoading} onClick={handleQuery} loadingText="查询中..." className="main-action-btn" style={{ minHeight: 40, padding: '0 16px' }}>
    ...
    <div className="chart-card">
      ...
      <div className="param-field" style={{ marginBottom: 16, maxWidth: 360 }}>
        ...
        <div className="param-input-prefix-wrap">
          <Mail className="w-4 h-4" style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} />
          <input
            type="email"
            className="param-input"
      ...
      {/* 触发条件 */}
      <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-strong)', marginBottom: 8 }}>触发条件</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxWidth: 360 }}>
        {ALERT_TRIGGER_OPTIONS.map((opt) => (
          <label key={opt.value} className="param-toggle" style={{ cursor: 'pointer' }}>
            ...
            <span style={{ color: 'var(--text-muted)', fontSize: 11, marginLeft: 'auto' }}>{opt.desc}</span>
          </label>
        ))}
      </div>
      ...
      <div className="bt-action-row" style={{ paddingLeft: 0, maxWidth: 360 }}>
        <LoadingButton isLoading={isLoading} onClick={handleSave} loadingText="保存中..." style={{ width: '100%' }}>
          <Bell className="w-4 h-4" />
          保存告警配置
        </LoadingButton>
      </div>
      {error && <div style={{ color: 'var(--danger)', fontSize: 13, marginTop: 8 }}>{error}</div>}
      {saved && <div style={{ color: 'var(--success)', fontSize: 13, marginTop: 8 }}>告警配置已保存</div>}
    </div>
  );
}
...
export default function TacticalPage() {
  ...
  const paramsPanel = (
    <ParamsPanel>
      {/* 信号构建器 */}
      <ParamsSection title="信号构建器" info="基于技术指标构建交易信号。每个信号包含若干触发条件（全部满足时激活）及目标权重（激活时切换的配置）">
        ...
        <button className="portfolios-add-btn" onClick={addSignal} style={{ width: '100%', justifyContent: 'center', marginTop: 4 }}>
      ...
      <ParamsSection title="聚合配置" info="多信号同时激活时的权重合成方式">
        <div className="param-field" style={{ marginBottom: 8 }}>
          ...
          <select
            ...
            onChange={(e) => setStrategy({ ...strategy, aggregationMethod: e.target.value as TacticalStrategy['aggregationMethod'] })}
          >
        ...
        {strategy.aggregationMethod === 'rank' && (
          <div className="params-row">
            <div className="param-field param-field-rolling">
              ...
              >
                {RANKING_METHOD_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            </div>
            <div className="param-field param-field-rolling">
      ...
      <div className="bt-action-row">
        <LoadingButton isLoading={isLoading} onClick={handleRunBacktest} loadingText="回测中..." style={{ width: '100%' }}>
    ...
    <div className="space-y-4">
      ...
      <div className="card">
        ...
        <div className="result-content">
          {activeTab === 'backtest' && (
            results ? <BacktestResultTab results={results} /> : (
  ...
  return (
    <div className="bt-page">
      ...
      <div className="bt-seo-card card">
        ...
        <div className="bt-seo-features">
          <div className="bt-seo-feature">
            ...
            <div className="bt-seo-feature-desc">技术指标参数、信号触发条件、目标权重、聚合方式、再平衡频率。</div>
          </div>
          <div className="bt-seo-feature">
            <div className="bt-seo-feature-title">可查看结果</div>
            <div className="bt-seo-feature-desc">收益曲线、统计指标、信号切换历史、实时价格与信号状态。</div>
          </div>
        </div>
      ...
      <ToolPageLayout title="战术策略参数" params={paramsPanel} results={resultsPanel} />
    </div>
  );
}
