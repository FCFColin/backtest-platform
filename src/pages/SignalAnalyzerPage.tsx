...
import type { SignalAnalysisRequest, SignalAnalysisResult, SignalType } from '../../shared/types/signal';
  ...
  const signalColumns: Column<{ date: string; type: 'buy' | 'sell'; price: number }>[] = [
    { key: 'date', label: '日期', sortValue: (r) => r.date },
    {
      ...
      render: (r) => (
        <span
          style={{
            color: r.type === 'buy' ? '#1a7a3a' : '#c94a4a',
            fontWeight: 600,
          }}
        >
          {r.type === 'buy' ? '买入' : '卖出'}
        ...
        <>
          ...
          <div className="chart-card">
            ...
            <ResponsiveContainer width="100%" height={350}>
              <LineChart data={results.equityCurve} margin={{ top: 5, right: 20, bottom: 5, left: 10 }}>
                ...
                <Line type="monotone" dataKey="value" stroke={CHART_COLORS[0]} strokeWidth={2} dot={false} activeDot={{ r: 4 }} name="权益" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </>
