...
func RunMonteCarlo(ctx context.Context, req MonteCarloRequest) (*MonteCarloResult, error) {
	...
	if len(dailyReturns) < mcTradingDays { return nil, fmt.Errorf("历史数据不足：需要至少1年(%d天)的日收益率，实际%d天", mcTradingDays, len(dailyReturns)) }
...
func applyDefaults(req *MonteCarloRequest) {
	...
	if req.MCParams.MinBlockYears > req.MCParams.MaxBlockYears { req.MCParams.MinBlockYears, req.MCParams.MaxBlockYears = req.MCParams.MaxBlockYears, req.MCParams.MinBlockYears }
...
func computePortfolioDailyReturns(portfolio MCPortfolioInput, priceData PriceDataMap, params MCBacktestParams) ([]float64, error) {
	...
	type assetPrices struct{ prices []float64; weight float64 }
	assetList := make([]assetPrices, 0, len(portfolio.Assets))
	for _, a := range portfolio.Assets { assetList = append(assetList, assetPrices{prices: engineutil.ExtractPrices(priceData, a.Ticker, tradingDates), weight: weights[a.Ticker]}) }
	...
	for i := 1; i < len(tradingDates); i++ {
		...
		for _, ap := range assetList {
			...
			if prevPrice > 0 && currPrice > 0 { weightedReturn += ap.weight * ((currPrice - prevPrice) / prevPrice); totalWeight += ap.weight }
		}
...
func calcPathMetrics(path []float64, startingValue float64, years float64) PathMetrics {
	...
	for j := 1; j < pathLen; j++ { if path[j-1] > 0 { dailyRets[j-1] = (path[j] - path[j-1]) / path[j-1] } }
...
func computeMCStatistics(paths [][]float64, threshold float64, startingValue float64) MCStatistics {
	...
	successCount := 0
	for i, path := range paths { finalValues[i] = path[len(path)-1]; if finalValues[i] >= target { successCount++ } }
...
func mcSortino(dailyRets []float64, cagr float64) float64 {
	...
	sumSq := 0.0
	for _, r := range dailyRets { if excess := r - dailyRF; excess < 0 { sumSq += excess * excess } }
...
type MonteCarloRequest struct { Portfolio MCPortfolioInput `json:"portfolio"`; PriceData PriceDataMap `json:"priceData"`; Params MCBacktestParams `json:"params"`; MCParams MCSimParams `json:"mcParams"` }
type MCPortfolioInput struct { Name string `json:"name"`; Assets []AssetInput `json:"assets"`; RebalanceFrequency string `json:"rebalanceFrequency"`; Drag float64 `json:"drag"`; TotalReturn bool `json:"totalReturn"` }
type AssetInput struct { Ticker string `json:"ticker"`; Weight float64 `json:"weight"` }
type PriceDataMap = engine.PriceDataMap
type MCBacktestParams struct { StartDate string `json:"startDate"`; EndDate string `json:"endDate"`; StartingValue float64 `json:"startingValue"`; AdjustForInflation bool `json:"adjustForInflation"`; RollingWindowMonths int `json:"rollingWindowMonths"`; BenchmarkTicker string `json:"benchmarkTicker"` }
type MCSimParams struct { NumSimulations int `json:"numSimulations"`; NumYears int `json:"numYears"`; MinBlockYears int `json:"minBlockYears"`; MaxBlockYears int `json:"maxBlockYears"`; SuccessThreshold float64 `json:"successThreshold"` }
type MonteCarloResult struct { Percentiles MCPercentiles `json:"percentiles"`; SuccessProbability []float64 `json:"successProbability"`; FinalDistribution []float64 `json:"finalDistribution"`; Statistics MCStatistics `json:"statistics"`; PerPathMetrics []PathMetrics `json:"perPathMetrics"`; RepresentativePaths MCRepresentativePaths `json:"representativePaths"`; SuccessProbabilities MCSuccessProbabilities `json:"successProbabilities"` }
type MCPercentiles struct { P5 []float64 `json:"p5"`; P10 []float64 `json:"p10"`; P25 []float64 `json:"p25"`; P50 []float64 `json:"p50"`; P75 []float64 `json:"p75"`; P90 []float64 `json:"p90"`; P95 []float64 `json:"p95"` }
type MCStatistics struct { MedianFinalValue float64 `json:"medianFinalValue"`; MeanFinalValue float64 `json:"meanFinalValue"`; SuccessRate float64 `json:"successRate"` }
type PathMetrics struct { FinalValue float64 `json:"finalValue"`; CAGR float64 `json:"cagr"`; MaxDrawdown float64 `json:"maxDrawdown"`; Volatility float64 `json:"volatility"`; Sharpe float64 `json:"sharpe"`; Sortino float64 `json:"sortino"` }
type MCRepresentativePaths struct { Best []float64 `json:"best"`; P25 []float64 `json:"p25"`; Median []float64 `json:"median"`; P75 []float64 `json:"p75"`; Worst []float64 `json:"worst"` }
type MCSuccessProbabilities struct { Survival []float64 `json:"survival"`; CapitalPreservation []float64 `json:"capitalPreservation"`; Profit []float64 `json:"profit"` }
func computePercentiles(paths [][]float64, totalDays int) MCPercentiles {
	numSims := len(paths)
	if numSims == 0 { return MCPercentiles{} }
	p5 := make([]float64, totalDays); p10 := make([]float64, totalDays); p25 := make([]float64, totalDays); p50 := make([]float64, totalDays); p75 := make([]float64, totalDays); p90 := make([]float64, totalDays); p95 := make([]float64, totalDays)
	type pctTarget struct{ pct float64; dst *[]float64 }
	...
	for day := 0; day < totalDays; day++ {
		...
		slices.Sort(values)
		for _, t := range targets { (*t.dst)[day] = values[max(0, min(int(float64(numSims-1)*t.pct), numSims-1))] }
	}
...
func computeSuccessProbability(paths [][]float64, threshold float64, startingValue float64) []float64 {
	...
	for day := 0; day < totalDays; day++ {
		success := 0
		for _, path := range paths { if path[day] >= target { success++ } }
...
func computeSuccessProbabilities(paths [][]float64, startingValue float64, numYears int) MCSuccessProbabilities {
	...
	for year := 1; year <= numYears; year++ {
		...
		survCount := 0; capPresCount := 0; profitCount := 0
...
func computeRepresentativePaths(paths [][]float64, totalDays int) MCRepresentativePaths {
	...
	type pathIndex struct{ idx int; finalValue float64 }
...
func runSimulations(ctx context.Context, historicalReturns []float64, totalDays int, numSims int, mcParams MCSimParams, startingValue float64) [][]float64 {
	...
	for w := 0; w < numCPU; w++ {
		...
		go func(start, n int) {
			...
			var seed int64
			var seedBuf [8]byte
			if _, err := rand.Read(seedBuf[:]); err == nil { seed = int64(binary.LittleEndian.Uint64(seedBuf[:])) } else { seed = time.Now().UnixNano() + int64(start) }
