...
if err != nil { t.Fatalf("RunMonteCarlo 返回错误: %v", err) }
if len(result.PerPathMetrics) != 10 { t.Errorf("期望 10 条模拟路径指标，实际 %d", len(result.PerPathMetrics)) }
	})
...
if err != nil { t.Fatalf("computePortfolioDailyReturns 返回错误: %v", err) }
if len(returns) == 0 { t.Error("收益率序列不应为空") }
	})
}
...
func BenchmarkRunMonteCarlo(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := RunMonteCarlo(context.Background(), req)
if err != nil { b.Fatalf("RunMonteCarlo failed: %v", err) }
	}
}
...
if err != nil { b.Fatalf("computePortfolioDailyReturns failed: %v", err) }
	}
}
