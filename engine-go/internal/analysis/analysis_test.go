...
func makePriceData(ticker string, dates []string, prices []float64) map[string]map[string]float64 {
	...
	for i, d := range dates { pd[ticker][d] = prices[i] }
	return pd
}
...
if len(result.Assets) != 0 { t.Errorf("expected 0 assets, got %d", len(result.Assets)) }
}
...
req := AnalysisRequest{ Tickers: []string{"SPY"}, PriceData: makePriceData("SPY", dates, prices), Params: AnalysisParams{ StartingValue: 10000, RollingWindowMonths: 12 } }
	result, err := RunAnalysis(context.Background(), req)
if err != nil { t.Fatalf("unexpected error: %v", err) }
if len(result.Assets) != 1 { t.Fatalf("expected 1 asset, got %d", len(result.Assets)) }
	asset := result.Assets[0]
if asset.Ticker != "SPY" { t.Errorf("expected ticker SPY, got %s", asset.Ticker) }
if len(asset.GrowthCurve) != 3 { t.Fatalf("expected 3 growth points, got %d", len(asset.GrowthCurve)) }
	expected := []float64{10000, 11000, 12000}
	for i, gp := range asset.GrowthCurve {
if math.Abs(gp.Value-expected[i]) > 0.01 { t.Errorf("growth[%d] = %v, want %v", i, gp.Value, expected[i]) }
	}
}
...
req := AnalysisRequest{ Tickers: []string{"X"}, PriceData: makePriceData("X", dates, prices), Params: AnalysisParams{StartingValue: 0} }
	result, err := RunAnalysis(context.Background(), req)
if err != nil { t.Fatalf("unexpected error: %v", err) }
if result.Assets[0].GrowthCurve[0].Value != 10000 { t.Errorf("expected default starting value 10000, got %v", result.Assets[0].GrowthCurve[0].Value) }
}
...
if err != nil { t.Fatalf("unexpected error: %v", err) }
if len(result.Assets) != 1 { t.Fatalf("expected 1 asset, got %d", len(result.Assets)) }
if result.Assets[0].Statistics.CAGR != 0 { t.Errorf("expected zero CAGR for insufficient data, got %v", result.Assets[0].Statistics.CAGR) }
}
...
if err == nil { t.Error("expected context cancellation error, got nil") }
}
func TestRunAnalysisCorrelationMatrix(t *testing.T) {
	...
	result, err := RunAnalysis(context.Background(), req)
if err != nil { t.Fatalf("unexpected error: %v", err) }
if math.Abs(result.Correlations[0][0]-1.0) > 1e-10 { t.Errorf("correlation[0][0] = %v, want 1.0", result.Correlations[0][0]) }
if math.Abs(result.Correlations[1][1]-1.0) > 1e-10 { t.Errorf("correlation[1][1] = %v, want 1.0", result.Correlations[1][1]) }
...
if math.Abs(stats.TotalReturn-expectedTotalReturn) > 1e-6 { t.Errorf("TotalReturn = %v, want %v", stats.TotalReturn, expectedTotalReturn) }
}
...
if err != nil { t.Fatalf("unexpected error: %v", err) }
	stats := result.Assets[0].Statistics
if stats.Beta != 0 { t.Errorf("Beta = %v, want 0 (no benchmark)", stats.Beta) }
if stats.Alpha != 0 { t.Errorf("Alpha = %v, want 0 (no benchmark)", stats.Alpha) }
}
