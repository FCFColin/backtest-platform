...
func TestAnalyzeSlippage_InsufficientData(t *testing.T) {
	...
	t.Run("仅1个交易日返回错误", func(t *testing.T) {
		req := LETFRequest{
			LETFSeries:  []PricePoint{{Date: "2024-01-01", Price: 100}},
			BenchSeries: []PricePoint{{Date: "2024-01-01", Price: 100}},
			Leverage:    2,
		}
...
func TestAnalyzeSlippage_WithSlippage(t *testing.T) {
	// LETF 跑输期望: 基准+10%, 期望+20%, 实际+15%, 滑点 = 5%
	req := LETFRequest{
		LETFSeries: []PricePoint{
			{Date: "2024-01-01", Price: 100},
			{Date: "2024-01-02", Price: 115}, // +15%
		},
		BenchSeries: []PricePoint{
			{Date: "2024-01-01", Price: 100},
			{Date: "2024-01-02", Price: 110}, // +10%
	...
	t.Run("annualDecay为零_天数不足", func(t *testing.T) {
		// 仅 2 天, years = 1/252, 但 cumExpected=1.2, cumLetf=1.15
		// 实际 annualDecay = expectedAnnual - letfAnnual
		// 这里只需验证它不是 NaN
...
func TestAnalyzeSlippage_EffectiveLeverage(t *testing.T) {
	// 数据量 < effectiveLeverageWindow(20), effectiveLeverage 应全为 nil
	req := LETFRequest{
		LETFSeries: []PricePoint{
			{Date: "2024-01-01", Price: 100},
	...
	// 3 天数据, 2 个收益率点, effectiveLeverage 应有 2 个元素且全为 nil
