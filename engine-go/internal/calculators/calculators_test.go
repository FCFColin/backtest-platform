...
func TestCalcCAGR(t *testing.T) {
	tests := []struct {
		...
		{
			name: "InitialAmount<=0返回零值",
			req:  CAGRRequest{InitialAmount: 0, FinalAmount: 100, Years: 10},
			check: func(t *testing.T, r CAGRResult) {
				if r != (CAGRResult{}) {
		...
		{
			name: "Years<=0返回零值",
			req:  CAGRRequest{InitialAmount: 100, FinalAmount: 200, Years: 0},
			check: func(t *testing.T, r CAGRResult) {
				if r != (CAGRResult{}) {
					t.Errorf("应返回零值, got %+v", r)
				}
...
func TestCalcTwoFundFrontier(t *testing.T) {
	t.Run("NumPoints<=0默认20", func(t *testing.T) {
		req := TwoFundFrontierRequest{
			Asset1Return: 0.05, Asset1Stdev: 0.10,
			Asset2Return: 0.10, Asset2Stdev: 0.20,
			Correlation: 0.3,
			NumPoints:   0,
		}
