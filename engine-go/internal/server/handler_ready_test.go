...
if resp["engine"] != "go" { t.Errorf("ready engine = %v, want go", resp["engine"]) }
}
func TestHandleReadyNoAuthRequired(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
if w.Code != http.StatusOK { t.Errorf("expected 200 without auth, got %d, body=%s", w.Code, w.Body.String()) }
}
