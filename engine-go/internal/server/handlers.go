...
func newProblem(c *gin.Context, status int, code, title, detail string) { sharedhttp.NewProblem(c, status, code, title, detail) }
func withComputeSpan(ctx context.Context, name string, attrs ...attribute.KeyValue) (context.Context, trace.Span) { return otel.Tracer("engine-go").Start(ctx, name, trace.WithAttributes(attrs...)) }
...
func bindJSON[T any](c *gin.Context, code, msg string, req *T) bool {
	if err := c.ShouldBindJSON(req); err != nil { newProblem(c, http.StatusBadRequest, code, "Bad Request", msg); return false }
	return true
}
var maxGoroutinesForHealth = 10000
func init() {
	if v := os.Getenv("ENGINE_MAX_GOROUTINES"); v != "" { if n, err := strconv.Atoi(v); err == nil && n > 0 { maxGoroutinesForHealth = n } }
}
...
func handleBacktest(c *gin.Context) {
	...
	if !bindJSON(c, "BACKTEST_BAD_REQUEST", "请求解析失败，请检查请求格式", &req) { return }
	if len(req.Portfolios) == 0 { newProblem(c, http.StatusBadRequest, "BACKTEST_EMPTY_PORTFOLIOS", "Bad Request", "portfolios 不能为空"); return }
	if req.PriceData == nil { newProblem(c, http.StatusBadRequest, "BACKTEST_EMPTY_PRICE_DATA", "Bad Request", "priceData 不能为空"); return }
...
func handleStatistics(c *gin.Context) {
	...
	if !bindJSON(c, "STATISTICS_BAD_REQUEST", "请求解析失败，请检查请求格式", &req) { return }
	if len(req.Values) < 2 { newProblem(c, http.StatusBadRequest, "STATISTICS_INSUFFICIENT_DATA", "Bad Request", "values 至少需要 2 个数据点"); return }
...
func handleAnalysis(c *gin.Context) {
	...
	if !bindJSON(c, "ANALYSIS_BAD_REQUEST", "请求格式错误", &req) { return }
	if len(req.Tickers) == 0 { newProblem(c, http.StatusBadRequest, "ANALYSIS_EMPTY_TICKERS", "Bad Request", "tickers 不能为空"); return }
	if req.PriceData == nil { newProblem(c, http.StatusBadRequest, "ANALYSIS_EMPTY_PRICE_DATA", "Bad Request", "priceData 不能为空"); return }
	for _, ticker := range req.Tickers { if _, ok := req.PriceData[ticker]; !ok { newProblem(c, http.StatusBadRequest, "ANALYSIS_TICKER_NOT_FOUND", "Bad Request", "ticker 在 priceData 中不存在"); return } }
...
func handlePCA(c *gin.Context) {
	...
	if !bindJSON(c, "PCA_BAD_REQUEST", "请求解析失败", &req) { return }
	if len(req.Tickers) < 2 { newProblem(c, http.StatusBadRequest, "PCA_INSUFFICIENT_TICKERS", "Bad Request", "至少需要 2 个 ticker"); return }
...
func handleLETFAnalyze(c *gin.Context) {
	...
	if !bindJSON(c, "LETF_BAD_REQUEST", "请求解析失败", &req) { return }
	letfData, ok1 := req.PriceData[req.LETFTicker]
	benchData, ok2 := req.PriceData[req.BenchmarkTicker]
	if !ok1 || !ok2 { newProblem(c, http.StatusBadRequest, "LETF_TICKER_NOT_FOUND", "Bad Request", "ticker 在 priceData 中不存在"); return }
	withComputeHandler(c, "LETF 滑点分析失败", func(ctx context.Context) (*letf.LETFResult, error) { return letf.AnalyzeSlippage(letf.LETFRequest{LETFSeries: letf.ToPricePoints(letfData), BenchSeries: letf.ToPricePoints(benchData), Leverage: req.Leverage}) })
}
func handleFactorRegression(c *gin.Context) {
	var req factorregression.FactorRegressionRequest
	if !bindJSON(c, "FR_BAD_REQUEST", "请求解析失败", &req) { return }
...
func handleOptimize(c *gin.Context) {
	...
	if !bindJSON(c, "OPTIMIZE_BAD_REQUEST", "请求解析失败，请检查请求格式", &req) { return }
	withComputeHandler(c, "优化计算失败", func(ctx context.Context) (*optimizer.OptimizeResponse, error) { ctx, span := withComputeSpan(ctx, "optimizer.optimize"); defer span.End(); return optimizer.Optimize(ctx, req) })
}
func handleEfficientFrontier(c *gin.Context) {
	var req optimizer.FrontierRequest
	if !bindJSON(c, "FRONTIER_BAD_REQUEST", "请求解析失败，请检查请求格式", &req) { return }
...
func handleMonteCarlo(c *gin.Context) {
	...
	if !bindJSON(c, "MONTE_CARLO_BAD_REQUEST", "请求解析失败，请检查请求格式", &req) { return }
	withComputeHandler(c, "蒙特卡洛模拟失败", func(ctx context.Context) (*montecarlo.MonteCarloResult, error) { ctx, span := withComputeSpan(ctx, "montecarlo.simulate"); defer span.End(); return montecarlo.RunMonteCarlo(ctx, req) })
}
func handleGoalOptimize(c *gin.Context) {
	var req goaloptimizer.GoalOptimizerRequest
	if !bindJSON(c, "GOAL_BAD_REQUEST", "请求解析失败", &req) { return }
...
func handleTacticalBacktest(c *gin.Context) {
	...
	if !bindJSON(c, "TACTICAL_BAD_REQUEST", "请求解析失败", &req) { return }
...
func handleTacticalGridSearch(c *gin.Context) {
	...
	if !bindJSON(c, "GRID_BAD_REQUEST", "请求解析失败", &req) { return }
...
func handleCalculators(c *gin.Context) {
	...
	if !bindJSON(c, "CALC_BAD_REQUEST", "请求解析失败", &req) { return }
	switch req.Type {
	case "cagr": if req.CAGR == nil { newProblem(c, http.StatusBadRequest, "CALC_MISSING_CAGR", "Bad Request", "cagr 类型需要 cagr 参数"); return }; c.JSON(http.StatusOK, gin.H{"success": true, "data": calculators.CalcCAGR(*req.CAGR)})
	case "swr": if req.SWR == nil { newProblem(c, http.StatusBadRequest, "CALC_MISSING_SWR", "Bad Request", "swr 类型需要 swr 参数"); return }; c.JSON(http.StatusOK, gin.H{"success": true, "data": calculators.CalcSWR(*req.SWR)})
	case "frontier": if req.Frontier == nil { newProblem(c, http.StatusBadRequest, "CALC_MISSING_FRONTIER", "Bad Request", "frontier 类型需要 frontier 参数"); return }; c.JSON(http.StatusOK, gin.H{"success": true, "data": calculators.CalcTwoFundFrontier(*req.Frontier)})
...
func handleSignalAnalyze(c *gin.Context) {
	...
	if !bindJSON(c, "SIGNAL_BAD_REQUEST", "请求解析失败，请检查请求格式", &req) { return }
	ctx, cancel := context.WithTimeout(c.Request.Context(), computeTimeout)
	defer cancel()
	missingTicker := func() { newProblem(c, http.StatusBadRequest, "SIGNAL_TICKER_NOT_FOUND", "Bad Request", "ticker 在 priceData 中不存在") }
	switch req.Mode {
	case "single":
		if req.Single == nil { newProblem(c, http.StatusBadRequest, "SIGNAL_MISSING_SINGLE", "Bad Request", "single 模式需要 single 参数"); return }
		tickerData, ok := req.PriceData[req.Single.Ticker]
		if !ok { missingTicker(); return }
		c.JSON(http.StatusOK, gin.H{"success": true, "data": signal.AnalyzeSignal(*req.Single, signal.ToPricePoints(tickerData))})
	case "dual":
		if req.Dual == nil { newProblem(c, http.StatusBadRequest, "SIGNAL_MISSING_DUAL", "Bad Request", "dual 模式需要 dual 参数"); return }
		td1, ok1 := req.PriceData[req.Dual.Signal1.Ticker]
		td2, ok2 := req.PriceData[req.Dual.Signal2.Ticker]
		if !ok1 || !ok2 { missingTicker(); return }
	...
	case "multi":
		if req.Multi == nil || len(req.Multi.Signals) == 0 { newProblem(c, http.StatusBadRequest, "SIGNAL_MISSING_MULTI", "Bad Request", "multi 模式需要 multi.signals 参数"); return }
