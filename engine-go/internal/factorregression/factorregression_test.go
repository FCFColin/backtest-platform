...
// makePerfectReturns 生成与 mktRF 完美线性关系的月度收益: ret = alpha + beta*(mktRf/100)
func makePerfectReturns(alpha, beta float64, ff []FFDataPoint) []MonthlyReturn {
	rets := make([]MonthlyReturn, len(ff))
	for i, f := range ff {
		rets[i] = MonthlyReturn{
			Date:  f.Date,
			Value: alpha + beta*(f.MktRf/100),
		}
	}
	return rets
}
...
func TestRunRegression_InsufficientData(t *testing.T) {
	...
	t.Run("仅2个对齐点返回零值", func(t *testing.T) {
		ff := []FFDataPoint{
			{Date: "2024-01", MktRf: 1.0, Smb: 0.5, Hml: 0.1},
			{Date: "2024-02", MktRf: 2.0, Smb: 0.8, Hml: 0.2},
		}
...
func TestRunRegression_DateFilter(t *testing.T) {
	...
	// 过滤后 2024-02 到 2024-04 共 3 个月，刚好 >= 3
...
func TestRunRegression_UnselectedFactor(t *testing.T) {
	...
	rets := makePerfectReturns(alpha, beta, ff)
	// 不选 mktRF，选 smb，则 Beta(mktRF) 应为 0
	req := FactorRegressionRequest{
		MonthlyReturns: rets,
		FFData:         ff,
		Factors:        []string{"smb"},
	...
	// mktRF 未被选为活跃因子, Beta 应为 0
	if r.Beta != 0 {
		t.Errorf("未选 mktRF 时 Beta 应为 0, got %v", r.Beta)
	}
	// SMB 系数应非零（因为 returns 与 smb 有线性关系通过 mktRF 间接，但不一定）
	if r.SMB == 0 {
		// smb 系数可能为 0 取决于数据，这里只验证不报错
	}
}
