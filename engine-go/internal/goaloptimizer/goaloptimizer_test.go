...
// makePriceData 生成单个 ticker 的递增价格数据用于测试。
...
func TestOptimizeGoals_Deterministic(t *testing.T) {
	...
	req := GoalOptimizerRequest{
		...
		InitialAmount:  10000,
		Years:          1,
		Assets:         []Asset{{Ticker: "A", Weight: 1}},
		PriceData:      pd,
		StartDate:      "2024-01-01",
		EndDate:        "2024-12-31",
...
func TestOptimizeGoals_StrictConstraintFiltersAll(t *testing.T) {
	pd := makePriceData("A", "2024-01-01", 260, 100, 0.001)
	numSims := 50
	// 负数约束: maxDrawdown >= 0 > -1.0 总成立, 所有路径都被跳过
	maxDD := -1.0
	req := GoalOptimizerRequest{
		TargetAmount:   20000,
		InitialAmount:  10000,
		Years:          1,
		Assets:         []Asset{{Ticker: "A", Weight: 1}},
		Constraints:    &Constraints{MaxDrawdown: &maxDD},
		PriceData:      pd,
		StartDate:      "2024-01-01",
		EndDate:        "2024-12-31",
		NumSimulations: &numSims,
...
func TestOptimizeGoals_NumSimulationsClamped(t *testing.T) {
	...
	t.Run("NumSimulations为负数被置为1", func(t *testing.T) {
		...
		// 1 次模拟, ProbabilityCurve 只有一个 bin 概率为 1
		if len(r.ProbabilityCurve) == 0 {
