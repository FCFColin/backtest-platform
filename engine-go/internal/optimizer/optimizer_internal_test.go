...
errorCases := []struct { name string; matrix [][]float64; errMsg string }{ {"奇异矩阵应报错", [][]float64{{1, 2}, {2, 4}}, "奇异矩阵应返回错误"}, {"空矩阵应报错", [][]float64{}, "空矩阵应返回错误"} }
for _, tc := range errorCases { t.Run(tc.name, func(t *testing.T) { _, err := invertDense(tc.matrix); if err == nil { t.Fatal(tc.errMsg) } }) }
	t.Run("单位矩阵的逆应为单位矩阵", func(t *testing.T) {
		identity := [][]float64{{1, 0}, {0, 1}}
		inv, err := invertDense(identity)
if err != nil { t.Fatalf("invertDense 返回错误: %v", err) }
if math.Abs(inv[0][0]-1.0) > 1e-10 || math.Abs(inv[1][1]-1.0) > 1e-10 { t.Errorf("单位矩阵的逆对角线应为 1，实际 %.6f, %.6f", inv[0][0], inv[1][1]) }
if math.Abs(inv[0][1]) > 1e-10 || math.Abs(inv[1][0]) > 1e-10 { t.Errorf("单位矩阵的逆非对角线应为 0，实际 %.6f, %.6f", inv[0][1], inv[1][0]) }
	})
...
if err != nil { t.Fatalf("invertDense 返回错误: %v", err) }
		for i := 0; i < 3; i++ {
			for j := 0; j < 3; j++ {
				dot := 0.0
				for k := 0; k < 3; k++ { dot += a[i][k] * inv[k][j] }
				expected := 0.0
if i == j { expected = 1.0 }
if math.Abs(dot-expected) > 1e-8 { t.Errorf("A*A^(-1)[%d][%d] = %.6f，期望 %.6f", i, j, dot, expected) }
			}
		}
	})
}
...
t.Run(tt.name, func(t *testing.T) { result := isPD(tt.matrix); if result != tt.expected { t.Errorf("期望 %v，实际 %v", tt.expected, result) } })
	}
}
func TestEnsurePD(t *testing.T) {
	t.Run("正定矩阵不应被修改", func(t *testing.T) {
		pd := [][]float64{{2, 0}, {0, 3}}
		result := ensurePD(pd)
if math.Abs(result[0][0]-2.0) > 1e-10 || math.Abs(result[1][1]-3.0) > 1e-10 { t.Error("正定矩阵不应被正则化修改") }
	})
t.Run("非正定矩阵应被正则化", func(t *testing.T) {
		nonPD := [][]float64{{0.001, 0}, {0, 0.001}}
		result := ensurePD(nonPD)
		if !isPD(result) { t.Error("正则化后矩阵应为正定") }
	})
}
...
if math.Abs(ret-0.075) > 1e-10 { t.Errorf("期望收益应为 0.075，实际 %.6f", ret) }
if vol <= 0 { t.Errorf("波动率应大于 0，实际 %.6f", vol) }
if sharpe == 0 && vol > 0 { t.Error("有波动率时 Sharpe 不应为 0") }
	})
}
...
t.Run(tt.name, func(t *testing.T) {
		...
		if result != tt.expected { t.Errorf("期望 %v，实际 %v", tt.expected, result) }
	})
	}
}
...
if w[1] < w[0] || w[1] < w[2] { t.Errorf("最高收益资产应获得最大权重：w=[%.4f, %.4f, %.4f]", w[0], w[1], w[2]) }
		sumW := 0.0
		for _, v := range w { sumW += v }
if math.Abs(sumW-1.0) > 0.01 { t.Errorf("权重和应接近 1，实际 %.4f", sumW) }
	...
	t.Run("约束 MaxWeight 应被遵守", func(t *testing.T) {
		...
		w := optimizeMaxReturn(mu, c)
		for i, v := range w {
if v > c.MaxWeight+1e-6 { t.Errorf("权重 %d = %.4f 超过 MaxWeight %.4f", i, v, c.MaxWeight) }
		}
	})
}
...
func TestClipWeights(t *testing.T) {
	t.Run("裁剪后权重和应接近 1", func(t *testing.T) {
		...
		sumW := 0.0
		for _, v := range result { sumW += v }
if math.Abs(sumW-1.0) > 0.01 { t.Errorf("裁剪后权重和应接近 1，实际 %.4f", sumW) }
	})
}
...
if v < c.MinWeight-1e-6 || v > c.MaxWeight+1e-6 { t.Errorf("权重 %d = %.4f 不在约束 [%.2f, %.2f] 内", i, v, c.MinWeight, c.MaxWeight) }
		}
	})
}
