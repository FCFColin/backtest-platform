...
func TestOptimize(t *testing.T) {
	...
	objectives := []struct {
		...
		{name: "最大夏普比", objective: "maxSharpe", checkValid: func(t *testing.T, resp *OptimizeResponse) {
			if resp.SharpeRatio == 0 {
				t.Error("最大夏普比优化结果 SharpeRatio 不应为 0")
			}
		}},
		{name: "最小波动率", objective: "minVolatility", checkValid: func(t *testing.T, resp *OptimizeResponse) {
			if resp.ExpectedVolatility <= 0 {
				t.Error("最小波动率优化结果波动率应大于 0")
			}
		}},
		{name: "最大收益", objective: "maxReturn", checkValid: func(t *testing.T, resp *OptimizeResponse) {
