...
// makePricePoints 生成日期连续的 PricePoint 切片。
...
// trendPrices 生成上升-下降-上升的价格序列, 用于触发 SMA 交叉信号。
...
func TestAnalyzeSignal_PeriodTooSmall(t *testing.T) {
	// period < 2 时内部会用 safePeriod=2, 不应 panic
	data := makePricePoints("2024-01-01", trendPrices())
