...
func RunBacktest(ctx context.Context, req BacktestRequest) (*BacktestResult, error) {
	...
	for _, pf := range req.Portfolios {
		...
		portfolioResults = append(portfolioResults, PortfolioResult{
			Name: pf.Name, GrowthCurve: curve, DrawdownCurve: ddCurve,
			RollingReturns: rollingReturns, AnnualReturns: annualRets, MonthlyReturns: monthlyRets,
			Statistics: stats, DrawdownEpisodes: episodes, AllocationHistory: allocHist})
...
if startValue <= 0 { startValue = 10000 }
	...
	for i, p := range prices {
		...
		curve[i] = DataPoint{ Date:  tradingDates[i].Format("2006-01-02"), Value: value, }
	}
	return curve
}
...
if values[i-1] > 0 { rets = append(rets, (values[i]-values[i-1])/values[i-1]) }
	}
	return rets
}
...
if startValue <= 0 { startValue = 10000 }
	...
	for i := range shares {
if initPrices[i] > 0 { shares[i] = holdings[i] / initPrices[i] }
	}
...
if glidepathYears == 0 { glidepathYears = 10 }
	...
	for _, cf := range params.OneTimeCashflows {
		amt := cf.Amount
if cf.Type == "withdrawal" { amt = -amt }
if amt != 0 { otcMap[cf.Date] += amt }
	}
...
if eff > 0 { holdings[i] = shares[i] * eff }
		...
		if dailyDrag != 1.0 {
			for i := range holdings { holdings[i] *= dailyDrag }
			pv = mathutil.Sum(holdings)
		}
		...
		if di%20 == 0 || (di == lastRebalanceDi && di > 0) {
			...
			if pv > 0 {
				for i := range holdings { snapshot[i] = holdings[i] / pv }
			}
...
func zeroHoldings(holdings []float64) {
	for i := range holdings { holdings[i] = 0 }
}
...
} else { matrix[i][j] = CalcCorrelation(dailyReturnsList[i], dailyReturnsList[j])
			}
		}
	}
	return matrix
...
func extractValues(curve []DataPoint) []float64 {
	...
	for i, dp := range curve { values[i] = dp.Value }
	return values
}
func extractDates(curve []DataPoint) []string {
	dates := make([]string, len(curve))
	for i, dp := range curve { dates[i] = dp.Date }
	return dates
}
...
if dateCPI > 0 { curve[i].Value = vals[i] * (startCPI / dateCPI) }
	}
}
...
func ComputeFingerprint(result *PortfolioResult) (string, error) {
	...
	if err := encoder.Encode(map[string]any{"growth_sampled": sampled}); err != nil { return "", err }
...
func detectDrawdownEpisodes(curve []DataPoint) []DrawdownEpisode {
	...
	for i := 1; i < len(curve); i++ {
		...
		if currentValue >= peakValue {
			...
			troughIdx = i
} else { if currentValue < troughValue { troughValue = currentValue; troughDate = currentDate; troughIdx = i }
...
if peakValue > 0 { returnFromPeakToTrough = (troughValue - peakValue) / peakValue }
	ep := DrawdownEpisode{
		PeakDate: peakDate, TroughDate: troughDate,
		RecoveryDate: recoveryDate, Depth: (peakValue - troughValue) / peakValue,
		TimeToTrough: timeToTrough, RecoveryTime: recoveryTime,
		TotalTimeDurationDays: totalDays, RecoveryFactor: recoveryFactor,
		CagrDuring: cagrDuring, UlcerDuring: ulcerDuring,
