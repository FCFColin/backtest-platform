...
func CalcAnnualizedStdev(dailyReturns []float64) float64 {
	if len(dailyReturns) < 2 { return 0 }
...
if excess := r - mar; excess < 0 { sumSquared += excess * excess }
	}
...
func CalcBeta(portfolioReturns, benchmarkReturns []float64) float64 {
	...
	if pr == nil { return 0 }
	if varB := mathutil.Covariance(br, br); varB != 0 { return mathutil.Covariance(pr, br) / varB }
	return 0
}
...
func CalcTrackingError(portfolioReturns, benchmarkReturns []float64) float64 {
	...
	for i := range pr { diffs[i] = pr[i] - br[i] }
...
func CalcExcessKurtosis(returns []float64) float64 {
	...
	if !ok { return 0 }
	return (float64(n*(n+1))/float64((n-1)*(n-2)*(n-3)))*sum -
...
func CalcMaxDrawdown(values []float64) MaxDrawdownResult {
	if len(values) < 2 { return MaxDrawdownResult{} }
	maxDD := 0.0
	maxDDDuration := 0
engineutil.IterDrawdowns(values, func(i, peakIdx int, peak float64) { if dd := (peak - values[i]) / peak; dd > maxDD { maxDD = dd; maxDDDuration = i - peakIdx } })
...
func CalcAvgDrawdown(values []float64) float64 {
	...
	var totalDD float64
	count := 0
engineutil.IterDrawdowns(values, func(i, peakIdx int, peak float64) { if peak > 0 { if dd := (peak - values[i]) / peak; dd > 0 { totalDD += dd; count++ } } })
	if count == 0 { return 0 }
...
engineutil.IterDrawdowns(values, func(i, peakIdx int, peak float64) { if peak > 0 { dd := (peak - values[i]) / peak; sumSquaredDD += dd * dd } })
...
engineutil.IterDrawdowns(values, func(i, peakIdx int, peak float64) { dd := 0.0; if peak > 0 { dd = (peak - values[i]) / peak }; result[i] = DrawdownPoint{Date: dates[i], Drawdown: dd} })
	return result
}
func CalcPWR(annualReturns []float64) float64 {
	if len(annualReturns) == 0 { return 0 }
	low, high := 0.0, 1.0
	for i := 0; i < 100; i++ {
		mid := (low + high) / 2
...
func CalcMonthlyReturns(values []float64, dates []string) []MonthlyReturn {
	...
	for i, v := range values {
		...
		if mv, ok := monthMap[key]; ok {
			mv.last = v
			monthMap[key] = mv
} else { monthMap[key] = monthVal{v, v}
		}
	}
...
func parseYear(dateStr string) int {
	if len(dateStr) < 4 || len(dateStr) > 10 { return 0 }
...
func parseYearMonth(dateStr string) (int, int) {
	for _, layout := range []string{"2006-01-02", "2006-01"} {
		if t, err := time.Parse(layout, dateStr); err == nil { return t.Year(), int(t.Month()) - 1 }
	}
	return parseYear(dateStr), 0
}
func CalcSWR(annualReturns []float64, years int, successRate float64) float64 {
	if len(annualReturns) < years || years <= 0 { return 0 }
	low, high := 0.0, 1.0
	for i := 0; i < 100; i++ {
		mid := (low + high) / 2
...
if simulateWithdrawal(annualReturns[start:start+years], withdrawalRate) { successes++ }
	}
...
func calcVaRLevels(returns []float64) VaRLevels {
	return VaRLevels{ One:  CalcVaR(returns, 0.99), Five: CalcVaR(returns, 0.95), Ten:  CalcVaR(returns, 0.90), }
}
func calcCVaRLevels(returns []float64) VaRLevels {
	return VaRLevels{ One:  CalcCVaR(returns, 0.99), Five: CalcCVaR(returns, 0.95), Ten:  CalcCVaR(returns, 0.90), }
}
...
if finalValue > 0 { cagr = CalcCAGR(req.StartingValue, finalValue, years) }
	...
	mwrrCfs := append(append([]Cashflow{}, req.MwrrCashflows...), Cashflow{Value: finalValue, Time: years})
	mwrr := -1.0
if finalValue > 0 { mwrr = CalcMWRR(mwrrCfs) }
...
varStats := VaRByFrequency{ Daily: calcVaRLevels(req.DailyReturns), Monthly: calcVaRLevels(req.MonthlyReturnValues), Annual: calcVaRLevels(req.AnnualReturnValues) }
cvarStats := VaRByFrequency{ Daily: calcCVaRLevels(req.DailyReturns), Monthly: calcCVaRLevels(req.MonthlyReturnValues), Annual: calcCVaRLevels(req.AnnualReturnValues) }
skewStats := SkewnessByFrequency{ Daily: CalcSkewness(req.DailyReturns), Monthly: CalcSkewness(req.MonthlyReturnValues), Annual: CalcSkewness(req.AnnualReturnValues) }
kurtStats := SkewnessByFrequency{ Daily: CalcExcessKurtosis(req.DailyReturns), Monthly: CalcExcessKurtosis(req.MonthlyReturnValues), Annual: CalcExcessKurtosis(req.AnnualReturnValues) }
	captureSpread := upsideDaily - downsideDaily
	return Statistics{
		CAGR: cagr, MWRR: mwrr, Stdev: stdevDaily, Sharpe: sharpe, Sortino: sortino, MaxDrawdown: dd.MaxDrawdown,
		MaxDrawdownDuration: dd.MaxDrawdownDuration, BestYear: maxAnnualRet, WorstYear: minAnnualRet,
		AvgYear:     mathutil.Mean(req.AnnualReturnValues),
		TotalReturn: totalReturn, MaxMonthlyReturn: MaxValue(req.MonthlyReturnValues),
		MinMonthlyReturn: MinValue(req.MonthlyReturnValues),
		AvgDrawdown:      CalcAvgDrawdown(req.Values), UlcerIndex: ulcerIdx,
		Calmar:                CalcCalmar(cagr, dd.MaxDrawdown),
		UlcerPerformanceIndex: CalcUPI(cagr, ulcerIdx), Beta: beta, Alpha: alpha, RSquared: rSq,
		TrackingError: trackingErr, InformationRatio: infoRatio,
		UpsideCapture: upsideDaily, DownsideCapture: downsideDaily,
		MaxDailyReturn: maxDailyRet, MinDailyReturn: minDailyRet, PWR: pwr, Var: varStats,
		Cvar: cvarStats, Skewness: skewStats, ExcessKurtosis: kurtStats,
		WinRate:         SkewnessByFrequency{Daily: pctPosDays, Monthly: pctPosMonths, Annual: pctPosYears},
		PctPositiveDays: pctPosDays, AvgAnnualReturn: mathutil.Mean(req.AnnualReturnValues),
		AvgMonthlyReturn: mathutil.Mean(req.MonthlyReturnValues),
		AvgDailyReturn:   mathutil.Mean(req.DailyReturns),
		StdevAnnual:      mathutil.Std(req.AnnualReturnValues),
		StdevMonthly:     mathutil.Std(req.MonthlyReturnValues) * math.Sqrt(12),
		StdevMonthlyRaw:  mathutil.Std(req.MonthlyReturnValues),
		...
		M2:                          m2, Treynor: treynor, DiversificationRatio: 1,
		BenchmarkCorrelation: benchmarkCorrelation, UpsideCorrelation: upsideCorr,
		DownsideCorrelation: downsideCorr, UpsideBeta: upsideBetaVal,
		DownsideBeta: downsideBetaVal, AlphaDaily: alphaDaily,
		AlphaAnnualized: alpha, UpsideCaptureDaily: upsideDaily,
		DownsideCaptureDaily: downsideDaily, CaptureSpreadDaily: captureSpread,
		UpsideCaptureAnnual: upsideDaily, DownsideCaptureAnnual: downsideDaily,
		CaptureSpreadAnnual: captureSpread, CaptureSpread: captureSpread,
		ActiveReturn: activeReturn, PctPositiveMonths: pctPosMonths,
		PctPositiveYears: pctPosYears, MaxAnnualReturn: maxAnnualRet,
		MinAnnualReturn: minAnnualRet, AvgDailyGain: avgDailyGain,
		AvgDailyLoss: avgDailyLoss, GainLossRatioDaily: gainLossRatioDaily,
		AvgMonthlyGain: avgMonthlyGain, AvgMonthlyLoss: avgMonthlyLoss,
		GainLossRatioMonthly: gainLossRatioMonthly, AvgAnnualGain: avgAnnualGain,
		AvgAnnualLoss: avgAnnualLoss, GainLossRatioAnnual: gainLossRatioAnnual, SWR: swr, SWR10Y: swr10y,
		PWR10Y: pwr10y, SWR20Y: swr20y, PWR20Y: pwr20y, SWR30Y: swr30y, PWR30Y: pwr30y, SWR40Y: swr40y,
		PWR40Y: pwr40y,
	}
}
