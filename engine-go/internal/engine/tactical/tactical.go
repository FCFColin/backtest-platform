...
func collectTickers(strategy TacticalStrategy) []string {
	...
	for _, sig := range strategy.Signals {
		for _, w := range sig.TargetWeights { set[w.Ticker] = true }
	}
...
func aggregateWeightedAverage(activeSignals []TradingSignal, allTickers []string) []WeightEntry {
	...
	for _, sig := range activeSignals {
		for _, w := range normalizeWeights(sig.TargetWeights, allTickers) { acc[w.Ticker] += w.Weight }
	}
	total := 0.0
...
if rc.Method != "" { method = rc.Method }
	...
	for _, sig := range activeSignals {
		for _, w := range normalizeWeights(sig.TargetWeights, allTickers) { score[w.Ticker] += w.Weight }
	}
	type ts struct {
		ticker string
		score  float64
...
} else { eq := 1 / float64(len(ranked))
		for i := range weights { weights[i] = eq }
	}
...
func aggregateSignals(strategy TacticalStrategy, activeFlags map[string][]bool, dateIdx int, allTickers []string) []WeightEntry {
	...
	for _, sig := range strategy.Signals {
if flags, ok := activeFlags[sig.ID]; ok && dateIdx < len(flags) && flags[dateIdx] { activeSignals = append(activeSignals, sig) }
	}
...
if priceMap == nil { priceMap = make(map[string]float64) }
		...
		for i, d := range dates {
if p, ok := priceMap[d]; ok { lastValid = p }
...
if prices[i-period] > 0 { result[i] = (prices[i]/prices[i-period] - 1) * 100 }
	}
	return result
}
...
func computeIndicatorValue(indicator TechnicalIndicator, prices []float64, period int) []*float64 {
	...
	default: raw = make([]float64, len(prices))
		for i := range raw { raw[i] = nan }
	}
...
if pp > 0 { holdings[t] *= req.PriceData[t][date] / pp }
		...
		if portfolioValue <= 0 {
			portfolioValue = 0
			for _, t := range allTickers { holdings[t] = 0 }
		...
		if !initialized || engineutil.ShouldRebalance(req.RebalanceFrequency, prevDate, date, 0, nil, nil, 0, nil) {
			...
			var names []string
			for _, sig := range strategy.Signals {
if f, ok := activeFlags[sig.ID]; ok && i < len(f) && f[i] { names = append(names, sig.Name) }
			}
...
if values[i-1] > 0 { r = (values[i] - values[i-1]) / values[i-1] }
		...
		stats = engine.CalculateStatisticsFromRequest(engine.StatisticsRequest{
			Values: values, Dates: dts, StartingValue: req.StartingValue,
			DailyReturns: dailyRets, AnnualReturnValues: []float64{}, MonthlyReturnValues: []float64{},
	...
	for i := 0; i < len(prices); i++ {
		...
		if ind == "rsi" {
			...
			if canRebalance {
				if !inPos && rsi[i] < p2 {
					inPos = true
} else if inPos && rsi[i] > 100-p2 { inPos = false
				}
			}
} else { if math.IsNaN(ma[i]) { signals[i], prevDate = inPos, dates[i]; continue }
			upper, lower := ma[i]*(1+threshold), ma[i]*(1-threshold)
			if canRebalance {
				if !inPos && prices[i] > upper {
					inPos = true
} else if inPos && prices[i] < lower { inPos = false
				}
			}
		}
		signals[i], prevDate = inPos, dates[i]
	...
	for _, p1 := range p1Vals {
		for _, p2 := range p2Vals {
			...
			btReq := engine.BacktestRequest{
				Portfolios: []engine.PortfolioInput{{Name: "grid-" + ftoa(p1) + "-" + ftoa(p2),
					Assets:             []engine.AssetInput{{Ticker: req.TradingTicker, Weight: 100}},
					RebalanceFrequency: "none"}}, PriceData: map[string]map[string]float64{req.TradingTicker: synthetic},
				Params: engine.BacktestParams{StartDate: req.StartDate, EndDate: req.EndDate,
					StartingValue: req.StartingValue, RollingWindowMonths: 12,
				},
			}
			...
			m := GridCombinationMetrics{
				Param1: p1, Param2: p2,
				CAGR: pr.Statistics.CAGR, MaxDrawdown: pr.Statistics.MaxDrawdown,
				Sharpe: pr.Statistics.Sharpe, TotalReturn: pr.Statistics.TotalReturn,
				Stdev: pr.Statistics.Stdev, Calmar: pr.Statistics.Calmar,
			}
...
if len(topResults) > 0 { best = &topResults[0] }
	return &TacticalGridResponse{
		TotalCombinations: total, AllMetrics: allMetrics, TopResults: topResults,
		Heatmap: HeatmapData{Param1Label: param1Label, Param2Label: param2Label, Param1Values: p1Vals,
			Param2Values: p2Vals, Matrix: matrix, Objective: req.Objective,
		},
