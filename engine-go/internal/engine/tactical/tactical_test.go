...
func makeDates(startDate string, n int) []string {
	...
	dates := make([]string, n)
	for i := 0; i < n; i++ { dates[i] = t.AddDate(0, 0, i).Format("2006-01-02") }
	return dates
}
func makePriceMap(ticker string, dates []string, prices []float64) map[string]map[string]float64 {
	...
	for i, d := range dates {
if i < len(prices) { pd[ticker][d] = prices[i] }
	}
	return pd
}
...
if r == nil { t.Fatal("应返回非 nil 结果") }
if len(r.Portfolio.GrowthCurve) != 0 { t.Errorf("空 dates 应无 GrowthCurve, got %d", len(r.Portfolio.GrowthCurve)) }
if len(r.SignalHistory) != 0 { t.Errorf("空 dates 应无 SignalHistory, got %d", len(r.SignalHistory)) }
}
func TestRunTacticalBacktest_NoSignals(t *testing.T) {
	...
	r, err := RunTacticalBacktest(context.Background(), req)
if err != nil { t.Fatalf("不应报错: %v", err) }
if len(r.Portfolio.GrowthCurve) != len(req.Dates) { t.Errorf("GrowthCurve 长度=%d, want %d", len(r.Portfolio.GrowthCurve), len(req.Dates)) }
if len(r.Portfolio.GrowthCurve) > 0 && math.Abs(r.Portfolio.GrowthCurve[0].Value-10000) > 1e-6 { t.Errorf("首点应为初始值 10000, got %v", r.Portfolio.GrowthCurve[0].Value) }
if len(r.SignalHistory) == 0 { t.Error("daily 再平衡应产生 SignalHistory 条目") }
}
...
if h.Date == "" { t.Errorf("SignalHistory[%d] 日期为空", i) }
		sum := 0.0
		for _, w := range h.Weights { sum += w.Weight }
if math.Abs(sum-1.0) > 0.01 { t.Errorf("SignalHistory[%d] 权重和=%v, want≈1", i, sum) }
	}
}
...
if err != nil { t.Fatalf("不应报错: %v", err) }
if r.TotalCombinations != 4 || len(r.AllMetrics) != 4 { t.Errorf("TotalCombinations=%d, AllMetrics len=%d, want 4/4", r.TotalCombinations, len(r.AllMetrics)) }
if len(r.Heatmap.Param1Values) != 2 || len(r.Heatmap.Param2Values) != 2 || len(r.Heatmap.Matrix) != 2 { t.Fatalf("Heatmap 结构错误: P1=%d, P2=%d, Matrix=%d", len(r.Heatmap.Param1Values), len(r.Heatmap.Param2Values), len(r.Heatmap.Matrix)) }
	for i, row := range r.Heatmap.Matrix {
if len(row) != 2 { t.Errorf("Matrix[%d] 列数=%d, want 2", i, len(row)) }
	}
if r.BestCombination == nil { t.Error("BestCombination 不应为 nil") }
if len(r.TopResults) > 4 || len(r.TopResults) > r.TotalCombinations { t.Errorf("TopResults 长度=%d, 应 <= 4 且 <= TotalCombinations(%d)", len(r.TopResults), r.TotalCombinations) }
}
func TestRunGridSearch_TopN(t *testing.T) {
	...
	r, err := RunGridSearch(context.Background(), req)
if err != nil { t.Fatalf("不应报错: %v", err) }
if len(r.TopResults) != 2 { t.Errorf("TopN=2 时 TopResults 长度=%d, want 2", len(r.TopResults)) }
...
if vPrev != vCurr && (vPrev < vCurr) != ascending { t.Errorf("TopResults 目标值非单调: idx %d→%d 方向变化", j-1, j) }
		}
		break
	}
}
func TestGenerateRange(t *testing.T) {
cases := []struct { name string; minVal, maxVal, step float64; wantLen int; wantFirst, wantLast float64 }{ {"正常范围", 1, 5, 1, 5, 1, 5}, {"step<=0返回单元素", 1, 5, 0, 1, 1, 1}, {"浮点步长", 0, 1, 0.5, 3, 0, 1} }
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			r := generateRange(tc.minVal, tc.maxVal, tc.step)
if len(r) != tc.wantLen { t.Errorf("应生成 %d 个值, got %d", tc.wantLen, len(r)) }
if len(r) > 0 && (r[0] != tc.wantFirst || r[len(r)-1] != tc.wantLast) { t.Errorf("范围应为 [%v,%v], got [%v,%v]", tc.wantFirst, tc.wantLast, r[0], r[len(r)-1]) }
		})
	}
}
func TestGetObjectiveValue(t *testing.T) {
	m := GridCombinationMetrics{CAGR: 0.1, MaxDrawdown: 0.2, Sharpe: 1.5}
cases := []struct { name string; objective string; want float64 }{ {"maxCAGR", "maxCAGR", 0.1}, {"minDrawdown取负", "minDrawdown", -0.2}, {"maxSharpe", "maxSharpe", 1.5}, {"未知目标默认CAGR", "unknown", 0.1} }
	for _, tc := range cases {
t.Run(tc.name, func(t *testing.T) { if got := getObjectiveValue(m, tc.objective); got != tc.want { t.Errorf("getObjectiveValue(%v) = %v, want %v", tc.objective, got, tc.want) } })
	}
}
