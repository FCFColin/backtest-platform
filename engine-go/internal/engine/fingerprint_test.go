...
if err != nil { t.Fatalf("ComputeFingerprint failed: %v", err) }
if fp1 == "" { t.Fatal("fingerprint should not be empty") }
	fp2, err := ComputeFingerprint(r1)
if err != nil { t.Fatalf("ComputeFingerprint failed: %v", err) }
if fp1 != fp2 { t.Fatalf("fingerprint not deterministic: %s vs %s", fp1, fp2) }
}
...
if len(sampled) != 3 { t.Fatalf("expected 3 sampled points, got %d", len(sampled)) }
if sampled[0].Date != "2020-01-01" { t.Fatalf("first sampled point should be first date") }
if sampled[len(sampled)-1].Date != "2020-01-05" { t.Fatalf("last sampled point should be last date") }
}
