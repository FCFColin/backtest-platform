...
func TestCalcVaR_ConfidenceEdgeCases(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			defer func() {
				if r := recover(); r != nil {
					t.Fatalf("CalcVaR panicked for conf=%v: %v", c.confidence, r)
				}
			}()
			got := CalcVaR(returns, c.confidence)
			if math.IsNaN(got) || math.IsInf(got, 0) {
				t.Errorf("CalcVaR(conf=%v) = %v, want finite 0", c.confidence, got)
			}
			if got != 0 {
...
func TestCalcVaR_ConfidenceSubnormal(t *testing.T) {
	...
	defer func() {
		if r := recover(); r != nil {
			t.Fatalf("CalcVaR panicked for subnormal confidence: %v", r)
		}
	}()
...
func TestCalcVaR_NilSlice(t *testing.T) {
	defer func() {
		if r := recover(); r != nil {
			t.Fatalf("CalcVaR panicked on nil slice: %v", r)
		}
	}()
...
func TestCalcCVaR_ConfidenceEdgeCases(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			defer func() {
				if r := recover(); r != nil {
					t.Fatalf("CalcCVaR panicked for conf=%v: %v", c.confidence, r)
				}
			}()
			got := CalcCVaR(returns, c.confidence)
			if math.IsNaN(got) || math.IsInf(got, 0) {
				t.Errorf("CalcCVaR(conf=%v) = %v, want finite 0", c.confidence, got)
			}
			if got != 0 {
...
func TestCalcCVaR_ConfidenceSubnormal(t *testing.T) {
	...
	defer func() {
		if r := recover(); r != nil {
			t.Fatalf("CalcCVaR panicked for subnormal confidence: %v", r)
		}
	}()
...
func TestCalcCVaR_NilSlice(t *testing.T) {
	defer func() {
		if r := recover(); r != nil {
			t.Fatalf("CalcCVaR panicked on nil slice: %v", r)
		}
	}()
...
func TestCalcSharpe_ZeroVolatility(t *testing.T) {
	...
	if math.IsNaN(got) || math.IsInf(got, 0) {
		t.Errorf("CalcSharpe with zero stdev = %v, want finite 0", got)
	}
	if got != 0 {
...
func TestCalcSortino_NilSlice(t *testing.T) {
	defer func() {
		if r := recover(); r != nil {
			t.Fatalf("CalcSortino panicked on nil slice: %v", r)
		}
	}()
...
func TestCalcMaxDrawdown_NilSlice(t *testing.T) {
	defer func() {
		if r := recover(); r != nil {
			t.Fatalf("CalcMaxDrawdown panicked on nil slice: %v", r)
		}
	}()
...
func TestCalculateStatisticsFromRequest_NilSlices(t *testing.T) {
	defer func() {
		if r := recover(); r != nil {
			t.Fatalf("CalculateStatisticsFromRequest panicked on nil slices: %v", r)
		}
	}()
	...
	if math.IsNaN(stats.Sharpe) || math.IsInf(stats.Sharpe, 0) {
		t.Errorf("Sharpe = %v, want finite", stats.Sharpe)
	}
	if stats.Sharpe != 0 {
