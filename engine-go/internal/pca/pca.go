...
func PerformPCA(req PCARequest) (*PCAResult, error) {
	...
	if len(commonDates) < 2 { return nil, errors.New("有效价格数据不足，至少需要 2 个交易日") }
	nTickers := len(tickers)
	nDates := len(commonDates)
	prices := make([][]float64, nDates)
	for i := range prices {
		prices[i] = make([]float64, nTickers)
		for j := 0; j < nTickers; j++ { prices[i][j] = priceData[tickers[j]][commonDates[i]] }
	}
	nReturns := nDates - 1
	...
	for i := 0; i < nReturns; i++ {
		...
		for j := 0; j < nTickers; j++ {
			prev := prices[i][j]
			curr := prices[i+1][j]
if prev != 0 { returns[i][j] = (curr - prev) / prev }
		}
	}
	...
	for j := 0; j < nTickers; j++ {
		...
		for i := 0; i < nReturns; i++ { col[i] = returns[i][j] }
...
if stds[j] == 0 { stds[j] = 1 }
	...
	for i := 0; i < nReturns; i++ {
		...
		for j := 0; j < nTickers; j++ { stdReturns[i][j] = (returns[i][j] - means[j]) / stds[j] }
	}
	cov := make([][]float64, nTickers)
	for j := range cov { cov[j] = make([]float64, nTickers) }
	stdCols := make([][]float64, nTickers)
	for j := 0; j < nTickers; j++ {
		stdCols[j] = make([]float64, nReturns)
		for i := 0; i < nReturns; i++ { stdCols[j][i] = stdReturns[i][j] }
	}
	for j := 0; j < nTickers; j++ {
		for k := 0; k < nTickers; k++ { cov[j][k] = mathutil.Covariance(stdCols[j], stdCols[k]) }
	}
	covFlat := make([]float64, nTickers*nTickers)
	for i := 0; i < nTickers; i++ {
		for j := 0; j < nTickers; j++ { covFlat[i*nTickers+j] = cov[i][j] }
	}
	covSym := mat.NewSymDense(nTickers, covFlat)
	var es mat.EigenSym
	if !es.Factorize(covSym, true) { return nil, errors.New("协方差矩阵特征值分解失败") }
	...
	for i := 0; i < nTickers; i++ {
		...
		for j := 0; j < nTickers; j++ { rawEigenvectors[i][j] = rawEigen.At(i, j) }
	}
	...
	for i := 0; i < nTickers; i++ { pairs[i] = eigenPair{val: rawEigenvalues[i], idx: i} }
	sort.Slice(pairs, func(i, j int) bool { return pairs[i].val > pairs[j].val })
	sortedEigenvalues := make([]float64, nTickers)
	sortedEigenvectors := make([][]float64, nTickers)
	for i := 0; i < nTickers; i++ { sortedEigenvectors[i] = make([]float64, nTickers) }
	for compIdx := 0; compIdx < nTickers; compIdx++ {
		srcIdx := pairs[compIdx].idx
		sortedEigenvalues[compIdx] = pairs[compIdx].val
for tickerIdx := 0; tickerIdx < nTickers; tickerIdx++ { sortedEigenvectors[tickerIdx][compIdx] = rawEigenvectors[tickerIdx][srcIdx] }
	}
	totalVar := 0.0
...
if v > 0 { cumSum += v }
if totalVar > 0 { cumulativeVariance[i] = cumSum / totalVar }
	}
	...
	for i := 0; i < nReturns; i++ {
		...
		for compIdx := 0; compIdx < nTickers; compIdx++ {
			sum := 0.0
			for j := 0; j < nTickers; j++ { sum += stdReturns[i][j] * sortedEigenvectors[j][compIdx] }
			scores[i][compIdx] = sum
		}
	}
	keep := nTickers
if req.NumComponents != nil && *req.NumComponents > 0 && *req.NumComponents < nTickers { keep = *req.NumComponents }
	result := &PCAResult{
	...
	for i := 0; i < nTickers; i++ { result.Loadings[i] = sortedEigenvectors[i][:keep] }
	for i := 0; i < nReturns; i++ { result.Scores[i] = scores[i][:keep] }
	return result, nil
}
