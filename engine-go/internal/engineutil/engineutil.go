...
func ShouldRebalance(
	...
	freqTrigger := false
	switch frequency {
	case "daily": freqTrigger = true
	case "none": return false
case "weekly": if datesParsed { _, pw := prevTime.ISOWeek(); _, cw := currTime.ISOWeek(); freqTrigger = cw != pw || currTime.Year() != prevTime.Year() }
case "monthly": if datesParsed { freqTrigger = currTime.Month() != prevTime.Month() || currTime.Year() != prevTime.Year() }
case "quarterly": if datesParsed { pq := (int(prevTime.Month()) - 1) / 3; cq := (int(currTime.Month()) - 1) / 3; freqTrigger = pq != cq || prevTime.Year() != currTime.Year() }
	case "annual": if datesParsed { freqTrigger = prevTime.Year() != currTime.Year() }
	case "threshold": if threshold > 0 && pv > 0 {
			for j := range holdings {
				if weights[j] == 0 { continue }
				actual := holdings[j] / pv
				dev := math.Abs(actual-weights[j]) / math.Abs(weights[j]) * 100
				if dev >= threshold { return true }
			}
		}
		return false
	default: return false
	}
	if freqTrigger { return true }
	if bands != nil {
		for i, w := range weights {
			actual := 0.0
if pv > 0 { actual = holdings[i] / pv }
			drift := actual - w
			if bands.AbsoluteBand != nil && math.Abs(drift) > *bands.AbsoluteBand/100 { return true }
			if bands.RelativeBand != nil && w > 0 && math.Abs(drift)/w > *bands.RelativeBand/100 { return true }
		}
	}
	return false
}
func NormalizeWeights(weights []float64) []float64 {
	...
	copy(result, weights)
	sum := 0.0
	for _, v := range result { sum += v }
	if sum <= 0 {
		for i := range result { result[i] = 1.0 / float64(n) }
		return result
	}
	for i := range result { result[i] /= sum }
	return result
}
...
func AlignDates(tickers []string, priceData map[string]map[string]float64) []string {
	...
	for i, t := range tickers {
		...
		if pd, ok := priceData[t]; ok {
			for d := range pd { ds[d] = true }
		}
		dateSets[i] = ds
	}
	if len(dateSets) == 0 || len(dateSets[0]) == 0 { return nil }
...
if all { commonDates = append(commonDates, d) }
	}
...
func GetSortedDates(priceData map[string]map[string]float64, tickers []string) []string {
	...
	for _, ticker := range tickers {
		if td, ok := priceData[ticker]; ok {
			for date := range td { dateSet[date] = struct{}{} }
		}
	}
...
if startDate != "" { start, _ = time.Parse("2006-01-02", startDate) }
if endDate != "" { end, _ = time.Parse("2006-01-02", endDate) }
	filtered := make([]time.Time, 0, len(dates))
	for _, d := range dates {
		if !start.IsZero() && d.Before(start) { continue }
		if !end.IsZero() && d.After(end) { continue }
...
func ExtractPrices(priceData map[string]map[string]float64, ticker string, dates []time.Time) []float64 {
	...
	if !ok { return make([]float64, len(dates)) }
	prices := make([]float64, len(dates))
	for i, d := range dates {
		dateStr := d.Format("2006-01-02")
if p, exists := tickerData[dateStr]; exists { prices[i] = p }
	}
	return prices
}
