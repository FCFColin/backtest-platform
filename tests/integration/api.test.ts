...
beforeAll(async () => { serverAvailable = await checkServerAvailable(API_BASE_URL); });
...
describe('E2E - 回测API正常场景', () => {
  skip('POST /api/backtest/portfolio - 正常回测VTI+BND', async () => {
    const { json } = await postPortfolio([
      { name: 'Test', assets: [{ ticker: 'VTI', weight: 60 }, { ticker: 'BND', weight: 40 }], rebalanceFrequency: 'quarterly' },
...
describe('E2E - 回测API做空场景', () => {
  skip('POST /api/backtest/portfolio - 做空场景不返回负值', async () => {
    const { res, json } = await postPortfolio([
      { name: 'Short Test', assets: [{ ticker: 'VTI', weight: 200 }, { ticker: 'NVDA', weight: -100 }], rebalanceFrequency: 'none' },
    ], { ...BASE_PARAMS, startDate: '2023-01-01' });
    expect(res.ok).toBe(true);
    // 所有value >= 0（爆仓后归零，不应有负值）
    expect(json.data.portfolios[0].growthCurve.filter((p: { value: number }) => p.value < 0)).toHaveLength(0);
  });
  skip('极端做空爆仓后CAGR=-1', async () => {
    const { json } = await postPortfolio([
      { name: '爆仓', assets: [{ ticker: 'SPY', weight: 300 }, { ticker: 'NVDA', weight: -200 }], rebalanceFrequency: 'none' },
    ], { ...BASE_PARAMS, startDate: '2023-01-01' });
    // 可能爆仓也可能不爆仓，但不应有负值
    expect(json.data.portfolios[0].growthCurve.filter((p: { value: number }) => p.value < 0)).toHaveLength(0);
  });
...
describe('E2E - 回测API偏离调仓', () => {
  skip('POST /api/backtest/portfolio - 偏离调仓不比季度调仓更易爆仓', async () => {
    const assets = [{ ticker: 'VTI', weight: 100 }, { ticker: 'BND', weight: 100 }, { ticker: 'AAPL', weight: -100 }];
    const { json: jsonQ } = await postPortfolio([{ name: 'Q', assets, rebalanceFrequency: 'quarterly' }]);
    const { json: jsonT } = await postPortfolio([{ name: 'T', assets, rebalanceFrequency: 'threshold', rebalanceThreshold: 5 }]);
    // 如果季度没爆仓，偏离也不应该爆
...
describe('E2E - 回测API错误处理', () => {
  skip('POST /api/backtest/portfolio - 无效ticker返回错误', async () => {
    const { json } = await postPortfolio([
      { name: 'Test', assets: [{ ticker: 'INVALID_TICKER_XYZ', weight: 100 }], rebalanceFrequency: 'none' },
    ], { ...BASE_PARAMS, startDate: '2020-01-01' });
    expect(json.success).toBe(false);
  });
  skip('POST /api/backtest/portfolio - 权重百分比正确处理', async () => {
    const { json } = await postPortfolio([
      { name: 'Weight Test', assets: [{ ticker: 'VTI', weight: 60 }, { ticker: 'BND', weight: 40 }], rebalanceFrequency: 'none' },
...
describe('E2E - 回测API数据一致性', () => {
  ...
  skip('增长曲线首日价值≈startingValue', async () => {
    const { json } = await postPortfolio([
      { name: 'Test', assets: [{ ticker: 'VTI', weight: 60 }, { ticker: 'BND', weight: 40 }], rebalanceFrequency: 'none' },
    ], { ...BASE_PARAMS, startDate: '2020-01-01', endDate: '2020-12-31' });
