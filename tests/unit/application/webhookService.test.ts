...
vi.mock('../../../packages/backend/src/db/pool.js', () => ({
  ...
  withTenant: vi.fn(async (_t: string, fn: (c: { query: typeof poolMocks.pool.query }) => Promise<void>) => { await fn({ query: poolMocks.pool.query }); }),
}));
const fetchMock = vi.hoisted(() => vi.fn());
vi.stubGlobal('fetch', fetchMock);
const ssrfMock = vi.hoisted(() => ({ assertSafeUrl: vi.fn().mockResolvedValue(undefined) }));
vi.mock('../../../packages/backend/src/utils/ssrfGuard.js', () => ({ assertSafeUrl: ssrfMock.assertSafeUrl }));
const envEncMock = vi.hoisted(() => ({
  encrypt: vi.fn().mockResolvedValue({ ciphertext: Buffer.from('enc'), iv: Buffer.from('iv'), tag: Buffer.from('tag'), kid: 'default' }),
  decrypt: vi.fn(),
}));
vi.mock('../../../packages/backend/src/utils/crypto.js', () => ({ encrypt: envEncMock.encrypt, decrypt: envEncMock.decrypt }));

import { signPayload, deliverWebhook, triggerWebhooks, processPendingDeliveries, cleanupOldDeliveries, WEBHOOK_CONSTANTS } from '../../../packages/backend/src/application/webhookService.js';
const { RETRY_DELAYS_MS, MAX_ATTEMPTS, AUTO_DISABLE_THRESHOLD, MAX_DELIVERIES_PER_ENDPOINT } = WEBHOOK_CONSTANTS;
...
function makeDeliveryRow(overrides: Record<string, unknown> = {}) {
  return { delivery_id: DELIVERY_ID, endpoint_id: ENDPOINT_ID, event_type: 'BacktestCompleted', payload: { runId: 'run-1' }, attempt_count: 0, url: URL, secret: Buffer.from('enc'), secret_iv: Buffer.from('iv'), secret_tag: Buffer.from('tag'), secret_kid: 'default', org_id: ORG_ID, ...overrides };
}
const findCall = (frag: string) => poolMocks.pool.query.mock.calls.find((c: unknown[]) => typeof c[0] === 'string' && (c[0] as string).includes(frag));
...
describe('webhookService', () => {
  ...
  describe('signPayload', () => {
    it('应生成与独立 HMAC-SHA256 计算一致的十六进制签名', () => {
      ...
      expect(signPayload(payload, SECRET)).toBe(crypto.createHmac('sha256', SECRET).update(payload).digest('hex'));
    });
  ...
  describe('deliverWebhook', () => {
    it.each([
      ['2xx', { ok: true, status: 200, text: async () => 'ok' }, { success: true, code: 200, body: 'ok' }],
      ['5xx', { ok: false, status: 503, text: async () => 'unavailable' }, { success: false, code: 503, body: undefined }],
      ...
      expect(r.success).toBe(expected.success); expect(r.responseCode).toBe(expected.code);
      if (expected.body) expect(r.responseBody).toBe(expected.body);
    });
    it('网络错误/超时应返回 success=false 且 responseCode=null', async () => {
      fetchMock.mockRejectedValue(new Error('fetch failed'));
      const r = await deliverWebhook({ url: URL, secret: SECRET }, 'Test', { a: 1 });
      expect(r.success).toBe(false); expect(r.responseCode).toBeNull(); expect(r.responseBody).toContain('fetch failed');
    });
    ...
    it('X-Webhook-Signature 应为 body 的正确 HMAC', async () => {
      ...
      expect(init.headers['X-Webhook-Signature']).toBe('sha256=' + crypto.createHmac('sha256', SECRET).update(init.body).digest('hex'));
    });
    it('响应体应截断至 1000 字符', async () => {
      fetchMock.mockResolvedValue({ ok: true, status: 200, text: async () => 'x'.repeat(2500) });
      expect((await deliverWebhook({ url: URL, secret: SECRET }, 'Test', {})).responseBody.length).toBe(1000);
    });
    it('SSRF 校验失败应返回 permanentFailure 且不调用 fetch（C-003）', async () => {
      ...
      const r = await deliverWebhook({ url: URL, secret: SECRET }, 'Test', { a: 1 });
      expect(r.success).toBe(false); expect(r.responseCode).toBeNull();
      expect(r.permanentFailure).toBe(true); expect(r.responseBody).toContain('SSRF blocked');
  ...
  describe('triggerWebhooks', () => {
    it('无匹配订阅时应跳过 INSERT（仅探测查询）', async () => {
      ...
      expect(poolMocks.pool.query.mock.calls[0][0] as string).toContain('SELECT 1 FROM webhook_endpoints');
    });
    it('有匹配订阅时应执行 INSERT...SELECT 创建 pending 投递', async () => {
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [{ 1: 1 }] }).mockResolvedValueOnce({ rowCount: 2 });
      await triggerWebhooks(ORG_ID, 'BacktestCompleted', { runId: 'r1' });
      expect(poolMocks.pool.query).toHaveBeenCalledTimes(2);
      const insertCall = poolMocks.pool.query.mock.calls[1];
      const sql = insertCall[0] as string;
      expect(sql).toContain('INSERT INTO webhook_deliveries'); expect(sql).toContain("'pending'");
      expect(insertCall[1]).toEqual([ORG_ID, 'BacktestCompleted', JSON.stringify({ runId: 'r1' })]);
    });
    it('仅活跃端点订阅该事件才被纳入（WHERE 子句）', async () => {
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [{ 1: 1 }] }).mockResolvedValueOnce({ rowCount: 1 });
      await triggerWebhooks(ORG_ID, 'RunCompleted', { id: 'r2' });
      const sql = poolMocks.pool.query.mock.calls[1][0] as string;
      expect(sql).toContain('org_id = $1'); expect(sql).toContain('is_active = TRUE'); expect(sql).toContain('= ANY(subscribed_events)');
    });
  });
  ...
  describe('processPendingDeliveries', () => {
    ...
    it('投递成功应标记 success 并重置端点 failed_consecutive_count=0', async () => {
      ...
      expect(dSql).toContain("status = 'success'"); expect(dSql).toContain('delivered_at = NOW()');
    ...
    it('投递失败且未达上限应标记 retrying 并安排 next_retry_at', async () => {
      ...
      expect(dSql).toContain("status = 'retrying'"); expect(dSql).toContain('next_retry_at = $4');
    ...
    it('达到 MAX_ATTEMPTS 应标记 failed 并递增端点失败计数', async () => {
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [makeDeliveryRow({ attempt_count: MAX_ATTEMPTS - 1 })] });
      ...
      expect(findCall('UPDATE webhook_endpoints')![0] as string).toContain('failed_consecutive_count = failed_consecutive_count + 1');
    });
    it('连续失败达到 AUTO_DISABLE_THRESHOLD(5) 应自动禁用端点', async () => {
      ...
      expect(eSql).toContain('is_active = CASE'); expect(eSql).toContain('THEN FALSE');
      expect(eSql).toContain('disabled_at = CASE'); expect(eSql).toContain('THEN NOW()');
  ...
  describe('cleanupOldDeliveries', () => {
    it('应使用 ROW_NUMBER 窗口函数删除每端点超出上限的旧记录', async () => {
      ...
      expect(sql).toContain('DELETE FROM webhook_deliveries'); expect(sql).toContain('ROW_NUMBER()');
      expect(sql).toContain('PARTITION BY endpoint_id'); expect(sql).toContain('ORDER BY created_at DESC');
      expect(sql).toContain('rn > $1'); expect(call[1]).toEqual([MAX_DELIVERIES_PER_ENDPOINT]);
    });
  ...
  describe('WEBHOOK_CONSTANTS', () => {
    it('AUTO_DISABLE_THRESHOLD 应为 5', () => { expect(AUTO_DISABLE_THRESHOLD).toBe(5); });
    it('MAX_DELIVERIES_PER_ENDPOINT 应为 100', () => { expect(MAX_DELIVERIES_PER_ENDPOINT).toBe(100); });
  });
});