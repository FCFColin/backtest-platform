...
vi.mock('../../../packages/backend/src/db/pool.js', () => ({ getPool: vi.fn(() => poolMocks.pool) }));
const minioMocks = vi.hoisted(() => ({ ensureBucketExists: vi.fn().mockResolvedValue(undefined), uploadAuditObject: vi.fn().mockResolvedValue(true), isMinioConfigured: vi.fn(() => true) }));
vi.mock('../../../packages/backend/src/infrastructure/minioClient.js', () => ({ ensureBucketExists: minioMocks.ensureBucketExists, uploadAuditObject: minioMocks.uploadAuditObject, isMinioConfigured: minioMocks.isMinioConfigured }));
import { config } from '../../../packages/backend/src/config/index.js';
import { signAuditEntry, writeAuditLog, getUnexportedAuditLogs, markExported, queryAuditLogs, verifyAuditIntegrity, type AuditLogEntry } from '../../../packages/backend/src/application/auditStorageService.js';
...
const hmac = (p: unknown) => crypto.createHmac('sha256', TEST_KEY).update(typeof p === 'string' ? p : JSON.stringify(p)).digest('hex');
...
const findUpdateCall = () => poolMocks.pool.query.mock.calls.find((c: unknown[]) => typeof c[0] === 'string' && (c[0] as string).includes('UPDATE audit_logs'));
function makeEntry(o: Partial<AuditLogEntry> = {}): AuditLogEntry {
  return { eventType: 'AuditEvent', userId: USER_ID, orgId: ORG_ID, ipAddress: '127.0.0.1', action: 'CREATE', resourceType: 'backtest_run', resourceId: 'run-1', payload: { method: 'POST', path: '/api/v1/backtest', statusCode: 200 }, ...o };
}
function makeDbRow(o: Record<string, unknown> = {}) {
  return { id: LOG_ID, event_type: 'AuditEvent', user_id: USER_ID, org_id: ORG_ID, ip_address: '127.0.0.1', action: 'CREATE', resource_type: 'backtest_run', resource_id: 'run-1', payload: { method: 'POST', path: '/api/v1/backtest' }, hmac_signature: '', object_key: null, exported_at: null, created_at: new Date('2026-07-25T10:00:00Z'), ...o };
}
function setupExport(payload: unknown, sig: string) {
  poolMocks.pool.query.mockResolvedValueOnce({ rows: [makeDbRow({ payload, hmac_signature: sig })] });
...
describe('auditStorageService', () => {
  ...
  afterEach(() => { config.AUDIT_HMAC_KEY = originalKey; vi.restoreAllMocks(); });
  describe('signAuditEntry', () => {
    ...
    it('不同 payload 应产生不同签名', () => { expect(signAuditEntry('{"a":1}')).not.toBe(signAuditEntry('{"a":2}')); });
    it('未配置 AUDIT_HMAC_KEY 时应返回空字符串', () => { config.AUDIT_HMAC_KEY = ''; expect(signAuditEntry('test')).toBe(''); });
  });
  describe('writeAuditLog', () => {
    ...
    it('可选字段为 null 时应传 null 而非 undefined', async () => {
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [] }).mockResolvedValueOnce({ rows: [{ id: LOG_ID }] });
      await writeAuditLog(makeEntry({ userId: null, orgId: null, resourceType: null, resourceId: null }));
      const args = callArgs(1);
  ...
  describe('getUnexportedAuditLogs', () => {
    ...
    it('DB 行应正确映射为 camelCase + ISO 时间戳', async () => {
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [makeDbRow({ exported_at: new Date('2026-07-25T12:00:00Z'), object_key: 'audit/2026/07/25/batch.jsonl' })] });
      const log = (await getUnexportedAuditLogs())[0];
      expect(log).toMatchObject({ eventType: 'AuditEvent', orgId: ORG_ID, hmacSignature: '', objectKey: 'audit/2026/07/25/batch.jsonl' });
  ...
  describe('queryAuditLogs', () => {
    it('无过滤条件时应查询全部（无 WHERE 子句）', async () => {
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [makeDbRow()] }).mockResolvedValueOnce({ rows: [{ total: 1 }] });
    ...
    it.each([
      ...
      ['多过滤条件组合', { orgId: ORG_ID, eventType: 'AuditEvent', userId: USER_ID, action: 'CREATE', startDate: '2026-07-01', endDate: '2026-07-31' }, 'org_id = $1', [ORG_ID]],
    ])('应支持 %s 过滤', async (_n, filter, expectedSql) => {
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [] }).mockResolvedValueOnce({ rows: [{ total: 0 }] });
      await queryAuditLogs(filter);
      expect(callSql(0)).toContain(expectedSql);
    });
    it('分页应正确计算 OFFSET（page=3, limit=20 → offset=40）', async () => {
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [] }).mockResolvedValueOnce({ rows: [{ total: 100 }] });
    ...
    it('总数查询应使用相同的 WHERE 条件', async () => {
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [] }).mockResolvedValueOnce({ rows: [{ total: 5 }] });
  ...
  describe('verifyAuditIntegrity', () => {
    it.each([
      { name: '有效签名应返回 valid=true', payload: { method: 'POST', path: '/api/v1/backtest' }, sigFn: (p: unknown) => hmac(p), valid: true },
      { name: '篡改 payload 后应返回 valid=false', payload: { method: 'DELETE', path: '/api/v1/admin/keys/xxx' }, sigFn: () => hmac({ method: 'POST', path: '/api/v1/backtest' }), valid: false },
      { name: '篡改 hmac_signature 后应返回 valid=false', payload: { method: 'POST' }, sigFn: () => 'a'.repeat(64), valid: false },
    ])('$name', async ({ payload, sigFn, valid }) => {
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [{ payload, hmac_signature: sigFn(payload) }] });
      expect((await verifyAuditIntegrity(LOG_ID)).valid).toBe(valid);
    });
    it('日志不存在时应返回 valid=false + 空签名', async () => {
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [] });
      expect(await verifyAuditIntegrity(LOG_ID)).toMatchObject({ valid: false, expected: '', actual: '' });
    });
    it('未配置 AUDIT_HMAC_KEY 时应返回 valid=false（fail-closed，D2-010）', async () => {
      config.AUDIT_HMAC_KEY = '';
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [{ payload: { a: 1 }, hmac_signature: 'some-sig' }] });
  ...
  describe('exportPendingAuditLogs', () => {
    ...
    it('MinIO 已配置时应上传 JSONL 并标记已导出', async () => {
      ...
      const parsed = JSON.parse((minioMocks.uploadAuditObject.mock.calls[0][1] as string).split('\n')[0]);
      expect(parsed).toMatchObject({ id: LOG_ID, eventType: 'AuditEvent', hmacSignature: signature });
    ...
    it('HMAC 校验失败的记录应被跳过（不写入 WORM）', async () => {
      ...
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [makeDbRow({ payload, hmac_signature: tamperedSig })] });
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [{ payload, hmac_signature: tamperedSig }] });
      const result = await exportPendingAuditLogs();
    ...
    it('上传失败时不应标记已导出（下次重试）', async () => {
      const payload = { method: 'POST' };
      const signature = hmac(payload);
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [makeDbRow({ payload, hmac_signature: signature })] });
      poolMocks.pool.query.mockResolvedValueOnce({ rows: [{ payload, hmac_signature: signature }] });
