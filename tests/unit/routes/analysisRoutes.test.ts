...
vi.mock('../../../packages/backend/src/infrastructure/dataFacade.js', () => ({ fetchHistoryData: dataServiceMocks.fetchHistoryData }));
vi.mock('../../../packages/backend/src/utils/engineClient.js', () => ({ callEngineStrict: engineMocks.callEngineStrict, EngineUnavailableError: EngineUnavailableErrorStub }));
vi.mock('../../../packages/backend/src/utils/logger.js', () => ({ logger: mockLogger(loggerMocks), sanitizeLog: (s: string) => s }));
vi.mock('../../../packages/backend/src/utils/metrics.js', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../../packages/backend/src/utils/metrics.js')>();
...
vi.mock('../../../packages/backend/src/middleware/rbac.js', () => ({
  ...
  Permission: { BACKTEST_RUN: 'backtest:run', STRATEGY_MANAGE: 'strategy:manage', SIGNAL_READ: 'signal:read' },
}));
vi.mock('../../../packages/backend/src/middleware/quota.js', () => ({ enforceQuota: () => (_req: unknown, _res: unknown, next: () => void) => next() }));

import analysisRoutes from '../../../packages/backend/src/routes/analysisRoutes.js';

const mockPcaResult = { eigenvalues: [2.5, 0.3, 0.2], eigenvectors: [[0.5, 0.5, 0.5]], explainedVarianceRatio: [0.83, 0.1, 0.07], principalComponents: [[1, 2, 3]] };
const mockLetfResult = { slippageCurve: [{ date: '2020-01-01', slippage: 0.01 }], annualDecay: 0.05 };
const mockOptimizeResult = { successProbability: 0.85, probabilityCurve: [{ year: 1, probability: 0.95 }], optimalPath: [], requiredContribution: 20000 };
...
async function startServer() { return startExpressApp((app) => app.use('/api/v1', analysisRoutes)); }
...
describe('analysisRoutes - PCA: POST /api/v1/pca/analyze', () => {
  ...
  const validBody = { tickers: ['SPY', 'QQQ', 'IWM'], startDate: '2020-01-01', endDate: '2024-01-01' };
  const pcaData = { data: { SPY: { '2020-01-01': 300.0, '2020-01-02': 301.0 }, QQQ: { '2020-01-01': 200.0, '2020-01-02': 201.0 }, IWM: { '2020-01-01': 150.0, '2020-01-02': 151.0 } }, degraded: false };
  beforeEach(async () => { server = await setupServer(mockPcaResult, pcaData); });
  ...
  it('应将 ticker 转大写并去重后调用 fetchHistoryData', async () => {
    await apiPost(`${server.url}/api/v1/pca/analyze`, { tickers: ['spy', 'SPY', 'QQQ'], startDate: '2020-01-01', endDate: '2024-01-01' });
  ...
  it('重复 ticker 去重后不足 2 个应返回 422', async () => {
    const { res, body } = await apiPost(`${server.url}/api/v1/pca/analyze`, { tickers: ['SPY', 'spy'], startDate: '2020-01-01', endDate: '2024-01-01' });
    expect(res.status).toBe(422);
    expect(body.error.code).toBe('VALIDATION_ERROR');
  });
  it('部分标的价格数据缺失时应返回 404', async () => {
    dataServiceMocks.fetchHistoryData.mockResolvedValue({ data: { SPY: { '2020-01-01': 300.0 }, QQQ: {}, IWM: { '2020-01-01': 150.0 } }, degraded: false });
    const { res, body } = await apiPost(`${server.url}/api/v1/pca/analyze`, validBody);
...
describe('analysisRoutes - LETF: POST /api/v1/letf/analyze', () => {
  let server: TestServer;
  const validBody = { letfTicker: 'TQQQ', benchmarkTicker: 'QQQ', leverage: 3, startDate: '2020-01-01', endDate: '2024-01-01' };
  const letfData = { data: { TQQQ: { '2020-01-01': 30.0, '2020-01-02': 31.0 }, QQQ: { '2020-01-01': 200.0, '2020-01-02': 201.0 } }, degraded: false };
  beforeEach(async () => { server = await setupServer(mockLetfResult, letfData); });
  afterEach(async () => { await server.close(); });
  ...
  it('应将 ticker 转为大写并调用 fetchHistoryData', async () => {
    await apiPost(`${server.url}/api/v1/letf/analyze`, { letfTicker: 'tqqq', benchmarkTicker: 'qqq', leverage: 3, startDate: '2020-01-01', endDate: '2024-01-01' });
  ...
  it.each([
    ['缺少 letfTicker', { benchmarkTicker: 'QQQ', leverage: 3, startDate: '2020-01-01', endDate: '2024-01-01' }],
    ['leverage 为负数', { letfTicker: 'TQQQ', benchmarkTicker: 'QQQ', leverage: -1, startDate: '2020-01-01', endDate: '2024-01-01' }],
    ['缺少 startDate', { letfTicker: 'TQQQ', benchmarkTicker: 'QQQ', leverage: 3, endDate: '2024-01-01' }],
  ])('%s 应返回 400（zod 校验失败）', async (_n, body) => {
...
describe('analysisRoutes - GoalOptimizer: POST /api/v1/goal-optimizer/optimize', () => {
  ...
  function createValidRequest() {
    return { targetAmount: 1000000, initialAmount: 100000, years: 20, assets: [{ ticker: 'SPY', weight: 100 }], numSimulations: 1000 };
  }
  beforeEach(async () => { server = await setupServer(mockOptimizeResult, { data: { SPY: { '2020-01-01': 300.0, '2020-01-02': 301.0 } }, degraded: false }); });
  afterEach(async () => { await server.close(); });
  it('有效参数应返回优化结果', async () => {
    const { res, body } = await apiPost(`${server.url}/api/v1/goal-optimizer/optimize`, createValidRequest());
    expect(res.status).toBe(200);
    expect(body.success).toBe(true);
  ...
  it.each([
    ['缺少 targetAmount', () => { const r = createValidRequest(); delete (r as Record<string, unknown>).targetAmount; return r; }],
    ['targetAmount 为负数', () => { const r = createValidRequest(); r.targetAmount = -100; return r; }],
    ['空 assets 数组', () => { const r = createValidRequest(); r.assets = []; return r; }],
  ...
  it('引擎抛错时应返回 500', async () => {
    ...
    const { res } = await apiPost(`${server.url}/api/v1/goal-optimizer/optimize`, createValidRequest());
    expect(res.status).toBe(500);
  });
});
...
describe('analysisRoutes - FactorRegression: POST /api/v1/analysis/factor-regression', () => {
  let server: TestServer;
  const validBody = { monthlyReturns: [0.01, -0.02, 0.015], ffData: [{ mktRF: 0.02, smb: 0.005, hml: -0.01 }], factors: ['mktRF', 'smb'], startDate: '2020-01', endDate: '2020-12' };
  const minimalBody = { monthlyReturns: [0.01], ffData: [{ mktRF: 0.02 }] };
  beforeEach(async () => { server = await setupServer({ alpha: 0.01, beta: 1.05 }); });
  afterEach(async () => { await server.close(); });
  it('完整参数应返回 200 + 引擎结果，factors/startDate/endDate 透传', async () => {
    const { res, body } = await apiPost(`${server.url}/api/v1/analysis/factor-regression`, validBody);
    expect(res.status).toBe(200);
    expect(body.success).toBe(true);
    expect(body.data).toEqual({ alpha: 0.01, beta: 1.05 });
    expect(engineMocks.callEngineStrict).toHaveBeenCalledWith('/api/engine/factor-regression', validBody);
  });
  it('省略 factors/startDate/endDate 时使用默认值', async () => {
    ...
    expect(engineMocks.callEngineStrict).toHaveBeenCalledWith('/api/engine/factor-regression', {
      monthlyReturns: [0.01], ffData: [{ mktRF: 0.02 }], factors: ['mktRF', 'smb', 'hml'], startDate: '', endDate: '',
    });
  });
  it.each([
    ...
    const { res, body: resBody } = await apiPost(`${server.url}/api/v1/analysis/factor-regression`, body);
    expect(res.status).toBe(400);
    expect(resBody.error.code).toBe('VALIDATION_ERROR');
    expect(engineMocks.callEngineStrict).not.toHaveBeenCalled();
  });
  it('引擎抛 EngineUnavailableError 应返回 503 + Retry-After', async () => {
    engineMocks.callEngineStrict.mockRejectedValueOnce(new EngineUnavailableErrorStub('/api/engine/factor-regression'));
  ...
  it('引擎抛普通 Error 应返回 500 FR_ERROR', async () => {
    ...
    const { res, body } = await apiPost(`${server.url}/api/v1/analysis/factor-regression`, minimalBody);
    expect(res.status).toBe(500);
...
describe('analysisRoutes - Calculator: POST /api/v1/calculators/:type', () => {
  let server: TestServer;
  beforeEach(async () => { server = await setupServer({ result: 'ok' }); });
  afterEach(async () => { await server.close(); });
  it('cagr 类型应返回 200 + 引擎结果', async () => {
    const { res, body } = await apiPost(`${server.url}/api/v1/calculators/cagr`, { initialAmount: 10000, years: 10, rate: 0.07 });
    expect(res.status).toBe(200);
    expect(body.success).toBe(true);
    ...
    expect(payload).toMatchObject({ type: 'cagr', initialAmount: 10000, years: 10, rate: 0.07 });
  });
  it('swr 与 frontier 类型同样被接受', async () => {
    for (const type of ['swr', 'frontier']) {
      engineMocks.callEngineStrict.mockClear();
      const { res } = await apiPost(`${server.url}/api/v1/calculators/${type}`, {});
      expect(res.status).toBe(200);
      expect(engineMocks.callEngineStrict).toHaveBeenCalledWith('/api/engine/calculators', { type });
    }
  });
  ...
  it('引擎抛 EngineUnavailableError 应返回 503 + Retry-After（ADR-031 fail-closed）', async () => {
    engineMocks.callEngineStrict.mockRejectedValueOnce(new EngineUnavailableErrorStub('/api/engine/calculators'));
  ...
  it('body 为空对象时仍以 type 透传到引擎', async () => {
    await apiPost(`${server.url}/api/v1/calculators/swr`, {});
    expect(engineMocks.callEngineStrict).toHaveBeenCalledWith('/api/engine/calculators', { type: 'swr' });
  });
});
