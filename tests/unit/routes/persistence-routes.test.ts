...
const mocks = vi.hoisted(() => ({
  portfolioRepo: { listPortfolios: vi.fn(), getPortfolio: vi.fn(), createPortfolio: vi.fn(), updatePortfolio: vi.fn(), deletePortfolio: vi.fn() },
  savedConfigRepo: { listConfigs: vi.fn(), getConfig: vi.fn(), createConfig: vi.fn(), updateConfig: vi.fn(), deleteConfig: vi.fn() },
...
vi.mock('../../../packages/backend/src/repositories/savedConfigRepo.js', () => mocks.savedConfigRepo);
vi.mock('../../../packages/backend/src/repositories/backtestRunRepo.js', () => mocks.backtestRunRepo);
...
async function startApp(sub = 'user-1'): Promise<TestServer> {
  return startExpressApp((app) => {
    app.use((req: TestRequest, _res, next) => { req.tenantId = ORG; req.user = { sub, role: 'analyst', tenant_id: ORG, org_role: 'analyst' }; next(); });
    app.use('/api/v1/portfolios', portfolioRoutes); app.use('/api/v1/configs', configRoutes); app.use('/api/v1/runs', runRoutes);
  });
}
...
describe('portfolioRoutes', () => {
  ...
  beforeEach(async () => { vi.clearAllMocks(); server = await startApp(); });
  ...
  it('POST / 创建成功返回 201 并以 user sub 为 owner', async () => {
    ...
    const { res } = await reqJson(base(), 'POST', { name: '60/40', assets: [{ ticker: 'SPY', weight: 100 }] });
    expect(res.status).toBe(201);
    expect(mocks.portfolioRepo.createPortfolio).toHaveBeenCalledWith(ORG, 'user-1', expect.objectContaining({ name: '60/40' }));
  });
  it('POST / 权重不合法（空 assets）返回 400', async () => {
    const { res } = await reqJson(base(), 'POST', { name: 'x', assets: [] });
    expect(res.status).toBe(400); expect(mocks.portfolioRepo.createPortfolio).not.toHaveBeenCalled();
  });
  it('GET / 返回列表', async () => {
    mocks.portfolioRepo.listPortfolios.mockResolvedValueOnce([{ id: ID, name: '60/40' }]);
    const { res, body } = await reqJson(base(), 'GET');
    expect(res.status).toBe(200); expect(body.data).toHaveLength(1);
  });
  it('GET /:id 成功返回 200', async () => {
    ...
    expect(res.status).toBe(200); expect(body.data.name).toBe('60/40');
  });
  ...
  it.each([['GET', 'getPortfolio'], ['PUT', 'updatePortfolio'], ['DELETE', 'deletePortfolio']])('/:id 非法 UUID 返回 400（%s）', async (method, repoFn) => {
    const { res } = await reqJson(`${base()}/bad`, method, method === 'GET' ? undefined : { name: 'x', assets: [{ ticker: 'SPY', weight: 100 }] });
    expect(res.status).toBe(400); expect(mocks.portfolioRepo[repoFn as keyof typeof mocks.portfolioRepo]).not.toHaveBeenCalled();
  });
  it('PUT /:id 更新成功返回 200', async () => {
    mocks.portfolioRepo.updatePortfolio.mockResolvedValueOnce({ id: ID, name: '80/20' });
    const { res } = await reqJson(`${base()}/${ID}`, 'PUT', { name: '80/20', assets: [{ ticker: 'SPY', weight: 80 }, { ticker: 'BND', weight: 20 }] });
    expect(res.status).toBe(200);
  });
  it('PUT /:id 不存在返回 404', async () => {
    mocks.portfolioRepo.updatePortfolio.mockResolvedValueOnce(null);
    const { res } = await reqJson(`${base()}/${ID}`, 'PUT', { name: 'x', assets: [{ ticker: 'SPY', weight: 100 }] });
    expect(res.status).toBe(404);
  });
...
describe('configRoutes', () => {
  ...
  it('apikey 调用方 owner 应为 null', async () => {
    await server.close(); server = await startApp('apikey:key-123');
    mocks.savedConfigRepo.createConfig.mockResolvedValueOnce({ id: ID, name: 'cfg' });
  ...
  it.each([['GET', 'getConfig'], ['PUT', 'updateConfig'], ['DELETE', 'deleteConfig']])('/:id 非法 UUID 返回 400（%s）', async (method, repoFn) => {
    const { res } = await reqJson(`${base()}/bad`, method, method === 'GET' ? undefined : { name: 'x', config: { a: 1 } });
    expect(res.status).toBe(400); expect(mocks.savedConfigRepo[repoFn as keyof typeof mocks.savedConfigRepo]).not.toHaveBeenCalled();
  });
  it('PUT /:id 更新成功返回 200', async () => {
  ...
  it('POST / 缺失 body 返回 400', async () => {
    ...
    expect(res.status).toBe(400); expect(mocks.savedConfigRepo.createConfig).not.toHaveBeenCalled();
  });
});
...
describe('runRoutes', () => {
  ...
  it.each([['GET', 'getRun'], ['DELETE', 'deleteRun']])('/:id 非法 UUID 返回 400（%s）', async (method, repoFn) => {
    const { res } = await reqJson(`${base()}/bad`, method);
    expect(res.status).toBe(400); expect(mocks.backtestRunRepo[repoFn as keyof typeof mocks.backtestRunRepo]).not.toHaveBeenCalled();
  });
  it('DELETE /:id 成功返回 200', async () => {
  ...
  it('POST / 缺失 body 返回 400', async () => {
    const { res } = await reqJson(base(), 'POST', {});
    expect(res.status).toBe(400); expect(mocks.backtestRunRepo.createRun).not.toHaveBeenCalled();
  });
  it('apikey 调用方 owner 应为 null', async () => {
    await server.close(); server = await startApp('apikey:key-456');
    mocks.backtestRunRepo.createRun.mockResolvedValueOnce({ id: ID });
