...
describe('writeEventInTransaction', () => {
  ...
  it('应使用正确的 INSERT SQL 与参数', async () => {
    ...
    // ADR-024 / T-11：INSERT 增加 event_id 去重列（未提供时为 null）+ ON CONFLICT DO NOTHING。
    expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO outbox'), [
      'BacktestSession',
