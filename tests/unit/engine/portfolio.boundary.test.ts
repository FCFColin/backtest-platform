...
describe('Portfolio引擎 - 价格数据缺失', () => {
  it('部分日期无价格数据，跳过不影响计算', () => {
    ...
    // 最终净值 = 10000 * (109/100) = 10900（价格从100涨到109）
    expect(gc.at(-1)!.value).toBeCloseTo(10900, 0);
  });
  ...
  it('ticker不在priceData中', () => {
    ...
    const result = runPortfolioBacktest([portfolio], priceData, makeParams());
    const gc = result.portfolios[0].growthCurve;
    // MISSING无价格数据，其持仓始终为初始值5000（无价格变化）
    // A价格不变，持仓也始终为5000，总值始终为10000
    expect(gc.at(-1)!.value).toBeCloseTo(10000, 0);
...
describe('Portfolio引擎 - 再平衡边界', () => {
  ...
  it('偏离调仓阈值100%：永远不调仓', () => {
    ...
    expect(finalTh100).toBeCloseTo(finalNone, 6);
  });
});
