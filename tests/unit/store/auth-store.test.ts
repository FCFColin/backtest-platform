...
const mocks = vi.hoisted(() => ({ mockApiFetch: vi.fn(), mockSetTokens: vi.fn(), mockClearTokens: vi.fn(), mockRefreshTokens: vi.fn(), mockFetch: vi.fn() }));
vi.mock('../../../packages/frontend/src/utils/apiClient.js', () => ({ apiFetch: mocks.mockApiFetch }));
vi.mock('../../../packages/frontend/src/utils/authTokens.js', () => ({ setTokens: mocks.mockSetTokens, clearTokens: mocks.mockClearTokens, refreshTokens: mocks.mockRefreshTokens }));
globalThis.fetch = mocks.mockFetch;

import { useAuthStore } from '../../../packages/frontend/src/store/authStore.js';

const mockUser = (overrides: Record<string, unknown> = {}) => ({ userId: 'u1', role: 'admin', tenantId: 't1', orgRole: 'owner', platformAdmin: false, ...overrides });
const mockOrg = (overrides: Record<string, unknown> = {}) => ({ orgId: 'o1', name: 'Org', slug: 'org', plan: 'free', status: 'active', role: 'owner', ...overrides });
const mockFetchRes = (ok: boolean, data: unknown, detail?: string) => ({ ok, json: async () => (ok ? { success: true, data } : { success: false, detail: detail ?? 'error' }) });
function resetState() { useAuthStore.setState({ user: null, org: null, orgs: [], initialized: false, loading: false, error: null }); }

describe('authStore', () => {
  beforeEach(() => { resetState(); vi.clearAllMocks(); });

  describe('isAuthenticated', () => {
    it('无 user 时应返回 false', () => { expect(useAuthStore.getState().isAuthenticated()).toBe(false); });
    it('有 user 时应返回 true', () => { useAuthStore.setState({ user: mockUser() }); expect(useAuthStore.getState().isAuthenticated()).toBe(true); });
  });
  ...
  describe('init', () => {
    it('refreshTokens 返回 false 时直接标记 initialized 且不调用 fetchMe', async () => {
      ...
      expect(useAuthStore.getState().initialized).toBe(true); expect(mocks.mockApiFetch).not.toHaveBeenCalled();
    });
    it('refreshTokens 返回 true 时应尝试 fetchMe', async () => {
      ...
      await useAuthStore.getState().init();
      expect(useAuthStore.getState().initialized).toBe(true); expect(useAuthStore.getState().user?.userId).toBe('u1');
    });
    it('已初始化时应直接返回', async () => {
    ...
    it('refreshTokens 失败时仍应标记 initialized', async () => {
      mocks.mockRefreshTokens.mockResolvedValue(false);
      await useAuthStore.getState().init();
      expect(useAuthStore.getState().initialized).toBe(true); expect(useAuthStore.getState().user).toBeNull();
    });
  });
  ...
  describe('loginPassword', () => {
    it('成功登录应 setTokens 并设置 user', async () => {
      ...
      expect(ok).toBe(true); expect(mocks.mockSetTokens).toHaveBeenCalledWith('at');
      expect(useAuthStore.getState().user?.userId).toBe('u1');
      expect(useAuthStore.getState().loading).toBe(false); expect(useAuthStore.getState().error).toBeNull();
    });
    it('登录失败应设置 error 并返回 false', async () => {
      ...
      expect(ok).toBe(false); expect(useAuthStore.getState().error).toBe('用户名或密码错误');
    });
    ...
    it('catch 异常时应返回 false', async () => {
      ...
      const ok = await useAuthStore.getState().loginPassword('u', 'p');
      expect(ok).toBe(false); expect(useAuthStore.getState().error).toContain('网络错误');
    });
  });
  ...
  describe('register', () => {
    ...
    it('注册失败应返回 false', async () => {
      ...
      expect(ok).toBe(false); expect(useAuthStore.getState().error).toBe('用户名已存在');
    });
    it('catch 异常时应返回 false', async () => {
      mocks.mockFetch.mockRejectedValue(new Error('网络错误'));
  ...
  describe('logout', () => {
    it('应调用服务端撤销并清空本地状态', async () => {
      ...
      expect(mocks.mockClearTokens).toHaveBeenCalled(); expect(useAuthStore.getState().user).toBeNull();
    });
    it('logout 应始终调用服务端并清除状态', async () => {
      mocks.mockFetch.mockResolvedValue({ ok: true });
      await useAuthStore.getState().logout();
      expect(mocks.mockFetch).toHaveBeenCalled(); expect(mocks.mockClearTokens).toHaveBeenCalled();
  ...
  describe('loadOrgs', () => {
    it('应加载组织列表并设置活跃组织', async () => {
      const orgs = [{ orgId: 'o1', name: 'A', slug: 'a', plan: 'free', status: 'active', role: 'owner' }, { orgId: 'o2', name: 'B', slug: 'b', plan: 'pro', status: 'active', role: 'member' }];
      mocks.mockApiFetch.mockResolvedValue(mockFetchRes(true, { orgs, activeOrgId: 'o1' }));
      await useAuthStore.getState().loadOrgs();
      expect(useAuthStore.getState().orgs).toHaveLength(2); expect(useAuthStore.getState().org?.orgId).toBe('o1');
    });
  ...
  describe('switchOrg', () => {
    it('切换成功后应更新 token 和 user', async () => {
      mocks.mockApiFetch.mockResolvedValueOnce(mockFetchRes(true, { accessToken: 'new-at', org: mockOrg({ orgId: 'o2', name: 'B', slug: 'b', plan: 'pro', role: 'member' }) }));
      mocks.mockApiFetch.mockResolvedValueOnce(mockFetchRes(true, mockUser({ role: 'member', tenantId: 'o2', orgRole: 'member' })));
      const ok = await useAuthStore.getState().switchOrg('o2');
      expect(ok).toBe(true); expect(mocks.mockSetTokens).toHaveBeenCalledWith('new-at');
    });
  ...
  describe('acceptInvite', () => {
    it('接受邀请成功后应加载组织列表', async () => {
      ...
      expect(result.ok).toBe(true); expect(result.orgId).toBe('o3');
    });
