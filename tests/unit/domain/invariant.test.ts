...
import type { Holding } from '../../../packages/backend/src/domain/aggregates/portfolio.js';
...
describe('validatePortfolioInvariants', () => {
  const mockHolding = () => ({ ticker: {} as Holding['ticker'], weight: Weight.create(0) });

  it('权重总和 ~100 时应该通过', () => {
    const h = mockHolding();
    const holdings = [
      { ...h, weight: Weight.create(60) },
      { ...h, weight: Weight.create(40) },
    ];
  ...
  it('负权重时应该抛出', () => {
    const holdings = [{ ticker: {} as Holding['ticker'], weight: { value: -10 } }];
    expect(() => validatePortfolioInvariants({ holdings })).toThrow('Invariant violation');
  });
});
