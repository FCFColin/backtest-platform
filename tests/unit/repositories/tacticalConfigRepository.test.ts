...
const dbMocks = vi.hoisted(() => {
  ...
  return { query: client.query, withTenant: vi.fn(), withTenantReadOnly: vi.fn(), client };
});

vi.mock('../../../packages/backend/src/db/pool.js', () =>
  createPoolModuleMock(dbMocks, { client: dbMocks.client }),
);
...
describe('findByTenant', () => {
  it('应使用 withTenantReadOnly 并返回映射后的配置列表', async () => {
    dbMocks.query.mockResolvedValue({ rows: [configRow] });
  ...
  it('limit 应被截断到 200', async () => {
    ...
    expect(dbMocks.query.mock.calls[0][1][1]).toBe(200);
  });

  it('offset 负数应被归零', async () => {
    dbMocks.query.mockResolvedValue({ rows: [] });
    await findByTenant(ORG, 10, -5);
    expect(dbMocks.query.mock.calls[0][1][2]).toBe(0);
  });
});
...
describe('create', () => {
  ...
  it('description 未提供时应传 null', async () => {
    dbMocks.query.mockResolvedValue({ rows: [{ ...configRow, description: null }] });
    await create(ORG, USER_ID, { name: 'x', config: {} });
    expect(dbMocks.query.mock.calls[0][1][3]).toBeNull();
  });
});

describe('update', () => {
  it('应动态构建 SET 子句并返回更新记录', async () => {
    dbMocks.query.mockResolvedValue({ rows: [{ ...configRow, name: 'Updated' }] });
  ...
  it('仅更新 description 时应正确构建参数', async () => {
    dbMocks.query.mockResolvedValue({ rows: [{ ...configRow, description: 'new' }] });
...
describe('remove', () => {
  it.each([
    ...
    dbMocks.query.mockResolvedValue({ rowCount: count });
...
describe('count', () => {
  it('应返回租户配置数量', async () => {
    dbMocks.query.mockResolvedValue({ rows: [{ count: 42 }] });
