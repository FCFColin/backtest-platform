...
import { savePortfolios, loadPortfolios, saveParameters, loadParameters, saveNamedConfig, loadNamedConfigs, deleteNamedConfig, clearAllData } from '../../../packages/frontend/src/utils/portfolioStorage.js';
import type { Portfolio, BacktestParameters } from '@backtest/shared';

const validPortfolios: Portfolio[] = [{ id: 'p1', name: 'Test Portfolio', assets: [{ ticker: 'VTI', weight: 60 }, { ticker: 'BND', weight: 40 }], rebalanceFrequency: 'quarterly' }];
const validParams: BacktestParameters = { startDate: '2010-01-01', endDate: '2024-12-31', startingValue: 10000, adjustForInflation: false, rollingWindowMonths: 12, benchmarkTicker: 'SPY' };
...
const unavailableStorage = (mode: 'throwGet' | 'throwSet' | 'throwRemove') => {
  const throwFn = () => { throw new Error('unavailable'); };
  const noop = vi.fn();
  return {
...
describe('savePortfolios / loadPortfolios', () => {
  it('保存后能正确加载', () => { savePortfolios(validPortfolios); expect(loadPortfolios()).toEqual(validPortfolios); });
  it('未保存时加载返回 null', () => { expect(loadPortfolios()).toBeNull(); });
  it('保存空数组也能正确加载', () => { savePortfolios([]); expect(loadPortfolios()).toEqual([]); });
  it('保存多个组合', () => {
    ...
    savePortfolios(portfolios); expect(loadPortfolios()).toEqual(portfolios);
  });
  it('覆盖保存：第二次保存替换第一次', () => {
    savePortfolios(validPortfolios);
    const np: Portfolio[] = [{ id: 'p2', name: 'New', assets: [{ ticker: 'SPY', weight: 100 }], rebalanceFrequency: 'none' }];
    savePortfolios(np); expect(loadPortfolios()).toEqual(np); expect(loadPortfolios()?.length).toBe(1);
  });
});

describe('saveParameters / loadParameters', () => {
  it('保存后能正确加载', () => { saveParameters(validParams); expect(loadParameters()).toEqual(validParams); });
  it('未保存时加载返回 null', () => { expect(loadParameters()).toBeNull(); });
  it('覆盖保存', () => {
    saveParameters(validParams);
    const np: BacktestParameters = { ...validParams, startingValue: 50000 };
    saveParameters(np); expect(loadParameters()).toEqual(np);
  });
});
...
describe('saveNamedConfig / loadNamedConfigs', () => {
  it('保存命名方案后能加载', () => {
    ...
    expect(configs.length).toBe(1); expect(configs[0].name).toBe('My Config');
    expect(configs[0].portfolios).toEqual(validPortfolios); expect(configs[0].parameters).toEqual(validParams);
    expect(configs[0].id).toBeTruthy(); expect(configs[0].savedAt).toBeTruthy();
  });
  it('未保存时加载返回空数组', () => { expect(loadNamedConfigs()).toEqual([]); });
  it('保存多个命名方案', () => {
    saveNamedConfig('Config 1', validPortfolios, validParams); vi.advanceTimersByTime(10);
    saveNamedConfig('Config 2', validPortfolios, validParams); vi.advanceTimersByTime(10);
  ...
  it.each([['空名称', ''], ['超长名称（1000+ 字符）', 'A'.repeat(1000)], ['XSS payload', '<script>alert(1)</script>']])('%s 也能保存', async (_n, name) => {
    saveNamedConfig(name, validPortfolios, validParams);
    const configs = loadNamedConfigs();
    expect(configs.length).toBe(1); expect(configs[0].name).toBe(name);
  });
});
...
describe('deleteNamedConfig', () => {
  ...
  it('空列表中删除不抛错', () => { expect(() => deleteNamedConfig('any-id')).not.toThrow(); });
});

describe('clearAllData', () => {
  it('清除所有存储数据', () => {
    savePortfolios(validPortfolios); saveParameters(validParams); saveNamedConfig('Test', validPortfolios, validParams);
    clearAllData();
    expect(loadPortfolios()).toBeNull(); expect(loadParameters()).toBeNull(); expect(loadNamedConfigs()).toEqual([]);
  });
  it('无数据时清除不抛错', () => { expect(() => clearAllData()).not.toThrow(); });
});
