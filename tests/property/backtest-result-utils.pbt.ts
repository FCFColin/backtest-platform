...
describe('PBT: compressBacktestResult 降采样不变量', () => {
      ...
      fc.property(
        ...
        (n, max) => {
          const result = compressBacktestResult(
            { portfolios: [makePortfolio(n)], correlations: [] },
            max,
          );
  ...
  it('压缩后首尾数据点与原始一致', () => {
    fc.assert(
      fc.property(fc.integer({ min: 101, max: 2000 }), (n) => {
        const original = makePortfolio(n);
        const result = compressBacktestResult({ portfolios: [original], correlations: [] }, 100);
  ...
  it('压缩后所有曲线长度一致', () => {
    fc.assert(
      fc.property(fc.integer({ min: 101, max: 1000 }), (n) => {
        const result = compressBacktestResult(
          { portfolios: [makePortfolio(n)], correlations: [] },
          100,
        );
  ...
  it('benchmarkGrowth 也应被压缩', () => {
    fc.assert(
      fc.property(fc.integer({ min: 101, max: 1000 }), (n) => {
        ...
        const result = compressBacktestResult(
          { portfolios: [makePortfolio(10)], benchmarkGrowth: bench, correlations: [] },
          50,
        );
  ...
  it('compressBacktestResultForSync 应省略 sync 字段且不超过 MAX_SYNC_CHART_POINTS', () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 1000 }), (n) => {
        const result = compressBacktestResultForSync({
          portfolios: [makePortfolio(n)],
          correlations: [],
        });
        const p = result.portfolios[0];
...
describe('PBT: getCacheKey 参数顺序无关性', () => {
  ...
  it('不同 type 应产生不同 key', () => {
    fc.assert(
      fc.property(fc.string({ minLength: 1 }), (ticker) => {
