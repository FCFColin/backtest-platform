...
function createRedisMocks(
  ...
  const {
    ...
    store = withStore ? new Map<string, string>() : undefined,
    sets = withSets ? new Map<string, Set<string>>() : undefined,
    handlers = withHandlers
      ? ({} as Record<string, Array<(...args: unknown[]) => void>>)
      : undefined,
    makeFn = (): ReturnType<typeof vi.fn> =>
...
export function createMockPool(): { query: ReturnType<typeof vi.fn> } {
  return { query: vi.fn().mockResolvedValue({ rows: [], rowCount: 0 }) } as unknown as {
