/**
 * 集中配置模块（公共入口）。
 * 所有消费方应从 `./config/index.js` 导入，以保持扁平的 `config.X` 访问路径。
 */
...
// logger 延后导入：此时 limits.ts → unleashClient.ts → logger.ts 已完成加载，
// 避免循环依赖 config/index.ts → logger.ts → config/index.ts（P1 合并 validation.ts 引入）。
