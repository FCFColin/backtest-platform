...
import { recordCacheHit } from './metrics.js';
...
// ─── Redis 可用性检测（遵循 dataCacheService 价格缓存模式） ───

let backtestCacheRedisAvailable: boolean | null = null;

async function isBacktestCacheRedisAvailable(): Promise<boolean> {
  if (backtestCacheRedisAvailable !== null) return backtestCacheRedisAvailable;
  try {
    await appRedis.ping();
    backtestCacheRedisAvailable = true;
    return true;
  } catch {
    backtestCacheRedisAvailable = false;
    logger.warn('[backtestCache] Redis 不可用，回测结果缓存降级到内存模式');
    return false;
  }
}
...
export async function setBacktestResultCache(key: string, result: BacktestResult): Promise<void> {
  ...
  const redisOk = await isBacktestCacheRedisAvailable();
  if (redisOk) {
    try {
      await appRedis.set(
...
/**
 * 获取缓存结果，未命中时执行 compute 并缓存。
 *
 * 相同 key 的并发请求共享同一个 Promise（singleflight），避免惊群效应：
 * 多个并发请求携带相同 portfolio+parameters 触发多次引擎调用，
 * singleflight 确保仅首个请求执行计算，其余共享结果。
 *
 * @param key - 缓存键
 * @param compute - 未命中时的计算函数（如调用引擎）
 * @returns 缓存命中结果或新计算结果
 */
export async function getOrComputeBacktestResult(
  key: string,
...
export function clearBacktestResultCache(): void {
  cache.clear();
  inFlight.clear();
  void isBacktestCacheRedisAvailable().then((ok) => {
    if (!ok) return;
    void (async () => {
      let cursor = '0';
      do {
