...
export const VALID_WEBHOOK_EVENTS: ReadonlySet<string> = new Set(Object.values(WEBHOOK_EVENT_TYPES));
...
export async function createWebhook(
  ...
  params: { orgId: string; url: string; secret: string; description?: string; subscribedEvents: string[] },
): Promise<{ id: string; url: string; description: string | null; isActive: boolean; subscribedEvents: string[]; createdAt: Date }> {
...
/** 向端点投递 webhook（POST + HMAC 签名头）。SSRF 失败标记 permanentFailure 不重试；10s 超时；响应体限制 1MB 防 OOM（C-003）。 */
export async function deliverWebhook(
  ...
  try {
    ...
    return { success: false, responseCode: null, responseBody: `SSRF blocked: ${(err as Error).message}`.slice(0, RESPONSE_BODY_TRUNCATE), permanentFailure: true };
  }
  ...
  try {
    ...
    return { success: res.ok, responseCode: res.status, responseBody: responseText.slice(0, RESPONSE_BODY_TRUNCATE) };
  } catch (err) {
    return { success: false, responseCode: null, responseBody: (err as Error).message.slice(0, RESPONSE_BODY_TRUNCATE) };
  } finally {
    clearTimeout(timer);
  }
}
  ...
  try {
    while (true) {
      ...
      if (received > maxBytes) {
        chunks.push(decoder.decode(value.subarray(0, value.byteLength - (received - maxBytes)), { stream: true }));
        break;
      }
...
export async function triggerWebhooks(orgId: string, eventType: string, payload: Record<string, unknown>): Promise<void> {
  const pool = getPool();
  const { rows: probe } = await pool.query(
    `SELECT 1 FROM webhook_endpoints
      WHERE org_id = $1 AND is_active = TRUE AND $2 = ANY(subscribed_events) LIMIT 1`,
    [orgId, eventType],
  );
  if (probe.length === 0) {
  ...
  logger.info({ orgId, eventType, created: rowCount ?? 0 }, '[webhookService] 已创建 webhook 投递记录');
}
...
export async function processPendingDeliveries(): Promise<void> {
  ...
  logger.info({ module: 'webhookService', pending: rows.length }, '[webhookService] 开始处理待投递记录');
...
async function processSingleDelivery(row: DeliveryRow): Promise<void> {
  const { delivery_id: deliveryId, endpoint_id: endpointId, event_type: eventType, attempt_count: attemptCount, url, org_id: orgId } = row;
  ...
  const toBuf = (v: Buffer | string) => (Buffer.isBuffer(v) ? v : Buffer.from(v));
  ...
  await withTenant(orgId, async (client) => {
    if (result.success) {
      ...
      await client.query(`UPDATE webhook_endpoints SET failed_consecutive_count = 0 WHERE id = $1`, [endpointId]);
      logger.info({ deliveryId, endpointId, eventType, responseCode: result.responseCode }, '[webhookService] webhook 投递成功');
      return;
    }
        ...
        await client.query(
          `UPDATE webhook_endpoints
              SET failed_consecutive_count = failed_consecutive_count + 1,
                  is_active = CASE WHEN failed_consecutive_count + 1 >= $2 THEN FALSE ELSE is_active END,
                  disabled_at = CASE WHEN failed_consecutive_count + 1 >= $2 THEN NOW() ELSE disabled_at END
            WHERE id = $1`,
          [endpointId, AUTO_DISABLE_THRESHOLD],
        );
        logger.warn({ deliveryId, endpointId, orgId, eventType, attempts: newAttemptCount }, '[webhookService] webhook 投递达到上限，标记失败');
      } else {
        logger.warn({ deliveryId, endpointId, orgId, eventType, reason: 'permanent_failure' }, '[webhookService] webhook 投递永久失败（SSRF），标记 failed 不重试');
      }
      return;
    }
    ...
    logger.info({ deliveryId, endpointId, eventType, attempts: newAttemptCount, nextRetryAt }, '[webhookService] webhook 投递失败，安排重试');
  });
}
...
export async function cleanupOldDeliveries(): Promise<void> {
  ...
  const { rowCount } = await pool.query(
    `DELETE FROM webhook_deliveries WHERE id IN (
        SELECT id FROM (
          SELECT id, ROW_NUMBER() OVER (PARTITION BY endpoint_id ORDER BY created_at DESC) AS rn FROM webhook_deliveries
        ) t WHERE rn > $1
      )`,
    [MAX_DELIVERIES_PER_ENDPOINT],
  );
  if (rowCount && rowCount > 0) {
    logger.info({ module: 'webhookService', deleted: rowCount, keep: MAX_DELIVERIES_PER_ENDPOINT }, '[webhookService] 已清理超额投递记录');
  }
}

export const WEBHOOK_CONSTANTS = { MAX_ATTEMPTS, RETRY_DELAYS_MS, MAX_DELIVERIES_PER_ENDPOINT, AUTO_DISABLE_THRESHOLD, RESPONSE_BODY_TRUNCATE };