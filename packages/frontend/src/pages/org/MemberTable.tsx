...
export function MemberTable({ members, isAdmin, busy, onChangeRole, onRemoveMember }: MemberTableProps) {
        ...
        <tbody>
          {members.map((m) => (
            <tr key={m.userId}>
              ...
              <td style={TABLE_TD}>{isAdmin && m.role !== 'owner' ? <RoleSelect value={m.role} disabled={busy} onChange={(r) => void onChangeRole(m.userId, r)} className="bg-input-bg text-fg border border-border-subtle rounded font-medium" style={{ height: 32 }} /> : <span style={{ textTransform: 'capitalize' }}>{m.role}</span>}</td>
              {isAdmin && (
                <td style={TABLE_TD}>
                  {m.role !== 'owner' && (
                    <button
