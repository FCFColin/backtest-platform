...
import { REBALANCE_FREQUENCIES, REBALANCE_FREQUENCY_COLORS, REBALANCE_LABELS } from '@backtest/shared';
import { apiFetch } from '@/utils/apiClient';
import { buildBacktestParameters } from '@/utils/constants';
export const REBALANCE_OPTIONS: { value: RebalanceFrequency; label: string; color: string }[] = REBALANCE_FREQUENCIES.map((value) => ({
  value,
...
export function applyRebalanceBands(portfolios: Array<Record<string, unknown>>, absoluteBand: number | '', relativeBand: number | '') {
...
export function extractFreqResult(json: unknown, freq: RebalanceFrequency, label: string, color: string): FreqResult {
