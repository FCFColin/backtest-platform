...
import { FREQ_ORDER, OFFSETS, fetchFreqResult, fetchOffsetResult, type FreqResult } from './rebalancingSensitivityBuilders.js';
...
function createRebalancingRunners(
  ...
  const runOffsetScanInner = async (freq: RebalanceFrequency, validAssets: Array<{ ticker: string; weight: number }>) => {
    ...
    s.setOffsetResults([]);
    try {
      s.setOffsetResults(await Promise.all(OFFSETS.map((o) => fetchOffsetResult(o, freq, validAssets, params))));
    } catch {
  ...
  const runSensitivity = async () => {
    ...
    s.setOffsetResults([]);
    try {
      const all = await Promise.all(s.selectedFreqs.map((f) => fetchFreqResult(f, validAssets, params, s.absoluteBand, s.relativeBand)));
...
export function useRebalancingState(): RebalancingState {
  ...
  const toggleFreq = (freq: RebalanceFrequency) => s.setSelectedFreqs((prev) => (prev.includes(freq) ? prev.filter((f) => f !== freq) : [...prev, freq]));
  const {
    items: assets,
    addItem: addAsset,
