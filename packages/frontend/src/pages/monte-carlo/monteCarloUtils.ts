...
const ao = (...aw: [string, number][]): PortfolioState['assets'] =>
...
const PRESETS: [string, PortfolioState['assets'], number, number, number, number, number][] = [
  ...
  ['presetAllStockDCA', ao(['VTI', 100]), 30, 1000, 50000, 1, 5],
  ['presetThreeFund', ao(['VTI', 50], ['VXUS', 30], ['BND', 20]), 25, 500, 200000, 2, 8],
];
...
function usePortfolioOperations(
  ...
  return {
    ...
    getTotalWeight: (pIdx: number) => wSum(portfolios[pIdx]),
    isComplete: (pIdx: number) => wSum(portfolios[pIdx]) === 100,
  };
}
...
async function fetchMcResult(
  ...
  const p = portfolios[idx],
    res = await apiFetch('/api/v1/backtest/monte-carlo', {
      method: 'POST',
...
async function executeSimulation(s: McSetters, ops: PortfolioOps): Promise<void> {
  ...
  const reqBody = {
    ...
    mcParams: {
      ...
      estimationMethod: s.estimationMethod || undefined,
    },
    objectives: {
      mode: s.simMode,
      goal1: s.goal1,
...
export function useMonteCarloState() {
  const s = useMcSetters(),
    ops = usePortfolioOperations(s.portfolios, s.setPortfolios);
...
export function buildTerminalHistogram(r: MonteCarloResult, startingValue: number) {
  ...
  const vals = metrics.map((m) => m.finalValue * startingValue),
    { bins, labelFor } = buildBinData(vals, 25, dollarKFormatter),
    [p5Val, p50Val, p95Val] = [0.05, 0.5, 0.95].map((f) => percentile(vals, f)),
    [p5Label, p50Label, p95Label] = [p5Val, p50Val, p95Val].map(labelFor);
...
export function buildSummaryData(r: MonteCarloResult, startingValue: number, t: TFunction) {
  ...
  return (Object.keys(METRIC_FORMAT) as DistMetric[]).map((key) => {
    const vals = metricValues(metrics, key, startingValue),
      p = (f: number) => percentile(vals, f),
      m = mean(vals),
      s = std(vals),
      fmt = METRIC_FORMAT[key],
      values: Record<string, string> = { Std: key === 'finalValue' ? fmtAmount(s) : fmtNum(s) };
