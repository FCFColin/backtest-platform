...
function McResultsWrapper({ state }: { state: McState }) {
  return <MonteCarloResultsPanel error={state.error} results1={state.results1} results2={state.results2} portfolios={state.portfolios} portfolioMode={state.portfolioMode} activeTab={state.activeTab} setActiveTab={state.setActiveTab} startingValue={state.startingValue} numSimulations={state.numSimulations} distMetric={state.distMetric} setDistMetric={state.setDistMetric} />;
}
