...
function HistogramChart({
  ...
  const { t } = useTranslation(),
    rm = useMediaQuery('(prefers-reduced-motion: reduce)');
  ...
  if (rl?.length)
    s[0].markLine = {
      silent: true,
      data: rl.map(({ label: l, color, value }) => ({
        xAxis: l,
  ...
  return (
    <EChart
      ...
      height={height}
      ariaLabel={t('Frequency Distribution')}
    />
  );
}
...
function FanChart({ data, t }: { data: mcu.FanDataPoint[]; t: (key: string) => string }) {
  const ms = data.map((d) => String(d.month)),
    mm = new Map(data.map((d) => [d.month, d])),
    c = getPortfolioColor(0);
  const bandPair = (
    nm: string,
    ...
    },
    {
      ...
      stack: nm,
      name: lb,
      data: data.map((d) => rng(d)[1] - rng(d)[0]),
      symbol: 'none',
      lineStyle: { opacity: 0 },
  ...
  return (
    <EChart
      ...
      height={450}
      ariaLabel={t('Monte Carlo Fan Chart')}
    />
  );
}
...
function McTermHist({ r, sv }: BaseProps) {
  const { t } = useTranslation(),
    h = mcu.buildTerminalHistogram(r, sv);
...
function McRangeTab({ r, sv }: BaseProps) {
  const { t } = useTranslation(),
    data = mcu.buildFanChartData(r, sv);
  if (!data.length) return <NoDataCard />;
  return (
    <div className="flex flex-col gap-4">
      <Card className="p-5">
        <h4 className={H4}>{t('Monte Carlo Fan Chart')}</h4>
        <FanChart data={data} t={t} />
      </Card>
...
function McSummaryTab({ r, sv }: BaseProps) {
  const { t } = useTranslation(),
    rows = mcu.buildSummaryData(r, sv, t);
        ...
        <MetricsGrid
          metrics={[
            ...
            { label: t('Mean Final Value'), value: fmtAmount(st.meanFinalValue * sv) },
            {
              ...
              value: fmtPct(st.successRate, 1),
              color: 'hsl(var(--success))',
            },
            {
              label: t('Simulation Count'),
              value: `${r.perPathMetrics?.length ?? s.numSimulations}`,
            },
          ]}
        />
      </div>
