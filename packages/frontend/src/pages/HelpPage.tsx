    ...
    <div className="flex w-full flex-col gap-3">
      ...
      <Card className="p-6">
        <div className="mb-6 flex gap-2 border-b-2 border-subtle pb-3">
          {tabs.map((tab) => (
            <button key={tab.key} onClick={() => setSection(tab.key)} className={`flex items-center gap-1.5 rounded-lg px-4 py-2 text-label font-semibold transition-colors ${section === tab.key ? 'bg-brand/10 text-brand' : 'text-fg-tertiary hover:text-fg-secondary'}`}>
              {tab.icon}
              {tab.label}
            </button>
          ))}
...
function HelpSection({ icon, title, description, children }: { icon: ReactNode; title: string; description?: string; children: ReactNode }) {
  return (
    <div>
      ...
      {description && <div className="mb-5 text-body leading-loose text-fg-secondary">{description}</div>}
      {children}
    </div>
  );
}
function HelpGrid({ children }: { children: ReactNode }) {
  return <div className="mb-6 grid gap-4 [grid-template-columns:repeat(auto-fit,minmax(280px,1fr))]">{children}</div>;
}
function HelpInfoBox({ title, icon, children }: { title?: string; icon?: ReactNode; children: ReactNode }) {
  return (
...
function MethodologySection() {
  ...
  return (
    <HelpSection icon={<BookOpen className="size-6 text-brand" />} title={t('help.methodology.title')} description={t('help.methodology.desc')}>
      <HelpGrid>
        {metrics.map((m) => (
          <MetricCard key={m.name} name={m.name} fullName={m.info.fullName} formula={m.formula} desc={m.info.desc} />
        ))}
      </HelpGrid>
...
function DataSection() {
  ...
  return (
    <HelpSection icon={<Database className="size-6 text-brand" />} title={t('help.data.title')} description={t('help.data.desc')}>
      <HelpGrid>
        {sources.map((s) => (
      ...
      <HelpInfoBox icon={<TrendingUp className="size-4 text-success" />} title={t('help.data.updateStrategyTitle')}>
...
function MetricCard({ name, fullName, formula, desc }: { name: string; fullName: string; formula: string; desc: string }) {
  return (
    <div className="rounded-lg bg-input-bg p-4">
      ...
      <div className="mb-2 overflow-x-auto rounded bg-elevated px-2.5 py-1.5 font-mono text-caption text-fg">{formula}</div>
...
function FaqItemRow({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-subtle">
      <button onClick={() => setOpen(!open)} className="flex w-full items-center justify-between bg-input-bg px-4 py-3 text-left text-body font-semibold text-fg">
        {q}
        <ChevronDown className="size-4 shrink-0 text-fg-tertiary transition-transform duration-200" style={{ transform: open ? 'rotate(180deg)' : 'none' }} />
      </button>
