...
function AlertEmailInput({ email, enabled, onChange }: { email: string; enabled: boolean; onChange: (v: string) => void }) {
  ...
  return (
    <Field className="max-w-[360px]">
      ...
      <div className="relative">
        ...
        <Input id="alert-email" type="email" className="pl-8" value={email} onChange={(e) => onChange(e.target.value)} placeholder="alert@example.com" disabled={!enabled} />
      </div>
    </Field>
  );
}
function AlertTriggerOptions({ config, onToggle }: { config: EmailAlertConfig; onToggle: (t: EmailAlertConfig['triggers'][number]) => void }) {
  const { t } = useTranslation();
  return (
    <div className="max-w-[360px]">
      <div className="mb-2 text-label font-semibold text-fg">{t('tactical.results.alertTrigger')}</div>
      <div className="flex flex-col gap-2">
        {ALERT_TRIGGER_OPTIONS.map((opt) => (
          <label key={opt.value} htmlFor={`trigger-${opt.value}`} className="flex cursor-pointer items-center gap-2.5">
            <Checkbox id={`trigger-${opt.value}`} checked={config.triggers.includes(opt.value)} onCheckedChange={() => onToggle(opt.value)} disabled={!config.enabled} />
...
function AlertsTab() {
  ...
  const toggleTrigger = (trigger: EmailAlertConfig['triggers'][number]) => {
    setConfig((prev) => ({
      ...prev,
      triggers: prev.triggers.includes(trigger) ? prev.triggers.filter((tk) => tk !== trigger) : [...prev.triggers, trigger]
    }));
  };
  const handleSave = () => {
    ...
    run(async () => {
      await apiPostJSON('/api/v1/tactical/alerts', { config }, t('tactical.results.alertSaveFailed'));
      setSaved(true);
  ...
  return (
    <Card className="p-4">
      ...
      <div className="mb-4 flex items-center gap-2.5">
        ...
        <Switch checked={config.enabled} onCheckedChange={(v) => setConfig((prev) => ({ ...prev, enabled: v }))} className="ml-auto" />
      </div>
      <div className="mb-4">
        <AlertEmailInput email={config.email} enabled={config.enabled} onChange={(v) => setConfig((prev) => ({ ...prev, email: v }))} />
      </div>
      <div className="mb-4">
    ...
    <div className="flex flex-col gap-3">
      ...
      <Card className="p-4">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="backtest">{results ? <BacktestResultTab results={results} /> : <BacktestEmptyState />}</TabsContent>
