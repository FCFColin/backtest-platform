...
import { Badge, Button, Input, type BadgeProps } from '@/components/ui/uiComponents';
...
function AboutContent() {
  const { t } = useTranslation(),
    features = aboutData.features as { iconName: string; titleKey: string; descKey: string }[];
  return (
    <div>
...
function LimitsContent() {
  const { t } = useTranslation(),
    limits = (aboutData.limits as { labelKey: string; valueKey: string; descKey: string }[]).map(
      (l) => ({ label: t(l.labelKey), value: t(l.valueKey), desc: t(l.descKey) }),
    );
  return (
    <div>
      <div className="mb-6 text-body leading-loose text-fg-secondary">
...
export function AboutPage({ section = 'about' }: { section?: string }) {
  const { t } = useTranslation(),
    tab = ABOUT_TABS.find((x) => x.key === section) ?? ABOUT_TABS[0];
  return (
...
const CHANGE_META = {
  ...
  HL_CLS = 'rounded-full bg-brand/10 px-2 py-0.5 text-label-tiny font-semibold text-brand',
  BADGE_CLS = 'mt-0.5 shrink-0 min-w-[44px] justify-center';
export function ChangelogPage() {
  const { t } = useTranslation(),
    raw = t('changelog.versions', { returnObjects: true }) as Record<
      string,
      { date: string; highlight?: string; changes: { type: ChangeType; text: string }[] }
    >,
    versions = Object.entries(raw).map(([version, v]) => ({ version, ...v }));
  return (
...
export function ContactPage() {
  const { t } = useTranslation(),
    ...
    update = (k: 'name' | 'email' | 'message', v: string) => setForm((p) => ({ ...p, [k]: v })),
    handleSubmit = (e: FormEvent) => {
      ...
      const subject = encodeURIComponent(
          `[Feedback] ${form.name} - ${form.message.slice(0, 30)}...`,
        ),
