...
function TickerListSection({ p }: { p: FrontierParamsProps }) {
  ...
  return (
    <section className="flex flex-col gap-3">
      ...
      <TickerTagInput tickers={p.tickers.filter(Boolean)} onChange={handleTagChange} minCount={2} placeholder={t('efficientFrontier.params.tickerPlaceholder')} />
    </section>
  );
}
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <Field>
        ...
        <Input type="number" min={5} max={100} value={p.numPoints} onChange={(e) => p.onNumPointsChange(Number(e.target.value))} />
      </Field>
      <Field>
...
function AdvancedParamsGrid({ p }: { p: FrontierParamsProps }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <SelectField label={t('efficientFrontier.params.solveSpeed')} value={p.solveSpeed} onChange={p.onSolveSpeedChange} options={solveSpeedOptions(t)} />
      <Field>
        <FieldLabel>{t('efficientFrontier.params.minInclusionWeight')}</FieldLabel>
        <div className="relative">
          <Input type="number" min={0} max={100} className="pr-9" value={p.minInclusionWeight} onChange={(e) => p.onMinInclusionWeightChange(Number(e.target.value))} />
          <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-caption text-fg-tertiary">%</span>
        </div>
      </Field>
      <SelectField label={t('efficientFrontier.params.rebalanceFreq')} value={p.rebalanceFrequency} onChange={p.onRebalanceFrequencyChange} options={rebalanceFreqOptions(t)} />
      <SelectField label={t('efficientFrontier.params.returnObjective')} value={p.returnObjective} onChange={p.onReturnObjectiveChange} options={returnObjOptions(t)} />
      <SelectField label={t('efficientFrontier.params.solver')} value={p.solver} onChange={p.onSolverChange} options={solverOptions(t)} />
      <Field>
        <FieldLabel>{t('efficientFrontier.params.allowCash')}</FieldLabel>
        <label className="flex h-10 cursor-pointer items-center gap-2 text-label text-fg-secondary">
          <Checkbox checked={p.allowCash} onCheckedChange={(c) => p.onAllowCashChange(c === true)} />
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <Button onClick={props.onRun} disabled={props.isLoading} variant="primary" className="w-full">
        {props.isLoading ? <Loader2 className="size-4 animate-spin" /> : <Play className="size-4" />}
        {props.isLoading ? t('efficientFrontier.params.calculating') : t('efficientFrontier.params.calcFrontier')}
      </Button>
    </div>
  );
}
