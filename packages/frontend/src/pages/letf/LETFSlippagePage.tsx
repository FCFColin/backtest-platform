...
function LETFParamsWrapper({ state }: { state: LETFState }) {
  return <LETFParamsPanel letfTicker={state.letfTicker} benchmarkTicker={state.benchmarkTicker} leverage={state.leverage} startDate={state.startDate} endDate={state.endDate} isLoading={state.isLoading} onLetfTickerChange={state.setLetfTicker} onBenchmarkTickerChange={state.setBenchmarkTicker} onLeverageChange={state.setLeverage} onStartDateChange={state.setStartDate} onEndDateChange={state.setEndDate} onRun={state.runAnalysis} />;
}
function LETFResultsWrapper({ state }: { state: LETFState }) {
  return <LETFResultsPanel results={state.results} error={state.error} isLoading={state.isLoading} leverage={state.leverage} />;
}
