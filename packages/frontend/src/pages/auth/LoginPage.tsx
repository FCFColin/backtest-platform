...
export default function LoginPage() {
  ...
  const sessionMessage = useMemo(() => (sessionExpired ? t('auth.login.sessionExpired') : null), [sessionExpired, t]);
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <AuthFormField label={t('auth.login.password')} value={password} onChange={setPassword} type="password" autoComplete="current-password" />
        <ErrorBanner message={formError || error || sessionMessage} />
        <AuthSubmitButton loading={loading} icon={<LogIn className="w-4 h-4" />} label={t('auth.login.submit')} loadingLabel={t('auth.login.submitting')} />
      </form>
    </AuthPageLayout>
  );
}
