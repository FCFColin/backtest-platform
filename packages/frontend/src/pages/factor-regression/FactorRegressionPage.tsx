...
const FACTOR_OPTIONS = [
  ...
  pc = getPortfolioColor,
  FACTOR_COLORS = { alpha: pc(0), beta: pc(1), smb: pc(2), hml: pc(3) } as const;
...
async function fetchRegression(
  ...
  if (!tks.length) throw new Error(i18n.t('Insufficient price data available'));
  const tot = assets.reduce((sum, x) => sum + (x.weight || 0), 0),
    w = new Map(assets.map((x) => [x.ticker, (x.weight || 0) / tot])),
    longest = tks.reduce((x, y) => (x.dailyReturns.length > y.dailyReturns.length ? x : y)),
  ...
  return apiPostJSON<FactorRegressionResult>(
    ...
    { monthlyReturns, ffData: await loadFF(), factors, startDate, endDate },
...
function useFactorRegressionState(t: TFunction) {
  const s = useSetterState({
    ...
    list = useAssetList([...C.DEFAULT_60_40_ASSETS], newAsset, 0),
    { isLoading, error, run, setError } = useAsyncAction(),
    toggleFactor = (k: string) => {
      const f = s.selectedFactors;
      s.setSelectedFactors(f.includes(k) ? f.filter((x) => x !== k) : [...f, k]);
    },
    runRegression = () => {
      const valid = list.assets.filter((x) => x.ticker.trim() !== '');
      if (!valid.length) return setError(t('Please add at least one ticker'));
      const err = validateAssetWeights(list.assets);
      if (err) return setError(err);
      if (!s.selectedFactors.length) return setError(t('Please select at least one factor'));
      s.setResult(null);
      run(async () => {
        try {
...
function regressionTable(r: FactorRegressionResult, f: string[], t: TFunction) {
  const F = FACTOR_COLORS,
    ...
    maybe = (on: boolean, x: [string, string, string, string, string]) => (on ? [x] : []),
    specs: Array<[string, string, string, string, string]> = [
    ...
    rows = specs.map(([l, c, v, s, x]) => ({ label: l, color: c, value: v, cls: s, desc: x })),
    cols: SimpleTableColumn<(typeof rows)[number]>[] = [
      {
        key: 'label',
        label: t('Coefficient'),
            ...
            <CollapsibleSection title={t('Regression Residuals')} defaultOpen>
              <Card className="p-4">
                <BarChartContent
                  ...
                  xDataKey="index"
                  height={200}
                  yTickFormatter={(v) => (v as number).toFixed(3)}
                  tooltipValueFormatter={(v) => [(v as number).toFixed(4), t('Residual')]}
