...
const HERO_KEY = 'backtest-hero-expanded',
  ...
  ICON_CL = 'p-2 bg-brand-subtle/8 rounded-lg group-hover:bg-brand-subtle/12 transition-colors',
  TOOL_CL =
    'text-caption px-2.5 py-1 bg-brand-subtle/8 text-brand rounded-md hover:bg-brand-subtle/15 transition-colors',
  MORE_CL = 'text-caption text-brand hover:underline flex items-center gap-1',
  CHEV_CL = 'h-4 w-4 ml-1',
  ROW_CL = 'flex items-center gap-1.5 px-2.5 py-2 border-b border-border-subtle last:border-b-0',
  ROW_BTN_CL = 'flex-1 text-left bg-transparent border-none cursor-pointer p-0',
  fmtDate = (d: string, lang: string) => new Date(d).toLocaleString(lang);
type CapProps = {
...
function useBacktestPageState() {
  const { t } = useTranslation(),
    run = useBacktestStore((s) => s.runBacktest),
  ...
  useEffect(() => {
    ...
    try {
      const j = JSON.parse(d) as { portfolios?: Portfolio[]; parameters?: BacktestParameters },
        ps = (j.portfolios ?? []).map((p) => ({ ...p, id: p.id || `portfolio-${Date.now()}` }));
...
function BacktestToolbar({ state: s }: { state: S }) {
  const { t, i18n } = useTranslation(),
    loading = useBacktestStore((x) => x.isLoading),
    count = useBacktestStore((x) => x.portfolios.length);
  return (
      ...
      {s.saveOpen && (
        <div className="mt-2 flex items-center gap-1.5">
          <Input
            ...
            className="flex-1"
            /* eslint-disable-next-line jsx-a11y/no-autofocus -- 弹出即聚焦命名框，焦点不依赖先验交互 */
            autoFocus
          />
