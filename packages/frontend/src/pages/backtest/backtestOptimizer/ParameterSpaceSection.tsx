...
function FreqMultiSelect({ s }: OptimizerSectionProps) {
  ...
  return (
    <div>
      <div className="mb-1.5 text-caption font-medium text-fg-secondary">{t('backtest.optimizer.rebalanceFreq')}</div>
      <div className="flex flex-wrap gap-2">
        {FREQ_OPTIONS.map((opt) => {
          const active = s.frequencies.includes(opt.value);
          return (
            <Button key={opt.value} variant={active ? 'primary' : 'secondary'} size="sm" onClick={() => s.toggleFreq(opt.value)}>
              {opt.label}
            </Button>
          );
        })}
...
function ThresholdRangeInputs({ s }: OptimizerSectionProps) {
  ...
  return (
    <div>
      <div className="mb-1.5 text-caption font-medium text-fg-secondary">{t('backtest.optimizer.thresholdRange')}</div>
      <ParamRow>
        {[
          ...
          <ParamCard key={label as string} label={label as string}>
            <div className="flex items-center gap-2">
              <Input type="number" step="0.5" className="font-mono tabular-nums" value={val as string} onChange={(e) => (set as (v: string) => void)(e.target.value)} />
...
function CapitalRangeInputs({ s }: OptimizerSectionProps) {
  ...
  return (
    <div>
      <div className="mb-1.5 text-caption font-medium text-fg-secondary">{t('backtest.optimizer.capitalRange')}</div>
      <ParamRow>
        {fields.map(([label, val, set]) => (
          <ParamCard key={label} label={label}>
            <div className="flex items-center gap-2">
              <span className="text-body text-fg-tertiary font-mono shrink-0">$</span>
              <Input type="number" step="1000" className="font-mono tabular-nums" value={val} onChange={(e) => set(e.target.value)} />
            </div>
          </ParamCard>
        ))}
      </ParamRow>
...
export function ParameterSpaceSection({ s }: OptimizerSectionProps) {
  const { t } = useTranslation();
  return (
    <ParamsSection title={t('backtest.optimizer.paramSpace')} info={t('backtest.optimizer.paramSpaceInfo')}>
