...
function ConstraintRow({ enabled, setEnabled, label, value, setValue, placeholder }: ConstraintRowProps) {
  return (
    <div className="flex items-center gap-2.5">
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2">
          <Input type="number" step="0.1" className="font-mono tabular-nums" value={value} onChange={(e) => setValue(e.target.value)} placeholder={placeholder} disabled={!enabled} />
...
export function ObjectiveSection({ s }: OptimizerSectionProps) {
  ...
  return (
    <ParamsSection title={t('backtest.optimizer.objective')} info={t('backtest.optimizer.objectiveInfo')}>
      <ParamRow>
        <ParamCard label={t('backtest.optimizer.target')}>
          <Select value={s.objective} onValueChange={(v) => s.setObjective(v as Objective)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="minMaxDrawdown">{t('backtest.optimizer.minMaxDrawdown')}</SelectItem>
      ...
      <div className="mt-3 flex flex-col gap-3">
        <ConstraintRow enabled={s.enableMaxDD} setEnabled={s.setEnableMaxDD} label={t('backtest.optimizer.maxDrawdownConstraint')} value={s.maxDD} setValue={s.setMaxDD} placeholder={t('backtest.optimizer.maxDrawdownPlaceholder')} />
        <ConstraintRow enabled={s.enableMinCagr} setEnabled={s.setEnableMinCagr} label={t('backtest.optimizer.cagrConstraint')} value={s.minCagr} setValue={s.setMinCagr} placeholder={t('backtest.optimizer.cagrPlaceholder')} />
      </div>
    </ParamsSection>
  );
}
