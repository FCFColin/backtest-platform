...
function DcaParamsSection({ dcaFrequency, setDcaFrequency, dcaPeriods, setDcaPeriods, startingValue, baseCurrency, investTbill, setInvestTbill }: { dcaFrequency: DcaFrequency; setDcaFrequency: (v: DcaFrequency) => void; dcaPeriods: number; setDcaPeriods: (v: number) => void; startingValue: number; baseCurrency: 'usd' | 'cny'; investTbill: boolean; setInvestTbill: (v: boolean) => void }) {
  const { t } = useTranslation();
  const prefix = baseCurrency === 'usd' ? '$' : '¥';
  return (
    <div className="mt-4">
      <div className="mb-1.5 text-caption font-semibold text-fg-tertiary">{t('lumpSumDca.dcaParams')}</div>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Field>
          <FieldLabel htmlFor="lumpsum-dca-frequency">{t('lumpSumDca.dcaFrequency')}</FieldLabel>
          <select id="lumpsum-dca-frequency" className={selectClassName} value={dcaFrequency} onChange={(e) => setDcaFrequency(e.target.value as DcaFrequency)}>
        ...
        <Field>
          ...
          <div className="relative">
            <Input type="number" className="pr-10" value={dcaPeriods} onChange={(e) => setDcaPeriods(Number(e.target.value) || 1)} min={1} max={360} />
            <span className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-caption text-fg-tertiary">{t('lumpSumDca.dcaPeriodsUnit')}</span>
          </div>
        </Field>
        <Field>
          ...
          <div className="relative">
            ...
            <Input type="text" className="pl-7 opacity-70" value={Math.round(startingValue / dcaPeriods).toLocaleString()} readOnly />
          </div>
        </Field>
...
function LumpSumVsDCAParamsForm({ state }: { state: LumpSumVsDCAState }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      />
      <DcaParamsSection dcaFrequency={state.dcaFrequency} setDcaFrequency={state.setDcaFrequency} dcaPeriods={state.dcaPeriods} setDcaPeriods={state.setDcaPeriods} startingValue={state.startingValue} baseCurrency={state.baseCurrency} investTbill={state.investTbill} setInvestTbill={state.setInvestTbill} />
      <PortfolioEditor singleMode assets={state.assets} totalWeight={state.totalWeight} onAdd={state.addAsset} onRemove={state.removeAsset} onUpdate={state.updateAsset} />
      <LoadingButton isLoading={state.isLoading} onClick={state.runComparison} loadingText={t('lumpSumDca.comparing')} className="w-full">
...
function LumpSumVsDCAResults({ state }: { state: LumpSumVsDCAState }) {
  const { t } = useTranslation();
  const fmtMoney = (v: number) => (state.baseCurrency === 'usd' ? `$${v.toLocaleString(undefined, { maximumFractionDigits: 0 })}` : `¥${v.toLocaleString(undefined, { maximumFractionDigits: 0 })}`);
  return (
    <>
      {state.error && (
