  ...
  return (
    <thead>
      <tr className="bg-input-bg">
        <th className="border-b-2 border-subtle px-3 py-2.5 text-left text-caption font-semibold text-fg-tertiary">{t('lumpSumDca.stats.metric')}</th>
        {results.map((r, idx) => (
          <th key={r.label} className="border-b-2 border-subtle px-3 py-2.5 text-right text-caption font-semibold text-fg-tertiary">
          ...
          {STATS_ROWS.map((row, rowIdx) => {
            ...
            return (
              <tr key={row.key} className={rowIdx % 2 === 1 ? 'bg-input-bg' : ''}>
                <td className="border-b border-subtle px-3 py-2 text-label text-fg-secondary">{t(row.label)}</td>
                {results.map((r) => {
                  const val = r[row.key];
                  return (
                    <td key={r.label} className="border-b border-subtle px-3 py-2 text-right font-mono text-label font-medium text-fg">
...
function ConclStatCard({ title, value, color }: { title: string; value: ReactNode; color?: string }) {
  return (
    <div className="rounded-lg bg-elevated p-3">
      <div className="mb-1 text-caption text-fg-tertiary">{title}</div>
      <div className="font-mono text-body font-semibold" style={{ color: color ?? 'hsl(var(--fg-secondary))' }}>
        {value}
      </div>
    </div>
  );
...
function ConclusionAnalysis({ ls, dca, fmtPct, fmtMoney }: { ls: CompareResult; dca: CompareResult } & Pick<FmtFns, 'fmtPct' | 'fmtMoney'>) {
  const { t } = useTranslation();
  ...
  return (
    <div className="mb-5 rounded-lg bg-input-bg p-4">
      <div className="mb-2.5 flex items-center gap-2">
        {lsWins ? <TrendingUp className="size-5 text-success" /> : <TrendingDown className="size-5 text-brand" />}
        <span className="text-body font-semibold text-fg">{t('lumpSumDca.conclusion.title')}</span>
      </div>
      <div className="mb-3 grid grid-cols-3 gap-3">
        <ConclStatCard title={t('lumpSumDca.conclusion.winningStrategy')} value={lsWins ? t('lumpSumDca.lumpSumLabel') : t('lumpSumDca.dcaLabel')} color={lsWins ? CHART_COLORS[0] : CHART_COLORS[1]} />
        <ConclStatCard
          title={t('lumpSumDca.conclusion.finalValueDiff')}
          value={
            <>
              {fmtMoney(finalValueDiff)} <span className="text-caption text-fg-tertiary">({finalValueDiffPct.toFixed(1)}%)</span>
            </>
          }
        />
        <ConclStatCard title={t('lumpSumDca.conclusion.maxDrawdownDiff')} value={fmtPct(mddDiff)} />
      </div>
      <ConclusionText lsWins={lsWins} ls={ls} dca={dca} fmtPct={fmtPct} fmtMoney={fmtMoney} finalValueDiffPct={finalValueDiffPct} />
    </div>
  );
}
  ...
  return (
    <div className="mt-4 flex items-start gap-2.5 rounded-lg bg-input-bg p-3">
      ...
      <div className="text-body leading-relaxed text-fg-tertiary">
        ...
        {lsWins ? t('lumpSumDca.conclusion.riskWarningLumpSum') : t('lumpSumDca.conclusion.riskWarningDca')}
        {t('lumpSumDca.conclusion.historicalDisclaimer')}
      </div>
    </div>
  );
}
export function LsDcaResultsCard({ s, fmtPct, fmtNum, fmtMoney }: FmtFns & { s: LumpSumVsDCAState }) {
  const { t } = useTranslation();
  if (s.results.length !== 2) return null;
  return (
    <Card className="p-5">
      <ConclusionAnalysis ls={s.results[0]} dca={s.results[1]} fmtPct={fmtPct} fmtMoney={fmtMoney} />
      <div className="mb-3 text-body font-semibold text-fg">{t('lumpSumDca.results.growthCurveTitle')}</div>
      <GrowthCurveChart results={s.results} />
      <div className="mb-3 mt-6 text-body font-semibold text-fg">{t('lumpSumDca.results.statsTitle')}</div>
