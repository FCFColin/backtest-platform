...
const TH = 'text-left text-xs font-semibold text-fg-tertiary px-[10px] py-2',
  TD = 'text-label text-fg-secondary py-2 px-[10px]',
  INPUT = 'bg-input-bg text-fg border border-border-subtle rounded font-medium',
  DEL_BTN = 'inline-flex items-center bg-transparent border-0 cursor-pointer text-danger',
...
function useOrgMembersState(admin: boolean) {
  const [members, setMembers] = useState<Member[]>([]),
    ...
    act = async (failMsg: string, doReq: () => Promise<Response>, clearError = false) => {
      setBusy(true);
      try {
...
export default function OrgMembersPage() {
  const { t } = useTranslation(),
    { org, isAdmin } = useOrgAuth(),
    s = useOrgMembersState(isAdmin),
    { load } = s;
...
function InviteDialog({
  ...
  const { t } = useTranslation(),
    ...
    [role, setRole] = useState<Role>('analyst'),
    submit = (e: FormEvent) => {
      e.preventDefault();
