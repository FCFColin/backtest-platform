...
function renderDir(d: SignalDir, t: TFunction): ReactNode {
  ...
  if (d === 'sell') return <span className="font-semibold text-neg">{t('signal.common.sell')}</span>;
...
function StatsComparisonTable({ statRows }: { statRows: StatRow[] }) {
  ...
  const columns: SimpleTableColumn<(typeof STAT_COLS)[number]>[] = [
    ...
    ...statRows.map((r, idx) => ({
      ...
      render: (col: (typeof STAT_COLS)[number]) => formatStat((r.stats as Record<string, number>)[col.key], col.fmt)
    }))
  ];
  return (
    <CollapsibleSection title={t('signal.dual.statsComparison')} defaultOpen className="rounded-xl border border-border bg-surface">
    ...
    <ResultsContainer>
      ...
      {results && (
        <>
          ...
          <CollapsibleSection title={t('signal.dual.signalComparison', { count: results.comparison.length })} defaultOpen className="rounded-xl border border-border bg-surface">
            {results.comparison.length > 0 ? <SortableTable columns={comparisonColumns} data={results.comparison} initialSortKey="date" initialSortDir="asc" /> : <div className="py-6 text-center text-body text-fg-tertiary">{t('signal.common.noSignal')}</div>}
          </CollapsibleSection>
          <CollapsibleSection title={t('signal.dual.equityCurveComparison')} defaultOpen className="rounded-xl border border-border bg-surface">
            <EquityLineChart
              data={equityData}
              series={[
                {
