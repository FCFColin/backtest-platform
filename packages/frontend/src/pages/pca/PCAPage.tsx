...
const EigenvalueBarChart = ({ data }: { data: { component: string; eigenvalue: number }[] }) => {
  const { t } = useTranslation();
  return (
    <Card className="p-4">
      <BarChartContent
...
const CumulativeVarianceChart = ({
  ...
  data: { component: string; cumulative: number }[];
}) => {
  const { t } = useTranslation();
  return (
    <Card className="p-4">
...
const PCAScatterChart = ({ data }: { data: { pc1: number; pc2: number }[] }) => (
  <Card className="p-4">
    <XYScatterChart
      xKey="pc1"
      yKey="pc2"
