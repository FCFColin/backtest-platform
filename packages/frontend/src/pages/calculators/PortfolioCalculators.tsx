...
export function TwoFundPortfolioCalculator() {
  ...
  const { frontier, minVarW, minVarCagr, minVarVol } = useMemo(() => computeTwoFundFrontier(cagrA, volA, cagrB, volB, corr), [cagrA, volA, cagrB, volB, corr]);
  return (
    <CollapsibleCard icon={PieChart} title={t('calculators.portfolio.twoFundTitle')}>
      ...
      <div className="mt-3">
        <Field label={t('calculators.portfolio.correlation')} value={corr} onChange={setCorr} step={0.05} min={-1} max={1} />
      </div>
      <div className="mt-1">
        <ResultRow label={t('calculators.portfolio.minVarWeight')} value={`${(minVarW * 100).toFixed(1)}%`} tone="brand" />
        <ResultRow label={t('calculators.portfolio.minVarCagr')} value={`${minVarCagr.toFixed(2)}%`} />
        <ResultRow label={t('calculators.portfolio.minVarVol')} value={`${minVarVol.toFixed(2)}%`} />
      </div>
