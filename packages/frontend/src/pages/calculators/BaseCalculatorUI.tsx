...
const TC: Record<ResultTone, string> = {
  brand: 'text-brand',
...
function normCdf(x: number) {
  const y = x / Math.SQRT2,
    a = Math.abs(y),
    t = 1 / (1 + 0.3275911 * a),
    p = NORM_COEFFS.reduce((acc, c) => acc * t + c) * t;
...
function ResultRow({ label, value, tone = 'default' }: Row) {
  return (
    <div className="flex items-center justify-between border-b border-border-subtle py-1.5 last:border-b-0">
      ...
      <span className={cn('font-mono tabular-nums text-label font-semibold', TC[tone])}>
        {value}
      </span>
    </div>
  );
...
const corrExtra: CalcConfig['extra'] = (s, set) => (
  <Field
    label="Correlation"
    value={s.corr ?? 0.2}
      ...
      chart: (
        <div className="mt-3">
          <SimpleAreaChart
            ...
            height={160}
            xDataKey="year"
            yTickFormatter={(v) => (v as number).toFixed(1)}
            tooltipFormatter={(v) => [(v as number).toFixed(3), t('Asset Ratio')]}
            series={[
              { dataKey: 'ratio', color: getPortfolioColor(2), width: 2, areaOpacity: 0.12 },
            ]}
          />
        </div>
      ),
