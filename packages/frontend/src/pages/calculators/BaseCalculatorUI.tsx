  ...
  return (
    <FieldShell>
      ...
      <div className="relative">
        <Input type="number" value={value} onChange={(e) => onChange(Number(e.target.value))} min={min} max={max} step={step} className={suffix ? 'pr-10' : undefined} />
        {suffix && <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-caption text-fg-tertiary">{suffix}</span>}
      </div>
    </FieldShell>
  );
}
...
export function ResultRow({ label, value, tone = 'default' }: ResultRowProps) {
  return (
    <div className="flex items-center justify-between border-b border-border-subtle py-1.5 last:border-b-0">
      ...
      <span className={cn('font-mono tabular-nums text-label font-semibold', RESULT_TONE_CLASS[tone])}>{value}</span>
    </div>
  );
}
export function InfoBox({ children }: { children: ReactNode }) {
  return <div className="mt-2.5 rounded-md bg-input-bg p-3 text-caption leading-relaxed text-fg-tertiary">{children}</div>;
}
...
export function CollapsibleCard({ icon: Icon, title, defaultOpen = false, children }: CollapsibleCardProps) {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <Card className="overflow-hidden bg-elevated">
      <Collapsible open={open} onOpenChange={setOpen} className="w-full">
        <CollapsibleTrigger className={cn('flex w-full items-center gap-2.5 p-4 text-left', 'transition-colors duration-150 hover:bg-hover')}>
          ...
          <ChevronDown className={cn('size-4 shrink-0 text-fg-tertiary transition-transform duration-200', open && 'rotate-180')} />
        </CollapsibleTrigger>
        <CollapsibleContent>
    ...
    <div className="mt-3 h-[220px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data}>
          ...
          />
          <Tooltip contentStyle={CHART_TOOLTIP_STYLE} formatter={(v: number, name: string) => [`${v.toFixed(2)}%`, name === 'cagr' ? 'CAGR' : name]} labelFormatter={(l: number) => t('calculators.base.volatilityWithValue', { value: `${l.toFixed(2)}%` })} />
    ...
    <div className="mt-3 h-[160px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data}>
          ...
          <Tooltip contentStyle={CHART_TOOLTIP_STYLE} formatter={(v: number) => [v.toFixed(3), t('calculators.base.assetRatio')]} />
          <Area type="monotone" dataKey="ratio" stroke={CHART_COLORS[2]} fill={CHART_COLORS[2]} fillOpacity={0.12} strokeWidth={2} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
