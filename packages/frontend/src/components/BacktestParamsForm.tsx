  ...
  return (
    <div className="flex items-start gap-3 py-2">
      ...
      <div className="flex-1">
        ...
        {description && (
          <div className="text-caption text-fg-tertiary mt-0.5">{description}</div>
        )}
      </div>
    </div>
  );
...
function BasicParamsSection() {
  const { t } = useParamField();
  const parameters = useBacktestStore(useShallow((s) => s.parameters));
  const updateParameter = useBacktestStore((s) => s.updateParameter);
  const [advancedOpen, setAdvancedOpen] = useState(false);

  const dateRangeMode =
    parameters.startDate === '' && parameters.endDate === '' ? 'all' : 'custom';
  ...
  const handleNum = (key: 'startingValue' | 'rollingWindowMonths', e: ChangeEvent<HTMLInputElement>) =>
