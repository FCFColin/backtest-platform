 ...
 *     <input ... />
 *   </ParamsSection>
 *   <ParamsSection title="高级设置" defaultOpen={false} plain>
 *     <input ... />
 *   </ParamsSection>
 * </ParamsPanel>
 */
 ...
 * - plain 模式：无底部边框，标题更低调，适合次要分区
 * @deprecated 请使用 components/params/ 目录下的新统一组件（ParamSection/ParamRow/ParamCard/ParamGroup/ActionBar）
 */
export function ParamsSection({
  ...
  return (
    <div style={plain ? {} : { borderBottom: '1px solid var(--border-soft)' }}>
      {title && (
        <div
        ...
        >
          <div className="flex items-center gap-1.5">
            ...
            <span
              style={{
                fontSize: plain ? '13px' : '14px',
                fontWeight: plain ? 500 : 600,
                color: plain ? 'var(--text-muted)' : 'var(--text-strong)',
              }}
            >
              {title}
            </span>
      ...
      {open && <div className={`px-2 ${plain ? 'pb-2' : 'pb-4'}`}>{children}</div>}
    </div>
  );
}
