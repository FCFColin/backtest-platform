...
import { GlidepathForm, PortfolioCardV2, AllocationBar, TotalWeightBlock } from './portfolioEditor/portfolioEditor.js';
...
function SinglePortfolioEditor({ assets, totalWeight, onAdd, onRemove, onUpdate, header, wrapInSection = true, cardStyle, isComplete }: Omit<SingleModeProps, 'singleMode'>) {
  const { t } = useTranslation();
  const complete = isComplete ?? validateAssetWeights(assets) === null;
  const card = (
    <div className="flex flex-col gap-1.5 p-3 bg-surface border border-border-subtle rounded-lg" style={wrapInSection ? undefined : cardStyle}>
      {header}
      <div className="flex flex-col gap-1.5">
        {assets.map((a, i) => (
          <div key={i} className="flex items-center gap-2">
            <Input type="text" value={a.ticker} onChange={(e) => onUpdate(i, 'ticker', e.target.value)} placeholder={t('optimizer.tickerPlaceholder')} className="h-8 min-w-0 flex-1 font-mono uppercase" />
            <AffixInput type="number" value={a.weight || ''} onChange={(e) => onUpdate(i, 'weight', Number(e.target.value))} min={0} max={100} suffix="%" className="h-8 w-[96px] shrink-0 font-mono tabular-nums" />
            <Button variant="destructive" size="icon" className="h-8 w-8 shrink-0" onClick={() => onRemove(i)} title={t('common.delete')} aria-label={t('common.delete')}>
              <X className="w-4 h-4" />
            </Button>
          </div>
        ))}
      ...
      <div className="flex items-center gap-2 pt-2 mt-1 border-t border-border-subtle">
        <span className="shrink-0 text-caption text-fg-tertiary uppercase tracking-wide">{t('portfolio.total')}</span>
...
const REBALANCE_KEYS: RebalanceFrequency[] = ['none', 'annual', 'quarterly', 'monthly', 'weekly', 'daily', 'threshold'];
...
const buildRebalanceOptions = (t: TFunc) => REBALANCE_KEYS.map((value) => ({ value, label: t(REBALANCE_LBL[value]) }));
...
function MenuButton({ label, icon, onClick }: { label: string; icon?: ReactNode; onClick: () => void }) {
  return (
    <button type="button" className="flex items-center gap-2 w-full px-3 py-2 text-left text-caption text-fg bg-transparent hover:bg-hover transition-colors border-0 cursor-pointer" role="menuitem" onClick={onClick}>
      {icon}
      {label}
    </button>
  );
}
function AddMenuDropdown({ t, onAdd, onAddPreset, onAddGlidepath, onLoadExample, onLoadCompareExample, onComingSoon, presetSubOpen, setPresetSubOpen, close }: AddMenuActions & { presetSubOpen: boolean; setPresetSubOpen: (v: boolean) => void; close: () => void }) {
  const wrap = (fn: () => void) => () => {
    fn();
    close();
  };
  return (
    <div className="absolute top-full right-0 mt-1 z-30 min-w-[200px] bg-surface border border-border rounded-lg shadow-lg py-1" role="menu">
      <MenuButton label={t('portfolio.addEmpty')} onClick={wrap(onAdd)} />
      <PresetSubmenu t={t} onAddPreset={onAddPreset} onSubOpen={presetSubOpen} setSubOpen={setPresetSubOpen} close={close} />
      ...
      <MenuButton label={t('portfolio.loadCompareExample')} icon={<GitCompare className="w-3.5 h-3.5 shrink-0" />} onClick={wrap(onLoadCompareExample)} />
    </div>
  );
}
function AddPortfolioMenu(props: AddMenuActions) {
  ...
  return (
    <div ref={menuRef} className="relative">
      <Button variant="secondary" size="sm" aria-expanded={addMenuOpen} onClick={() => setAddMenuOpen((v) => !v)}>
        <Plus className="w-3.5 h-3.5" />
        {t('portfolio.addPortfolio')}
        <ChevronDown className="w-3.5 h-3.5" />
      </Button>
      {addMenuOpen && <AddMenuDropdown {...props} presetSubOpen={presetSubOpen} setPresetSubOpen={setPresetSubOpen} close={() => setAddMenuOpen(false)} />}
    </div>
  );
}
function PresetSubmenu({ t, onAddPreset, onSubOpen, setSubOpen, close }: { t: TFunc; onAddPreset: (presetId: string) => void; onSubOpen: boolean; setSubOpen: (v: boolean) => void; close: () => void }) {
  return (
    <div className="relative" onMouseEnter={() => setSubOpen(true)} onMouseLeave={() => setSubOpen(false)}>
      <button type="button" className="flex w-full px-3 py-2 text-left text-caption text-fg bg-transparent hover:bg-hover transition-colors border-0 cursor-pointer" role="menuitem">
        {t('portfolio.addPreset')}
        <ChevronDown className="w-3 h-3 ml-auto rotate-[-90deg]" />
      </button>
      {onSubOpen && (
        <div className="absolute top-0 right-full mr-1 min-w-[240px] bg-surface border border-border rounded-lg shadow-lg py-1" role="menu">
...
function PortfolioEditorHeader({ t, count, ...menuActions }: { t: TFunc; count: number } & AddMenuActions) {
  return (
    <div className="flex items-center justify-between gap-2 flex-wrap mb-3">
      ...
      <div className="flex items-center gap-1.5">
        <Button variant="ghost" size="sm" onClick={() => useToastStore.getState().addToast('warning', t('portfolio.comingSoon'))}>
...
function MultiPortfolioEditor() {
  ...
  return (
    <div className="flex flex-col gap-2">
      <PortfolioEditorHeader t={t} count={portfolios.length} onAdd={() => addPortfolio()} onAddPreset={(presetId) => addPortfolio(presetId)} onAddGlidepath={handleAddGlidepath} onLoadExample={() => addPortfolio('60-40')} onLoadCompareExample={handleLoadCompareExample} onComingSoon={() => useToastStore.getState().addToast('warning', t('portfolio.comingSoon'))} />
      {showGlidepathForm && (
        <GlidepathForm
      ...
      <div className={portfolios.length === 1 ? 'max-w-[460px]' : 'flex flex-wrap items-start gap-3'}>
        {portfolios.length === 0 ? (
          ...
          portfolios.map((portfolio, idx) => <PortfolioCardV2 key={portfolio.id} portfolio={portfolio} index={idx} color={getPortfolioColor(idx)} rebalanceOptions={rebalanceOptions} nonGlidepathPortfolios={nonGlidepathPortfolios} onUpdate={updatePortfolio} onDelete={() => removePortfolio(portfolio.id)} onDuplicate={() => duplicatePortfolio(portfolio.id)} onSave={(p) => handleSavePortfolio(p, parameters, t)} onDeepAnalysis={() => {}} />)
        )}
      </div>
    </div>
  );
