...
function SinglePortfolioEditor({
  ...
  const { t } = useTranslation(),
    weightError = isComplete !== undefined ? null : validateAssetWeights(assets),
    complete = isComplete ?? weightError === null,
    card = (
      <div
...
function MultiPortfolioEditor() {
  const { t } = useTranslation(),
    [confirmDialog, confirmDelete] = useConfirmDialog(),
    portfolios = useBacktestStore((s) => s.portfolios),
    addPortfolio = useBacktestStore((s) => s.addPortfolio),
    addGlidepath = useBacktestStore((s) => s.addGlidepath),
    duplicatePortfolio = useBacktestStore((s) => s.duplicatePortfolio),
    removePortfolio = useBacktestStore((s) => s.removePortfolio),
    updatePortfolio = useBacktestStore((s) => s.updatePortfolio),
    parameters = useBacktestStore((s) => s.parameters),
    rebalanceOptions = useMemo(() => buildRebalanceOptions(t), [t]),
    [showGlidepathForm, setShowGlidepathForm] = useState(false),
    nonGlidepathPortfolios = useMemo(() => portfolios.filter((p) => !p.isGlidepath), [portfolios]),
    handleAddGlidepath = () => {
