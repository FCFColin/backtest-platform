...
interface AnalysisErrorAlertProps {
  ...
  prefix?: ReactNode;
  /** 自定义 className，默认空；用于覆盖默认 Card 样式 */
  className?: string;
 ...
 * @returns 渲染的错误提示卡片，error 为空时返回 null
 */
...
export function EmptyResultsHint({ text, className }: EmptyResultsHintProps) {
  return (
    <Card className={cn('flex items-center justify-center p-12 text-center text-body text-fg-tertiary', className)}>
      {text}
    </Card>
  );
}
 ...
 * @param props - 见 WarningBannersProps
 * @returns 渲染的警告横幅组
 */
