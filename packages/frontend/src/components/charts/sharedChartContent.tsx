...
export interface SimpleChartProps extends CommonChart {
  type?: 'line' | 'area';
  data: ChartDataPoint[];
  xDataKey?: string;
  ...
  legendFormatter?: (n: string) => string;
  areaColor?: string;
  colorOffset?: number;
  dataZoom?: boolean;
  ariaLabel?: string;
...
export function SimpleChart({
  ...
  xTickInterval: xInt,
  xTickFontSize: xSize,
  yTickFormatter: yFmt = (v) => v.toFixed(0),
  yDomain = ['auto', 'auto'],
  yScale,
  yLabel,
  tooltipFormatter: tFmt,
  tooltipLabelFormatter: tLbl,
  showLegend,
  legendPosition: lgPos = 'bottom',
  legendFormatter: lgFmt,
  areaColor,
  colorOffset = 0,
  dataZoom,
  ariaLabel,
  ...
  const anim = useChartAnimation(data.length >= 100).isAnimationActive,
    isArea = type === 'area',
    isCat = xType !== 'number',
    leg = showLegend ?? !isArea,
    dz = dataZoom === true && data.length >= 100,
    lgAnchor = lgPos === 'top' ? { top: 0 } : { bottom: dz ? 26 : 0 };
  // ECharts series 对象为开放式结构，此处仅构造无需跨项复用的字面量
  const arr = series.map(
    (
      s,
      i,
    ): EChartsOption['series'] extends infer T
      ? T extends readonly (infer U)[]
        ? U
        : never
      : never => {
      const c = s.color ?? getPortfolioColor(i + colorOffset);
      return {
        name: s.name ?? s.dataKey,
        type: 'line',
        data: isCat
          ? data.map((d) => d[s.dataKey] ?? null)
          : data.map((d) => [Number(d[xKey]), d[s.dataKey] ?? null]),
        ...
        lineStyle: {
          width: s.width ?? 2.5,
          type: (s.dash ?? 'solid') as 'solid' | 'dashed' | 'dotted',
          color: c,
          opacity: s.opacity,
        },
        itemStyle: { color: c },
        ...
        areaStyle: isArea
          ? areaColor
            ...
            : { color: c, opacity: s.areaOpacity ?? 0.15 }
          : undefined,
      };
    },
  );
    ...
    xAxis: isCat
      ? C.categoryAxis(
          data.map((d) => d[xKey] ?? ''),
          {
            ...
            nameGap: 28,
            formatter: xFmt,
            interval: xInt === 'preserveStartEnd' ? 'auto' : xInt,
            preserveStartEnd: xInt === 'preserveStartEnd',
            fontSize: xSize ?? 11,
          },
        )
      : C.valueXAxis({ formatter: xFmt, name: xLabel, nameGap: 38, fontSize: xSize ?? 11 }),
    yAxis: C.valueYAxis({
      formatter: yFmt,
      name: yLabel,
      nameGap: 48,
...
export const SimpleAreaChart = mkSimple('area', 440),
  SimpleLineChart = mkSimple('line');
interface BarChartContentProps extends CommonChart {
  data: ChartDataPoint[];
  seriesNames: string[];
  xDataKey: string;
...
export function BarChartContent({
  ...
  const anim = useChartAnimation(data.length >= 100).isAnimationActive,
    single = seriesNames.length === 1 && signColorSingleSeries;
  return (
    <EChart
      option={{
        grid: { left: 80, right: 40, top: 20, bottom: 20 + (showLegend ? 24 : 0) },
        xAxis: C.categoryAxis(
          data.map((d) => String(d[xDataKey])),
          { fontSize: xTickFontSize ?? 11, interval: xTickInterval ?? 'auto' },
        ),
        yAxis: C.valueYAxis({ formatter: yFmt, name: yLabel, nameGap: 48 }),
        tooltip: C.tooltipOption(C.axisTooltipFormatter(undefined, tFmt)),
        legend: showLegend ? C.chartLegend() : undefined,
      ...
      height={height}
      ariaLabel={seriesNames.join(', ')}
    />
  );
}
type ScatterStyle<T> = (d: T) => string;
export interface XYScatterSeriesSpec<T extends object = object> extends BaseSeries {
  data: T[];
  colorOf?: ScatterStyle<T>;
...
interface XYScatterChartProps<T extends object = object> extends CommonChart {
  xKey: string;
  yKey: string;
  xName: string;
  yName: string;
  ...
  series: XYScatterSeriesSpec<T>[];
}
export function XYScatterChart<T extends object = Record<string, number | string>>({
  xKey,
  yKey,
  xName,
  yName,
  ...
  lines,
  onClick,
}: XYScatterChartProps<T>) {
  const pts = series.reduce((n, s) => n + s.data.length, 0),
    anim = useChartAnimation(pts >= 500).isAnimationActive;
  type Row = Record<string, number | string>;
  const rows = series.map((s) => s.data as readonly Row[]);
  // ECharts series 对象为开放式结构，此处仅构造无需跨项复用的字面量
  const arr = series.map(
    (
      s,
      ...
      : never => {
      const row = rows[si];
      const zv = s.zDataKey ? row.map((d) => Number(d[s.zDataKey!]) || 0) : [],
        mm = zv.length ? { min: Math.min(...zv), max: Math.max(...zv) } : null,
        sz = (v: number) => {
          if (!mm || mm.min === mm.max) return s.symbolSize ?? 8;
          const [rMin, rMax] = s.zRange ?? zRange;
          return rMin + ((v - mm.min) / (mm.max - mm.min)) * (rMax - rMin);
        };
      return {
        type: 'scatter',
        data: row.map((d, di) => ({
          ...
          itemStyle: s.colorOf
            ? { color: s.colorOf(s.data[di]) }
            : { color: s.color ?? getPortfolioColor(si), opacity: s.opacity },
          symbolSize: s.zDataKey ? sz(Number(d[s.zDataKey]) || 0) : (s.symbolSize ?? 8),
        })),
        symbol: s.symbol ?? 'circle',
        label: s.showLabels ? C.scatterLabel() : undefined,
  ...
  if (lines)
    for (const l of lines)
      arr.push({
        ...
        lineStyle: {
          color: l.color,
          type: (l.dash ?? 'dashed') as 'solid' | 'dashed' | 'dotted',
          width: l.width ?? 2,
        },
        silent: true,
        tooltip: { show: false },
  ...
  const opt: EChartsOption = {
    ...
    tooltip: C.tooltipOption((p: { name: string; value: [number, number]; color: string }) => {
      const [x, y] = p.value,
        h = lFmt ? lFmt(p.name) : p.name;
      const row = (v: number, n: string, k: string) => {
        const r = tFmt ? tFmt(v, k) : [String(v), n];
        const [vv, nn] = Array.isArray(r) ? r : [r, n];
        return C.tooltipRow(p.color, nn, String(vv));
      };
      return (
        (h
          ? `<div style="font-weight:600;margin-bottom:6px;color:hsl(var(--fg))">${C.escapeHtml(h)}</div>`
          : '') +
        row(x, xName, xKey) +
        row(y, yName, yKey)
      );
  ...
  return (
    <EChart option={opt} height={height} ariaLabel={`${xName} vs ${yName}`} onClick={onClick} />
  );
}
