...
export default function SeasonalityChart({ portfolios }: SeasonalityChartProps) {
  ...
  const monthLabels = Array.from({ length: 12 }, (_, i) => t('charts.seasonality.monthLabel', { n: i + 1 }));
  ...
  return (
    <ChartCard title={t('charts.seasonality.title')} data={data} csvFilename="seasonality">
      <BarChartContent data={data} seriesNames={portfolios.map((p) => p.name)} xDataKey="month" height={400} yTickFormatter={(v) => `${v.toFixed(0)}%`} tooltipValueFormatter={(v) => [`${v.toFixed(2)}%`, '']} yLabel={t('charts.seasonality.avgReturnAxis')} barRadius={2} signColorSingleSeries />
    </ChartCard>
  );
}
