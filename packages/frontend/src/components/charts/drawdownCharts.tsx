...
function DrawdownAreaChart({
  ...
  const stats = useMemo(
    () => (showStats ? computeStats(portfolios[0]?.drawdownCurve ?? []) : null),
...
export function DrawdownChart({ portfolios }: { portfolios: P[] }) {
  const { t } = useTranslation();
  return (
    <DrawdownAreaChart
...
export function UnderwaterCurve({ portfolios }: { portfolios: P[] }) {
  const { t } = useTranslation();
  return (
    <DrawdownAreaChart
      portfolios={portfolios}
