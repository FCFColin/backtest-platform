...
function RiskMetricSelector({ metrics, selected, onChange }: { metrics: Array<{ key: RiskMetricKey; label: string }>; selected: RiskMetricKey; onChange: (v: RiskMetricKey) => void }) {
  return (
    <select className="bg-input-bg text-fg border border-border-subtle rounded font-medium cursor-pointer" style={{ width: 130, fontSize: 12, padding: '4px 8px' }} value={selected} onChange={(e) => onChange(e.target.value as RiskMetricKey)}>
      {metrics.map((m) => (
...
function RiskScatterChart({ data, riskLabel }: { data: ScatterPoint[]; riskLabel: string }) {
  return <ScatterChartContent data={data} xDataKey="risk" yDataKey="cagr" nameDataKey="name" xName={riskLabel} yName="CAGR" xLabel={`${riskLabel} (%)`} yLabel="CAGR (%)" tooltipFormatter={(value: number | string, name: string) => (name === 'risk' ? [`${typeof value === 'number' ? value.toFixed(2) : value}%`, riskLabel] : name === 'cagr' ? [`${typeof value === 'number' ? value.toFixed(2) : value}%`, 'CAGR'] : [String(value), name])} tooltipLabelFormatter={() => ''} />;
}
export const RiskReturnChart = memo(function RiskReturnChart({ results }: { results: AssetAnalysisResult }) {
