import { AreaChart, Area, CartesianGrid, ResponsiveContainer, Brush } from 'recharts';
...
function AllocationAreaChart({ data, assets, showBrush, fillOpacity }: { data: Array<Record<string, string | number>>; assets: Portfolio['assets']; showBrush: boolean; fillOpacity: number }) {
  return (
    <ResponsiveContainer width="100%" height={400}>
      <AreaChart data={data} margin={CHART_MARGIN}>
        ...
        <ChartTooltip labelFormatter={(label: string) => label} formatter={(value: number, name: string) => [`${value.toFixed(1)}%`, name]} />
        <ChartLegend />
        {assets.map((asset, idx) => (
          <Area key={asset.ticker} type="monotone" dataKey={asset.ticker} name={asset.ticker} stackId="1" stroke={CHART_COLORS[idx % CHART_COLORS.length]} fill={CHART_COLORS[idx % CHART_COLORS.length]} fillOpacity={fillOpacity} activeDot={{ r: 5, stroke: 'var(--bg-elevated)', strokeWidth: 2 }} />
        ))}
        {showBrush && <Brush dataKey="date" height={20} stroke="var(--brand)" travellerWidth={8} tickFormatter={dateFormatter} />}
      </AreaChart>
    </ResponsiveContainer>
  );
}
function AllocationHistoryChart({ assets, allocationHistory }: { assets: Portfolio['assets']; allocationHistory: NonNullable<AllocationPortfolio['allocationHistory']> }) {
  ...
  return (
    <ChartCard title={t('charts.portfolioAllocation.title')} data={data} csvFilename="portfolio-allocation">
      <AllocationAreaChart data={chartData} assets={assets} showBrush={chartData.length > 100} fillOpacity={0.6} />
    </ChartCard>
  );
}
function InitialWeightChart({ assets, growthCurve }: { assets: Portfolio['assets']; growthCurve: Array<{ date: string; value: number }> }) {
  const { t } = useTranslation();
  ...
  return (
    <ChartCard title={t('charts.portfolioAllocation.title')} data={data} csvFilename="portfolio-allocation">
      <AllocationAreaChart data={data} assets={assets} showBrush={data.length > 100} fillOpacity={0.6} />
      <div className="text-label-tiny mt-2 text-center" style={{ color: 'var(--text-muted)', fontStyle: 'italic' }}>
