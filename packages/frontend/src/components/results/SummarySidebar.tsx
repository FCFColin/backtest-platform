...
export function SummarySidebar({ stats, totalYears, positiveYears }: SummarySidebarProps) {
  ...
  return (
    <>
      {/* 移动端：横向滚动指标行 */}
      <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1 lg:hidden" role="list" aria-label={t('summarySidebar.title')}>
        {metrics.map((m) => (
          <Card key={m.labelKey} role="listitem" className="flex-shrink-0 min-w-[130px] p-3" data-testid={m.testId}>
            <div className="text-label-tiny text-fg-tertiary mb-1 whitespace-nowrap">{t(m.labelKey)}</div>
            <div className={cn('text-body font-mono tabular-nums font-semibold', toneClass(m.tone))}>{m.value}</div>
          </Card>
        ))}
      </div>
      {/* 桌面端：sticky 指标卡片 */}
      <Card className="hidden lg:block p-4 lg:sticky lg:top-20" data-testid="summary-sidebar">
        ...
        <dl className="space-y-2.5">
          {metrics.map((m) => (
            <div key={m.labelKey} className="flex items-center justify-between gap-3" data-testid={m.testId}>
              <dt className="text-caption text-fg-tertiary whitespace-nowrap">{t(m.labelKey)}</dt>
              <dd className={cn('text-caption font-mono tabular-nums font-semibold text-right', toneClass(m.tone))}>{m.value}</dd>
            </div>
          ))}
        </dl>
      </Card>
