    ...
    <div className="bg-surface border border-border rounded-xl" data-testid="drawdown-episodes-panel">
      ...
      <div className="flex items-center justify-between p-4 border-b border-border">
        ...
        <div className="flex items-center gap-2">
          <select value={severity} onChange={(e) => setSeverity(e.target.value as Severity)} className="text-caption bg-input-bg border border-border rounded-md px-2 py-1 text-fg" aria-label={t('components.drawdownEpisodes.aria.severity')} data-testid="filter-severity">
          ...
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value as 'depth' | 'duration' | 'recovery')} className="text-caption bg-input-bg border border-border rounded-md px-2 py-1 text-fg" aria-label={t('components.drawdownEpisodes.aria.sortBy')} data-testid="sort-selector">
      ...
      <div>
        ...
        {hasMore && (
          <div className="p-4 border-t border-border-subtle text-center">
            <button onClick={() => setDisplayLimit((prev) => prev + 10)} className="text-caption text-brand hover:underline" data-testid="show-more-episodes">
...
function DrawdownSummary({ episodes }: { episodes: DrawdownEpisode[] }) {
  ...
  const summary = {
    ...
    avgRecovery: episodes.filter((e) => e.recoveryTime > 0).reduce((s, e) => s + e.recoveryTime, 0) / Math.max(episodes.filter((e) => e.recoveryTime > 0).length, 1)
  };
  return (
...
function DrawdownEpisodeRow({ episode, testId }: { episode: DrawdownEpisode; testId: string }) {
  ...
  return (
    <div className="border-b border-border-subtle last:border-b-0" data-testid={testId}>
      <button onClick={() => setExpanded(!expanded)} className="w-full flex items-stretch hover:bg-hover/50 transition-colors">
        <div className={cn('w-1 flex-shrink-0', sev === 'severe' && 'bg-danger', sev === 'moderate' && 'bg-warning', sev === 'mild' && 'bg-fg-tertiary/40')} />
        <div className="flex-1 flex items-center gap-6 py-4 px-6">
          <div className="w-24 text-right">
            <div className="text-h3 font-mono tabular-nums text-neg">{formatPercent(episode.depth)}</div>
          </div>
          <div className="flex-1">
            <TimelineViz episode={episode} />
          </div>
          <div className="text-caption text-fg-tertiary flex flex-col items-end">
            <span data-testid="episode-status">{episode.recoveryDate ? t('components.drawdownEpisodes.status.recovered') : t('components.drawdownEpisodes.status.ongoing')}</span>
            <span className="font-mono tabular-nums" data-testid="episode-duration">
              {formatDuration(episode.totalTimeDurationDays)}
            </span>
          </div>
          {expanded ? <ChevronDown className="h-4 w-4 text-fg-tertiary" /> : <ChevronRight className="h-4 w-4 text-fg-tertiary" />}
        </div>
      </button>
      {expanded && (
        <div className="px-6 pb-4 pl-11 grid grid-cols-2 md:grid-cols-3 gap-4 bg-surface-sunken/30">
          ...
          {episode.recoveryTime > 0 && <DetailField label={t('components.drawdownEpisodes.detail.recoveryTime')} value={formatDuration(episode.recoveryTime)} />}
          {episode.recoveryFactor !== undefined && <DetailField label={t('components.drawdownEpisodes.detail.recoveryFactor')} value={episode.recoveryFactor.toFixed(2)} />}
          {episode.cagrDuring !== undefined && <DetailField label={t('components.drawdownEpisodes.detail.periodCagr')} value={formatPercent(episode.cagrDuring)} colorize />}
...
function TimelineViz({ episode }: { episode: DrawdownEpisode }) {
  ...
  const totalMs = recoveryDate ? recoveryDate.getTime() - peakDate.getTime() : Date.now() - peakDate.getTime();
  ...
  return (
    <div className="relative h-8" data-testid="drawdown-timeline">
      ...
      <div className="absolute left-0 top-1/2 -translate-y-1/2 flex flex-col items-center">
        ...
        <div className="mt-1 text-micro text-fg-tertiary font-mono whitespace-nowrap">{episode.peakDate}</div>
      </div>
      <div className="absolute top-1/2 -translate-y-1/2 flex flex-col items-center" style={{ left: `${Math.max(0, Math.min(100, troughPos))}%` }}>
        <div className="w-3 h-3 rounded-full bg-danger" />
        {!troughLabelHidden && <div className="mt-1 text-micro text-fg-tertiary font-mono whitespace-nowrap">{episode.troughDate}</div>}
      </div>
      {recoveryDate && (
        <div className="absolute right-0 top-1/2 -translate-y-1/2 flex flex-col items-center">
          <div className="w-2.5 h-2.5 rounded-full bg-success" />
          {!recoveryLabelHidden && <div className="mt-1 text-micro text-fg-tertiary font-mono whitespace-nowrap">{episode.recoveryDate}</div>}
        </div>
      )}
    </div>
  );
}
function DetailField({ label, value, colorize }: { label: string; value: string; colorize?: boolean }) {
  return (
    <div>
      <div className="text-label-tiny text-fg-tertiary">{label}</div>
      <div className={cn('text-body font-mono tabular-nums', colorize && value.startsWith('-') && 'text-neg', colorize && !value.startsWith('-') && 'text-pos')}>{value}</div>
    </div>
  );
}
