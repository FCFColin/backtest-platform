...
-- 汇总迁移回滚：DROP 全部表（回滚后系统视为未初始化，重新执行 001 即全新初始化）。
-- schema_migrations 由 migrations.ts 管理，不在此 DROP 列表内。
...
DROP TABLE IF EXISTS prices, tickers, cpi_data, exchange_rates, users, organizations,
  ...
  backtest_runs, outbox, api_keys, email_verification_tokens, invitations,
