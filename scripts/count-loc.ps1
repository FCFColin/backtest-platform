...
$fileCount = ($result | Measure-Object -Property Count -Sum).Sum
# 追加全局账本（本地观测流；nightly ε-1 为入库权威写入者）
Add-Content $ledgerPath ('{"ts":"' + (Get-Date -Format o) + '","total":' + $total + ',"files":' + $fileCount + '}')
$cut = $baseline - $total
...
Write-Host ''
Write-Host ('=== 全仓库合计: {0} lines in {1} files ===' -f $total, $fileCount)
