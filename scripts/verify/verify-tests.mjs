...
await runCheck(results, 'C-012', () => {
  ...
  const parseLine = (label) => {
    ...
    const p = m[1];
    return { failed: parseInt(p.match(/(\d+)\s+failed/)?.[1] ?? '0'), passed: parseInt(p.match(/(\d+)\s+passed/)?.[1] ?? '0'), skipped: parseInt(p.match(/(\d+)\s+skipped/)?.[1] ?? '0'), found: true };
  };
  ...
  if (tests.failed > 0 || (r.code !== 0 && !tests.found)) {
    ...
    if (failedTests.length === 0) for (const m of output.matchAll(/FAIL\s+(.+?(?:\.test\.[jt]sx?))/g)) if (failedTests.length < 10) failedTests.push(m[1].trim());
  }
  const pass = r.code === 0 && tests.failed === 0;
  return {
    status: pass ? 'PASS' : 'FAIL',
    summary: pass ? `单测全部通过: ${tests.passed} passed, 0 failed` : r.code !== 0 && !tests.found ? `单测运行异常: exit ${r.code}` : `单测失败: ${tests.failed} failed / ${tests.passed} passed / ${tests.skipped} skipped`,
    details: { exitCode: r.code, testsPassed: tests.passed, testsFailed: tests.failed, filesFailed: files.failed, failedTests, outputTail: output.slice(-3000) },
  };
});
...
await runCheck(results, 'C-013', () => {
  ...
  const staticChecks = [
    ...
    ['缺失 coverage-summary.json 时 exit(1)', /existsSync/.test(src) && /process\.exit\(1\)/.test(src) && /覆盖率数据缺失|未找到 coverage-summary\.json/.test(src)],
    ['JSON 解析失败时 exit(1)', /JSON\.parse/.test(src) && /catch/.test(src) && /解析失败/.test(src) && /process\.exit\(1\)/.test(src)],
    ['缺少 total 字段时 exit(1)', /!total|typeof total/.test(src) && /缺少 total|total 汇总字段/.test(src) && /process\.exit\(1\)/.test(src)],
    ['全局门槛检查（≥80%/70%）', /GLOBAL_THRESHOLDS/.test(src) && /lines:\s*80/.test(src) && /functions:\s*80/.test(src) && /branches:\s*70/.test(src) && /pct\s*<\s*threshold/.test(src)],
    ['失败时 exit(1)', /(?:globalFailures|criticalFailures|failures)\.length\s*>\s*0[\s\S]*?process\.exit\(1\)/.test(src)],
  ...
  if (scriptExists) {
    ...
    const semanticOk = r.code === 0 ? /通过|pass|✅|PASS/i.test(out) : /覆盖率|coverage|门槛|threshold|FAIL|✕/i.test(out);
  ...
  return { status: checks.every((c) => c.pass) ? 'PASS' : 'FAIL', summary: `覆盖率门控: ${passed}/${checks.length} 项通过`, details: { checks } };
});

// ── C-014: Go 覆盖率验证 ─────────────────────────────────────
await runCheck(results, 'C-014', () => {
  if (!existsSync(join(PROJECT_ROOT_PATH, 'engine-go'))) return { status: 'FAIL', summary: 'engine-go 目录不存在' };
  const testR = runCmd('cd engine-go && go test ./... -coverprofile=coverage.out', { timeout: 300000 });
  if (testR.code !== 0) return { status: 'FAIL', summary: `go test 失败 (exit ${testR.code})`, details: { outputTail: (testR.out + testR.err).slice(-2000) } };
  const coverR = runCmd('cd engine-go && go tool cover -func=coverage.out');
  if (coverR.code !== 0 || !coverR.out.trim()) return { status: 'FAIL', summary: `go tool cover 失败` };
  const lines = coverR.out.trim().split('\n');
  const totalCov = parseFloat(lines[lines.length - 1].match(/([\d.]+)%/)?.[1] ?? '0');
  const pkgs = ['signal', 'pca', 'letf', 'factorregression', 'goaloptimizer', 'mathutil', 'calculators', 'engine/tactical'];
  const pkgResults = {};
  for (const pkg of pkgs) {
    const pkgLines = lines.filter((l) => l.includes(`internal/${pkg}/`));
    const pcts = pkgLines.map((l) => parseFloat(l.match(/([\d.]+)%/)?.[1] ?? '0'));
    pkgResults[pkg] = pkgLines.length === 0 ? { found: false } : { found: true, max: Math.max(...pcts).toFixed(2), count: pkgLines.length };
  }
  const allHaveCov = Object.values(pkgResults).every((r) => r.found && r.max > 0);
  const pass = totalCov >= 70 && allHaveCov;
  return { status: pass ? 'PASS' : 'FAIL', summary: `Go 覆盖率: ${totalCov}% (>= 70%), 8 包 ${allHaveCov ? '全覆盖' : '部分缺失'}`, details: { totalCov, pkgResults } };
});
...
await runCheck(results, 'H-zod-coverage', () => {
  ...
  const files = readdirSync(absDir).filter((f) => f.endsWith('.ts') && f !== 'routeUtils.ts').sort();
  const allMissing = [];
  ...
  for (const f of files) {
    ...
    while ((m = routeRegex.exec(content)) !== null) {
      ...
      const window = content.slice(m.index, nextIdx > 0 ? nextIdx : Math.min(content.length, m.index + 4000));
      if (!/validate\s*\(/.test(window)) {
        const pm = content.slice(m.index).match(/\(\s*(['"`])([^'"`]*?)\1/) || content.slice(m.index).match(/\(\s*([A-Za-z_]\w*)/);
        missing.push({ method, path: pm ? (pm[2] ?? pm[1] ?? '<unknown>') : '<unknown>', line: content.slice(0, m.index).split('\n').length, file: f });
      }
    }
  ...
  return {
    status: ok ? 'PASS' : 'FAIL',
    summary: ok ? `all POST/PUT/PATCH have Zod validation (${validatedRoutes}/${totalRoutes})` : `${allMissing.length} route(s) missing validation (${validatedRoutes}/${totalRoutes}, ${pct}%)`,
