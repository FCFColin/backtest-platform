...
import { runCmd, fileExists, readFileContent, grepInCode, runCheck, finishVerify } from './_lib.mjs';
...
await runCheck(results, 'C-007', () => {
  ...
  const details = { overlays: {}, pathCheck: {}, kubectlVersion: kubectlRes.out.trim().split('\n')[0] };
  let allPass = true;
  for (const env of OVERLAYS) {
    const dir = `k8s/overlays/${env}/`;
    const kExists = fileExists(`${dir}kustomization.yaml`);
    if (!kExists) { details.overlays[env] = { kustomizationExists: false, pass: false }; allPass = false; continue; }
    ...
    const flaggedRes = runCmd(`kubectl kustomize --load-restrictor LoadRestrictionsNone ${dir}`, { timeout: 60000 });
  ...
  return { status: allPass ? 'PASS' : 'FAIL', summary: allPass ? 'All 3 overlays kustomize build OK' : 'Some overlays failed', details };
});

// ── C-008: readiness probe ───────────────────────────────────
await runCheck(results, 'C-008', () => {
  const ALLOWED = ['/api/ready', '/health/ready'];
  const WHITELIST = { 'alertmanager-deployment.yaml': '/-/ready', 'frontend-deployment.yaml': '/', 'otel-collector.yaml': '/', 'unleash-deployment.yaml': '/health' };
  ...
  for (const match of probeMatches) {
    ...
    let pathValue = null, probeType = null;
    for (let i = match.line; i < Math.min(lines.length, match.line + 12); i++) {
      const line = lines[i] || '';
      if (line.trim() !== '' && !line.trim().startsWith('#') && line.length - line.trimStart().length <= indent) break;
      ...
      if (pm) { pathValue = pm[1].trim().replace(/^['"]|['"]$/g, ''); break; }
    }
    ...
    if (needsCheck) findings.push({ file: match.file, line: match.line, path: pathValue, conforms });
  }
  const nonConforming = findings.filter((f) => f.path !== null && !f.conforms);
  return {
    status: nonConforming.length === 0 ? 'PASS' : 'FAIL',
    summary: nonConforming.length === 0 ? `All ${findings.length} readinessProbe paths conform` : `${nonConforming.length} non-conforming: ${nonConforming.map((f) => `${f.file}:${f.line}`).join('; ')}`,
...
await runCheck(results, 'C-009', () => {
  ...
  } catch (e) { return { status: 'FAIL', summary: `Cannot read k8s/network-policies: ${e.message}` }; }
  ...
  for (const file of npFiles) {
    ...
    for (const doc of content.split(/^---\s*$/m)) {
      if (!doc.trim() || !/^kind:\s*NetworkPolicy\s*$/m.test(doc) || !/prometheus/i.test(doc)) continue;
      const ports = [...doc.matchAll(/^\s*port:\s*(\d+)\s*$/gm)].map((m) => parseInt(m[1], 10));
      policies.push({ file, ports, hasForbidden: ports.includes(FORBIDDEN), allAllowed: ports.length > 0 && ports.every((p) => ALLOWED_PORTS.includes(p)) });
    }
  }
  ...
  return { status: pass ? 'PASS' : 'FAIL', summary: pass ? `${policies.length} prometheus NetworkPolicy port(s) all allowed` : 'NetworkPolicy check failed', details: { policies } };
});

// ── C-010: Prometheus metrics ────────────────────────────────
await runCheck(results, 'C-010', () => {
  const EXPECTED = ['outbox_unprocessed_count', 'outbox_oldest_unprocessed_age_seconds', 'outbox_total_rows', 'rate_limiter_redis_unavailable_total', 'ws_connections_active'];
  ...
  for (const match of registerMatches) {
    ...
    if (content === undefined) { try { content = readFileContent(match.file); fileCache.set(match.file, content); } catch { continue; } }
    const lines = content.split('\n');
    for (let i = match.line - 1; i >= Math.max(0, match.line - 15); i--) {
      const nm = lines[i] && lines[i].match(/^\s*name:\s*['"]([^'"]+)['"]/);
      if (nm) { foundMetrics.push(nm[1]); break; }
    }
  }
  ...
  return {
    ...
    summary: staticAllFound ? 'All 5 metrics registered to getPrometheusRegister()' : `Missing metrics: ${missing.join(', ')}`,
...
await runCheck(results, 'C-011', () => {
  ...
  return { status: ok ? 'PASS' : 'FAIL', summary: ok ? `HPA fields OK (${right.length} uses of stabilizationWindowSeconds)` : `wrong=${wrong.length}, right=${right.length}` };
});
