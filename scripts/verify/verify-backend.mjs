...
try {
  ...
  results['C-002'] = { status: 'FAIL', summary: `脚本异常: ${e.message}`, details: { error: e.message, stack: e.stack } };
}

// ── C-003: Webhook SSRF 防护验证 ───────────────────────────────
try {
  results['C-003'] = await (async () => {
  ...
  results['C-003'] = { status: 'FAIL', summary: `验证脚本异常: ${e.message}`, details: { error: e.message } };
}
...
try {
  ...
  if (!fileExists(C018_targetFile)) {
    results['C-018'] = { status: 'FAIL', summary: `${C018_targetFile} 不存在` };
  } else {
  ...
  results['C-018'] = { status: 'FAIL', summary: `验证脚本异常: ${e.message}`, details: { error: e.message } };
}
...
try {
  ...
  if (!fileExists(C020_targetFile)) {
    results['C-020'] = { status: 'FAIL', summary: `${C020_targetFile} 不存在` };
  } else {
    ...
    if (timeoutMs === null) {
      results['C-020'] = { status: 'FAIL', summary: `${C020_targetFile} 中未找到 ENGINE_TIMEOUT_MS 的数值`, details: { content: content.slice(0, 2000) } };
    } else {
  ...
  results['C-020'] = { status: 'FAIL', summary: `验证脚本异常: ${e.message}`, details: { error: e.message } };
}
...
try {
  ...
  if (!existsSync(C021_absDir)) {
    results['C-021'] = { status: 'FAIL', summary: `${C021_targetDir} 目录不存在` };
  } else {
  ...
  results['C-021'] = { status: 'FAIL', summary: `验证脚本异常: ${e.message}`, details: { error: e.message } };
}
...
try {
  ...
  results['C-024'] = { status: 'FAIL', summary: `脚本异常: ${e.message}`, details: { error: e.message, stack: e.stack } };
}

writeAggregatedResult('verify-backend', results);
process.exit(0);
