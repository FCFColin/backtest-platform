...
content = content.replaceAll(
  '{config.portfolios.length} 个组合',
  "{config.portfolios.length} {t('backtest.portfoliosCount')}",
);
...
content = content.replaceAll(
  '`${sd.name} 保本率`',
  "`${sd.name} ${t('backtest.preservationRate')}`",
);
content = content.replaceAll(
  '`${pt.years}年存活率`',
  "`${pt.years}${t('common.years')}${t('backtest.survivalRate')}`",
);
content = content.replaceAll(
  '`${pt.years}年保本率`',
  "`${pt.years}${t('common.years')}${t('backtest.preservationRate')}`",
);
