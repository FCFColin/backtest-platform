...
═══════════════════════════════════════════════
§2 当前状态快照（@f4f63551 · 实测）
═══════════════════════════════════════════════

【LOC 四层口径】(powershell -NoProfile -File scripts/count-prod-loc.ps1, @f4f63551)
全仓 87,911
生产域毛值 53,556 含契约层
契约层(扣除) 1,057 backend/src/schemas 8 文件
迁移(单列观测) 447 migrations/*.sql 只增不减
净生产代码 52,499 ← G-1 门禁口径
...
SSR−344 A4+17 A5+46 B3+39 a11y+21 U-1+67 U-2P1+299 → 净 +84【全部实测】

G-1 即时状态：52,499，距 ≤52,000 差 499 行
