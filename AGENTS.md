...
- **修改代码时必须尽可能减少行数，而不是增加行数**。如果不得不增加，必须尽可能少增加，并且增加后立即自我激进精简一波。
- **始终寻找削减机会、激进重构优化简化机会和删除机会**。每次修改代码时同步扫描周围的死代码、冗余和简化机会。包括但不限于：
...
- **不得对抗格式化工具**（prettier / gofmt / eslint）。所有代码必须通过 `prettier --write`、`gofmt -w`、`eslint` 检查。
- 不允许 `// prettier-ignore`、`# fmt: off`、超长行、YAML/JSON 流式折叠牺牲可读性。
