...
# 版本：v2.1 · 基准：master@31e3cf4b · 87,044 行
...
生产代码（ex-契约层 ex-迁移）≈ 53,276 【实测·@31e3cf4b】
...
G-1 生产代码门禁
硬上限 ≤ 55,000 行（当前 53,276，冗余 1,724）
目标 ≤ 52,000 行（缺口 1,276）
口径：packages/*/src + engine-go + data-fetcher
− 契约层（schemas）− 迁移文件
tests/scripts/config 不设行数门禁
(53276, pnpm loc 后手算, @31e3cf4b)【实测】
...
β 已兑现 −29 ✓ 【实测】 @第2会话
engine gonum −200~~400 【推断·低置信】 待 spike
D-5 i18n −100~~200 【推断】 待采样
ε 流程件 0 【确定】 待落地
─────────────────────────────────────────────────
可承诺合计 −63（已兑现）
乐观合计 −363~663
...
注：乐观合计 << 缺口。engine spike 若字节级通过且净省≥200，
配合 D-5 执行，缺口有望收窄至 ~500 行内。
届时用户选择：接受 ~52.7k / 授权新战场 / 认定完成。
