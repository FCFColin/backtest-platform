...
- **处理审计发现或修复清单时，只执行"能减行或零成本"的修复**（顺手 bug、死代码、重复收敛）；需大量增行或高风险的一律跳过，并在回复中列明原因。
- **如确需增加行数**：尽可能少增加，多增加有用功能与修复，低性价比不做。
- **功能与可读性优先于行数**：不得以减行为由删除仍暴露配置面/仍有对标参考价值的功能子系统；删除前须确认零生产消费者且无悬挂配置面。
