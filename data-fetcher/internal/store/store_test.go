...
if len(result) != 0 { t.Errorf("expected 0 results for nil input, got %d", len(result)) }
	result = filterPricePointsByDate([]PricePoint{}, "2024-01-01", "2024-12-31")
if len(result) != 0 { t.Errorf("expected 0 results for empty input, got %d", len(result)) }
}
...
if len(result) != 3 { t.Fatalf("expected 3 results, got %d", len(result)) }
}
func TestFilterPricePointsByDate_BoundaryDates(t *testing.T) {
	...
	result := filterPricePointsByDate(prices, "2024-01-01", "2024-12-31")
if len(result) != 3 { t.Fatalf("expected 3 results (boundaries inclusive), got %d", len(result)) }
if result[0].Date != "2024-01-01" { t.Errorf("first result date = %s, want 2024-01-01", result[0].Date) }
if result[2].Date != "2024-12-31" { t.Errorf("last result date = %s, want 2024-12-31", result[2].Date) }
}
...
if len(result) != 2 { t.Fatalf("expected 2 results (start only), got %d", len(result)) }
}
...
if len(result) != 2 { t.Fatalf("expected 2 results (end only), got %d", len(result)) }
}
...
if len(result) != 3 { t.Fatalf("expected 3 results (no bounds), got %d", len(result)) }
}
func TestFilterPricePointsByDate_AllOutOfRange(t *testing.T) {
	...
	result := filterPricePointsByDate(prices, "2024-01-01", "2024-12-31")
if len(result) != 0 { t.Errorf("expected 0 results (all out of range), got %d", len(result)) }
}
...
if p.Date != "2024-01-15" { t.Errorf("Date = %s, want 2024-01-15", p.Date) }
if p.Open != 100.5 { t.Errorf("Open = %v, want 100.5", p.Open) }
if p.Volume != 1000000 { t.Errorf("Volume = %d, want 1000000", p.Volume) }
}
func TestSearchResult_Fields(t *testing.T) {
	r := SearchResult{Ticker: "AAPL", Name: "Apple Inc", Market: "US"}
if r.Ticker != "AAPL" { t.Errorf("Ticker = %s, want AAPL", r.Ticker) }
if r.Name != "Apple Inc" { t.Errorf("Name = %s, want Apple Inc", r.Name) }
if r.Market != "US" { t.Errorf("Market = %s, want US", r.Market) }
}
func TestNew_EmptyDatabaseURL(t *testing.T) {
	_, err := New(context.Background(), "", nil)
if err == nil { t.Error("New with empty databaseURL should return error") }
}
func TestNew_InvalidDatabaseURL(t *testing.T) {
	_, err := New(context.Background(), "not-a-valid-url", nil)
if err == nil { t.Error("New with invalid databaseURL should return error") }
}
...
if err != nil { t.Errorf("expected nil error for empty input, got: %v", err) }
if valid != nil { t.Errorf("expected nil valid for empty input, got: %v", valid) }
if invalid != nil { t.Errorf("expected nil invalid for empty input, got: %v", invalid) }
}
