...
if poolCfg.ConnConfig.TLSConfig == nil { poolCfg.ConnConfig.TLSConfig = &tls.Config{ ServerName: poolCfg.ConnConfig.Host, } }
...
func (ds *DataStore) GetPriceData(ctx context.Context, ticker, startDate, endDate string) ([]PricePoint, bool, error) {
	...
	if err != nil { return nil, false, fmt.Errorf("查询价格数据失败: %w", err) }
	defer rows.Close()
...
if err := rows.Scan(&date, &p.Open, &p.High, &p.Low, &p.Close, &p.Volume, &adjClose); err != nil { return nil, false, fmt.Errorf("扫描价格行失败: %w", err) }
		p.Date = date.Format("2006-01-02")
if adjClose != nil { p.AdjClose = *adjClose }
		prices = append(prices, p)
	}
	if len(prices) > 0 { return prices, false, nil }
...
if endDate == "" { endDate = time.Now().Format("2006-01-02") }
	...
	if err != nil { return nil, false, fmt.Errorf("标的数据不存在: %s", ticker) }
...
func (ds *DataStore) fetchAndStoreFromProvider(ctx context.Context, ticker, startDate, endDate string) ([]PricePoint, error) {
	...
	goStart := startDate
if goStart == "" { goStart = "2000-01-01" }
	goEnd := endDate
if goEnd == "" { goEnd = time.Now().Format("2006-01-02") }
	dailyPrices, providerName, err := provider.FetchWithFallback(providers, ticker, goStart, goEnd)
	if err != nil { return nil, fmt.Errorf("从 provider 获取 %s 失败: %w", ticker, err) }
...
if err := ds.writeGoPricesToDB(ctx, ticker, dailyPrices); err != nil { slog.Warn("写入数据库失败（不影响返回）", "ticker", ticker, "error", err) }
...
func (ds *DataStore) writeGoPricesToDB(ctx context.Context, ticker string, prices []provider.DailyPrice) error {
	if ds.pool == nil { return fmt.Errorf("数据库未连接") }
	batch := &pgx.Batch{}
	batch.Queue(`
	...
	defer br.Close()
	if _, err := br.Exec(); err != nil { return fmt.Errorf("插入 ticker 失败: %w", err) }
	for range prices { if _, err := br.Exec(); err != nil { return fmt.Errorf("写入价格数据失败: %w", err) } }
	return nil
}
func (ds *DataStore) SearchTickers(ctx context.Context, query string, limit int) ([]SearchResult, error) {
	rows, err := ds.pool.Query(ctx, `
		...
		LIMIT $2
	`, "%"+query+"%", limit)
	if err != nil { return nil, fmt.Errorf("搜索标的失败: %w", err) }
	defer rows.Close()
	var results []SearchResult
	for rows.Next() {
		var r SearchResult
		if err := rows.Scan(&r.Ticker, &r.Name, &r.Market); err != nil { return nil, fmt.Errorf("扫描搜索结果失败: %w", err) }
		results = append(results, r)
	}
	return results, nil
}
func (ds *DataStore) BatchValidateTickers(ctx context.Context, tickers []string) (valid []string, invalid []string, err error) {
	if len(tickers) == 0 { return nil, nil, nil }
	rows, err := ds.pool.Query(ctx, `
		SELECT DISTINCT ticker FROM prices WHERE ticker = ANY($1)
	`, tickers)
	if err != nil { return nil, nil, fmt.Errorf("校验标的失败: %w", err) }
	defer rows.Close()
	validSet := make(map[string]bool, len(tickers))
	for rows.Next() {
		var t string
		if err := rows.Scan(&t); err != nil { return nil, nil, fmt.Errorf("扫描校验结果失败: %w", err) }
		validSet[t] = true
	}
