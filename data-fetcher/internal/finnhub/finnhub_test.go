...
func parseCandleResponse(body []byte) ([]provider.DailyPrice, error) {
	...
	if resp.S == "no_data" { return []provider.DailyPrice{}, nil }
	if resp.S != "ok" { return nil, errors.New("Finnhub API 错误: status=" + resp.S) }
	n := len(resp.T)
	if n == 0 { return []provider.DailyPrice{}, nil }
	prices := make([]provider.DailyPrice, 0, n)
	for i := 0; i < n; i++ {
		if i >= len(resp.C) { break }
		if resp.C[i] == 0 { continue }
...
for _, r := range resp.Result { results = append(results, provider.TickerInfo{ Ticker: r.Symbol, Name:   r.Description, Market: "美股", }) }
	return results, nil
}
...
if ts != 0 { t.Errorf("dateToUnix(\"invalid-date\") = %d, want 0", ts) }
}
...
func TestParseCandleResponse_NoData(t *testing.T) {
	...
	prices, err := parseCandleResponse(body)
if err != nil { t.Fatalf("unexpected error: %v", err) }
if len(prices) != 0 { t.Fatalf("expected 0 prices for no_data, got %d", len(prices)) }
}
func TestParseCandleResponse_ErrorStatus(t *testing.T) {
	body := []byte(`{"s":"error"}`)
	_, err := parseCandleResponse(body)
if err == nil { t.Fatal("expected error for status=error, got nil") }
}
...
func TestParseCandleResponse_EmptyArrays(t *testing.T) {
	...
	prices, err := parseCandleResponse(body)
if err != nil { t.Fatalf("unexpected error: %v", err) }
if len(prices) != 0 { t.Fatalf("expected 0 prices for empty arrays, got %d", len(prices)) }
}
...
func TestParseCandleResponse_CloseArrayShorterThanT(t *testing.T) {
	...
	prices, err := parseCandleResponse(body)
if err != nil { t.Fatalf("unexpected error: %v", err) }
if len(prices) != 1 { t.Fatalf("expected 1 price (break on short C), got %d", len(prices)) }
}
...
if results[0].Name != "Apple Inc" { t.Errorf("Name[0] = %q, want Apple Inc", results[0].Name) }
...
func TestNewProvider_WithoutAPIKey(t *testing.T) {
	...
	defer func() { if key != "" { os.Setenv("FINNHUB_API_KEY", key) } }()
	p := NewProvider()
if p != nil { t.Fatal("expected nil provider when FINNHUB_API_KEY not set") }
}
func TestNewProvider_WithAPIKey(t *testing.T) {
	os.Setenv("FINNHUB_API_KEY", "test-key")
	defer os.Unsetenv("FINNHUB_API_KEY")
	p := NewProvider()
if p == nil { t.Fatal("expected non-nil provider when FINNHUB_API_KEY set") }
if name := p.Name(); name != "finnhub" { t.Errorf("Name() = %q, want finnhub", name) }
}
