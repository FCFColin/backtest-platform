...
if r == nil { t.Fatal("NewRegistry() returned nil") }
if len(r.priorities) != 3 { t.Errorf("priorities len = %d, want 3", len(r.priorities)) }
}
func TestRegistry_Register(t *testing.T) {
	r := NewRegistry([]string{"finnhub", "yfinance"})
	p := &mockProvider{name: "finnhub"}
	r.Register(p)
if _, ok := r.providers["finnhub"]; !ok { t.Error("provider not registered under name finnhub") }
}
...
if providers[0].Name() != "akshare" { t.Errorf("ForTicker(%q) routed to %s, want akshare", ticker, providers[0].Name()) }
	}
}
...
if len(providers) != 2 { t.Fatalf("ForTicker(AAPL) returned %d providers, want 2", len(providers)) }
if providers[0].Name() != "finnhub" { t.Errorf("first provider = %s, want finnhub", providers[0].Name()) }
if providers[1].Name() != "yfinance" { t.Errorf("second provider = %s, want yfinance", providers[1].Name()) }
}
...
if len(providers) != 2 { t.Fatalf("expected 2 fallback providers for A-share without akshare, got %d", len(providers)) }
}
...
if used != "p1" { t.Errorf("used provider = %q, want p1", used) }
if len(result) != 1 { t.Fatalf("expected 1 price, got %d", len(result)) }
if result[0].Close != 100 { t.Errorf("Close = %v, want 100", result[0].Close) }
if p2.calls != 0 { t.Errorf("p2 should not be called, got %d calls", p2.calls) }
}
func TestFetchWithFallback_FirstFails_SecondSucceeds(t *testing.T) {
	...
	result, used, err := FetchWithFallback([]Provider{p1, p2}, "AAPL", "2024-01-01", "2024-01-31")
if err != nil { t.Fatalf("unexpected error: %v", err) }
if used != "p2" { t.Errorf("used provider = %q, want p2", used) }
if len(result) != 1 { t.Fatalf("expected 1 price, got %d", len(result)) }
if result[0].Close != 200 { t.Errorf("Close = %v, want 200", result[0].Close) }
if p1.calls != 1 { t.Errorf("p1 should be called once, got %d", p1.calls) }
if p2.calls != 1 { t.Errorf("p2 should be called once, got %d", p2.calls) }
}
...
if err == nil { t.Fatal("expected error when all providers fail, got nil") }
if p1.calls != 1 { t.Errorf("p1 should be called once, got %d", p1.calls) }
if p2.calls != 1 { t.Errorf("p2 should be called once, got %d", p2.calls) }
}
func TestFetchWithFallback_EmptyProviderList(t *testing.T) {
	_, _, err := FetchWithFallback([]Provider{}, "AAPL", "2024-01-01", "2024-01-31")
if err == nil { t.Fatal("expected error for empty provider list, got nil") }
}
...
if used != "p2" { t.Errorf("used provider = %q, want p2", used) }
if len(result) != 1 { t.Fatalf("expected 1 price, got %d", len(result)) }
if p3.calls != 0 { t.Errorf("p3 should not be called, got %d calls", p3.calls) }
}
