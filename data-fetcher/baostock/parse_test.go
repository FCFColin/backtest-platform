...
// buildResponse constructs a proper baostock response with correct header format.
// Header: version(7) + MsgSplit(1) + msgType(2) + MsgSplit(1) + bodyLen(10) = 21 chars
...
func TestParseKDataResponseDynamic_ValidJSON(t *testing.T) {
	...
	if isLast {
		t.Error("expected isLast=false for valid response with data")
	}
	if len(data) != 1 {
		t.Fatalf("expected 1 data point, got %d", len(data))
	}
	if data[0]["date"] != "2024-01-02" {
		t.Errorf("date = %s, want 2024-01-02", data[0]["date"])
	}
}
...
func TestParseKDataResponseDynamic_ShortResponse(t *testing.T) {
	...
	if err != nil {
		t.Errorf("expected no error for short response, got: %v", err)
	}
	if !isLast {
		t.Error("expected isLast=true for short response")
	}
	if data != nil {
		t.Error("expected nil data for short response")
	}
}
...
func TestParseKDataResponseDynamic_NoRecord(t *testing.T) {
	...
	if !isLast {
		t.Error("expected isLast=true when no record found")
	}
}
...
func TestParseAllStockResponse_ValidJSON(t *testing.T) {
	...
	if stocks[0].Code != "sh.600000" {
		t.Errorf("first Code = %s, want sh.600000", stocks[0].Code)
	}
	if stocks[0].CodeName != "浦发银行" {
		t.Errorf("first CodeName = %s, want 浦发银行", stocks[0].CodeName)
	}
}
...
func TestParseAllStockResponse_ShortResponse(t *testing.T) {
	...
	if stocks != nil {
		t.Error("expected nil stocks for short response")
	}
}
...
func TestParseAllStockResponse_PartialFields(t *testing.T) {
	...
	if stocks[0].Code != "sh.600000" {
		t.Errorf("Code = %s, want sh.600000", stocks[0].Code)
	}
}
...
func TestParseTradeDatesResponse_ValidJSON(t *testing.T) {
	...
	if len(dates) != 3 {
		t.Fatalf("expected 3 trading dates, got %d", len(dates))
	}
	if dates[0] != "2024-01-02" {
		t.Errorf("first date = %s, want 2024-01-02", dates[0])
	}
}
...
func TestParseTradeDatesResponse_ShortResponse(t *testing.T) {
	...
	if dates != nil {
		t.Error("expected nil dates for short response")
	}
}
...
func TestParseTradeDatesResponse_AllNonTradingDays(t *testing.T) {
	...
	if len(dates) != 0 {
		t.Errorf("expected 0 trading dates, got %d", len(dates))
	}
}