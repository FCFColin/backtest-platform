...
func cmdFetchSIM(cfg *WorkerConfig, startDate, endDate string) error {
	...
	for _, ticker := range simTickers {
		...
		`, ticker, def.Category); err != nil { slog.Warn("插入 tickers 记录失败", "ticker", ticker, "error", err) }
	}
	successCount := 0
...
func cmdUpdate(cfg *WorkerConfig, incremental bool, startDate, endDate string) error {
	...
	if err != nil { return fmt.Errorf("检查 tickers 表失败: %w", err) }
	if empty {
		slog.Info("tickers 表为空，自动种子化默认 ETF 宇宙")
		if err := seedUniverse(ctx, pool); err != nil { return fmt.Errorf("种子化 ETF 宇宙失败: %w", err) }
	}
	tickers, err := loadTickerList(ctx, pool)
	if err != nil { return fmt.Errorf("加载标的列表失败: %w", err) }
...
if err == nil && lastDate != nil { actualStart = *lastDate }
		}
