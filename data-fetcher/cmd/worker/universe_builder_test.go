...
if entries[0].Ticker != "AAPL" { t.Errorf("first Ticker = %s, want AAPL", entries[0].Ticker) }
if entries[0].Category != "US Equity" { t.Errorf("first Category = %s, want US Equity", entries[0].Category) }
}
func TestParseNASDAQList_ETF(t *testing.T) {
	data := []byte("Symbol|Security Name|Market Category|Test Issue|Financial Status|Round Lot Size|ETF|NextShares\nSPY|SPDR S&P 500|P|N|N|100|Y|N")
	entries := parseNASDAQList(data)
if len(entries) != 1 { t.Fatalf("expected 1 entry, got %d", len(entries)) }
if entries[0].Category != "ETF" { t.Errorf("ETF Category = %s, want ETF", entries[0].Category) }
}
func TestParseNASDAQList_SkipsTestIssues(t *testing.T) {
	data := []byte("Symbol|Security Name|Market Category|Test Issue|Financial Status|Round Lot Size|ETF|NextShares\nTEST|Test Stock|Q|Y|N|100|N|N\nAAPL|Apple Inc|Q|N|N|100|N|N")
	entries := parseNASDAQList(data)
if len(entries) != 1 { t.Fatalf("expected 1 entry (test issue skipped), got %d", len(entries)) }
if entries[0].Ticker != "AAPL" { t.Errorf("Ticker = %s, want AAPL", entries[0].Ticker) }
}
func TestParseNASDAQList_EmptySymbol(t *testing.T) {
	data := []byte("Symbol|Security Name|Market Category|Test Issue|Financial Status|Round Lot Size|ETF|NextShares\n|Empty Stock|Q|N|N|100|N|N")
	entries := parseNASDAQList(data)
if len(entries) != 0 { t.Errorf("expected 0 entries (empty symbol), got %d", len(entries)) }
}
...
if len(entries) != 2 { t.Fatalf("expected 2 entries, got %d", len(entries)) }
if entries[0].Ticker != "BRK.A" { t.Errorf("first Ticker = %s, want BRK.A", entries[0].Ticker) }
}
...
if len(result) != 2 { t.Fatalf("expected 2 entries, got %d", len(result)) }
}
func TestMergeAndDedup_RemovesDuplicates(t *testing.T) {
	list1 := []TickerEntry{{Ticker: "AAPL", Name: "Apple"}}
	list2 := []TickerEntry{{Ticker: "AAPL", Name: ""}} // duplicate, empty name
	result := mergeAndDedup(list1, list2)
if len(result) != 1 { t.Fatalf("expected 1 entry (deduped), got %d", len(result)) }
}
func TestMergeAndDedup_FillsEmptyName(t *testing.T) {
	...
	result := mergeAndDedup(list1, list2)
if len(result) != 1 { t.Fatalf("expected 1 entry, got %d", len(result)) }
if result[0].Name != "Apple" { t.Errorf("Name should be filled from second list, got %q", result[0].Name) }
}
func TestMergeAndDedup_CaseInsensitive(t *testing.T) {
	list1 := []TickerEntry{{Ticker: "AAPL", Name: "Apple"}}
	list2 := []TickerEntry{{Ticker: "aapl", Name: "Apple Lower"}}
	result := mergeAndDedup(list1, list2)
if len(result) != 1 { t.Fatalf("expected 1 entry (case insensitive dedup), got %d", len(result)) }
}
func TestMergeAndDedup_Empty(t *testing.T) {
	result := mergeAndDedup()
if len(result) != 0 { t.Errorf("expected 0 entries, got %d", len(result)) }
}
...
func TestLoadTickersFromFile_PlainTicker(t *testing.T) {
	...
	entries, err := loadTickersFromFile(tmpFile)
if err != nil { t.Fatalf("unexpected error: %v", err) }
if len(entries) != 3 { t.Fatalf("expected 3 entries, got %d", len(entries)) }
	for _, e := range entries {
if e.Category != "Custom" { t.Errorf("Category = %s, want Custom", e.Category) }
	}
}
func TestLoadTickersFromFile_SkipsComments(t *testing.T) {
	...
	entries, err := loadTickersFromFile(tmpFile)
if err != nil { t.Fatalf("unexpected error: %v", err) }
if len(entries) != 2 { t.Fatalf("expected 2 entries (comments skipped), got %d", len(entries)) }
}
func TestLoadTickersFromFile_FileNotFound(t *testing.T) {
	_, err := loadTickersFromFile("/nonexistent/path/tickers.txt")
if err == nil { t.Error("expected error for non-existent file, got nil") }
}
func TestLoadTickersFromFile_EmptyFile(t *testing.T) {
	...
	entries, err := loadTickersFromFile(tmpFile)
if err != nil { t.Fatalf("unexpected error: %v", err) }
if len(entries) != 0 { t.Errorf("expected 0 entries for empty file, got %d", len(entries)) }
}
func TestTickerEntry_Fields(t *testing.T) {
	e := TickerEntry{Ticker: "AAPL", Name: "Apple Inc", Category: "US Equity", Market: "US"}
if e.Ticker != "AAPL" { t.Errorf("Ticker = %s, want AAPL", e.Ticker) }
if e.Name != "Apple Inc" { t.Errorf("Name = %s, want Apple Inc", e.Name) }
if e.Category != "US Equity" { t.Errorf("Category = %s, want US Equity", e.Category) }
if e.Market != "US" { t.Errorf("Market = %s, want US", e.Market) }
}
func TestDefaultETFUniverse_NotEmpty(t *testing.T) {
if len(DefaultETFUniverse) == 0 { t.Error("DefaultETFUniverse should not be empty") }
}
func TestDefaultETFUniverse_HasValidTickers(t *testing.T) {
	for _, meta := range DefaultETFUniverse {
if meta.Ticker == "" { t.Error("found entry with empty Ticker") }
if meta.Name == "" { t.Errorf("ticker %s has empty Name", meta.Ticker) }
	}
}
...
if !found { t.Error("DefaultETFUniverse should contain SPY") }
}
