...
异常登录检测: 登录失败 5 次/15min → 锁 15min（用户名）；IP 5min 10 次 → 封 1h（ANOMALY_LOGIN_* 可配）；登录限流每用户 10 次/15min；异地登录 >500km 突变 → 告警（计划中）；非工作时间(02-06)管理员登录 → 告警（计划中）。
...
SBOM: CI nightly 自动 CycloneDX, 覆盖 5 镜像, 保留 30 天。处置: 用户数据软删 30d→硬删；审计 180d；备份 7 份。
