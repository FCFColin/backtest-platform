...
> **注**：统计指标计算已统一到 Go 引擎。Node-canonical 路径通过 `statisticsClient.ts` 调用 Go 引擎的 `/api/engine/statistics` 端点获取统计结果，原 `statistics.ts`（~684 行 TS）已删除。
...
| `statisticsClient.ts` | 统计指标计算 — Go 引擎 HTTP 客户端                      |
