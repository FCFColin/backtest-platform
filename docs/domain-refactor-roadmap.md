...
当前领域逻辑（统计计算、再平衡、信号）大量以过程式函数散落于 `api/engine/*`，
...
| 上下文                        | 职责                          | 子域类型 | 当前代码                                                         |
| ----------------------------- | ----------------------------- | -------- | ---------------------------------------------------------------- |
| **Backtesting**（回测）       | 组合回测、再平衡、现金流      | 核心域   | `api/engine/portfolio.ts`、`api/application/backtest-service.ts` |
| **Analytics**（统计分析）     | Sharpe/回撤/相关性/PCA 等指标 | 核心域   | `api/engine/statistics.ts`、`pca.ts`                             |
| **Strategy**（策略/信号）     | 战术信号、网格搜索、择时      | 支撑域   | `api/engine/tactical*.ts`、`signal.ts`                           |
| **MarketData**（行情数据）    | 价格/指数/汇率获取与缓存      | 支撑域   | `api/services/dataService.ts`、`data-fetcher/`                   |
| **Identity & Access**（身份） | 认证、RBAC、会话              | 通用域   | `api/middleware/jwtAuth.ts`、`rbac.ts`、`userService.ts`         |
| **Auditing**（审计/事件）     | 审计日志、Outbox、领域事件    | 通用域   | `api/middleware/auditLog.ts`、`api/domain/events/`               |
...
1. `Portfolio` 聚合（已存在 `api/domain/aggregates/portfolio.ts`）：将权重归一化、再平衡触发等规则内聚到聚合方法。
...
- [x] 每个限界上下文有明确的模块边界（`api/application/*`），跨上下文经 service/事件通信
- [x] 路由层不再 import `api/engine/*`（类型 re-export 经 application 层）
- [x] 核心域规则内聚到聚合/值对象（权重、Ticker、再平衡事件）并有单测
- [x] CQRS 接口约定 + query/command service 分离（`cqrs.ts`、`backtest-query-service`）
- [x] 领域事件为跨上下文副作用通道（`RebalanceTriggered`、`BacktestCompleted` + Outbox）
