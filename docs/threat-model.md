...
| S-2  | 攻击者伪造 `X-Engine-Auth` 请求头直接调用 Rust 引擎 | Rust 引擎端点      | 高     | [v2.0] Rust 引擎已迁移到 Go engine-service（ADR-008），X-Engine-Auth 认证机制保留。端口暴露问题仍需解决 | ⚠️ 部分 |
| S-3  | 攻击者直接访问 Go 数据服务（:5003）无认证       | 数据服务端点       | 高     | 未实现：Go 服务无认证机制，端口暴露到主机                                                     | ❌ 未缓解 |
| S-4  | 攻击者伪造 `x-request-id` 请求头注入日志       | 日志系统           | 低     | 已实现：长度校验（≤128），但未做字符过滤                                                      | ⚠️ 部分 |
...
| R-2  | 计算密集型端点无认证，无法追溯资源消耗者         | 计算资源           | 中     | **v1.1 升级**：`optionalApiKey` 已升级为 `optionalJwtAuth`（待 T-P1-8 后续推广到 backtest 路由），可识别 JWT 用户身份并关联到资源消耗日志。当前 backtest 路由仍使用 `optionalApiKey`，计划在 T-P1-12 一并升级 | ⚠️ 部分 |
| R-3  | 数据写入操作无审计记录                           | 数据完整性         | 中     | 未实现：Python 脚本写入 JSON 文件无审计日志                                                  | ❌ 未缓解 |
...
| I-2  | Go/Rust 服务端口暴露到主机，同网段可探测            | 服务元信息         | 中     | [v2.0] Rust 引擎已迁移到 Go（ADR-008），减少一个暴露端口；Go 数据服务端口仍暴露 | ⚠️ 部分 |
...
| D-4  | Go 数据服务无速率限制，可被直接攻击                 | 数据服务           | 高     | 未实现：Go 服务无速率限制中间件，端口暴露到主机。[v2.0] Go 数据服务合并了 Python 数据获取功能（ADR-008），可统一添加速率限制；PostgreSQL 连接池可配置最大连接数 | ❌ 未缓解 |
...
| 优先级 | 编号   | 威胁摘要                                       | 建议措施                                                     |
| ------ | ------ | ---------------------------------------------- | ------------------------------------------------------------ |
| P0     | S-3    | Go 数据服务无认证，端口暴露到主机               | 绑定 `127.0.0.1:5003` 或添加 API Key 认证                   |
| P0     | D-4    | Go 数据服务无速率限制                           | 添加 gin 速率限制中间件                                      |
| P1     | I-2    | Go 端口暴露到主机所有接口                       | Docker 端口绑定 `127.0.0.1:5003:5003`                       |
| P1     | S-2    | Go 引擎端口暴露，同网段可绕过 Node API          | 绑定 `127.0.0.1:5002` 或添加 X-Engine-Auth 校验             |
| P1     | PG-1   | PostgreSQL 连接未加密（v2.0 新增）              | 生产环境 DATABASE_URL 必须使用 TLS                           |
| P2     | T-4    | 服务间 HTTP 通信未加密                          | Docker 内部网络隔离已提供基础防护，mTLS 可后续引入          |
| P2     | R-3    | 数据写入无审计记录                              | Python 脚本添加写入日志                                      |
| P2     | I-3    | API Key 可能在日志中泄露                        | 确认 pino-http req 序列化配置，排除敏感头                    |
| P2     | PG-2   | DATABASE_URL 泄露（v2.0 新增）                  | K8s Secret 管理，禁止写入镜像或 git                          |
| P3     | R-2    | 计算端点身份关联（v1.1 部分缓解）               | T-P1-12 将 backtest 路由从 `optionalApiKey` 升级为 `optionalJwtAuth` |
| P3     | S-4    | x-request-id 注入日志                           | 添加字符过滤（仅允许字母数字和连字符）                       |
...
| 维度         | 现状                                                         | 评分 |
| ------------ | ------------------------------------------------------------ | ---- |
| 认证         | **v2.0**：管理端点 JWT + API Key 双模式；Go 引擎/数据服务通过 Node API 间接认证 | ⭐⭐⭐ |
| 授权         | **v2.0**：RBAC 三角色七权限，声明式中间件 | ⭐⭐⭐ |
| 输入校验     | Ticker 正则防路径遍历；参数校验；请求体大小限制         | ⭐⭐⭐⭐ |
| 速率限制     | Node API 双层限流；Go 服务有限流；PostgreSQL 连接池限制 | ⭐⭐⭐ |
| 日志审计     | pino 结构化日志 + request_id；管理端点审计日志 | ⭐⭐⭐ |
| 传输安全     | **v2.0**：PostgreSQL 支持 TLS；服务间 HTTP 依赖网络隔离；无 mTLS | ⭐⭐⭐ |
