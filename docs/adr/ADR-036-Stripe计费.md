...
- `packages/backend/src/services/billingService.ts`：`ensureCustomer`、`createCheckoutSession`、`createPortalSession`、`constructWebhookEvent`（签名校验）、`handleWebhookEvent`（同步 `subscriptions` + 回写 `organizations.plan/status`，取消时计划回落 `free`）。
- `packages/backend/src/routes/billingRoutes.ts`：`GET /subscription`、`POST /checkout`、`POST /portal`（auth+tenant，写操作要求 ADMIN_ACCESS）。
