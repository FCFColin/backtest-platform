...
| 决策者 | 架构组                      |
| 范围   | CI 编排、依赖管理、质量门禁 |
| 关联   | ADR-004（Express 框架选型） |

> 注：本 ADR 原误编为 ADR-038，与 ADR-038（灾难恢复策略）编号冲突。ADR-038-DR-Strategy 为更早登记的 Proposed 决策，故本 CI 分层决策重新编号为 ADR-052，内容不变。
