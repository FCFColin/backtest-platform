# ADR-010: 计费与配额（Stripe + 按计划配额 + 公平调度）
...
- 数据（迁移 001_initial_schema.sql）：stripe_customers(org_id PK, stripe_customer_id)、subscriptions(org_id, stripe_subscription_id UNIQUE, plan, status, current_period_end, cancel_at_period_end)。两表启用 RLS 并附加 is_platform_admin 旁路策略——webhook 回调无租户上下文，经 withPlatformAdmin 以平台身份读写（见迁移 001 RLS 段）
