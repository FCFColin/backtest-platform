...
| 服务         | Logs               | Metrics           | Traces                   |
| ------------ | ------------------ | ----------------- | ------------------------ |
| Node API     | ✅ pino + trace_id | ✅ `/api/metrics` | ✅ OTel + pg             |
| engine-go    | ✅ slog            | ✅ `/metrics`     | ✅ otelgin               |
| data-fetcher | ✅ slog            | ✅ `/metrics`     | ✅ otelgin               |
| engine-rs    | ⚠️ tracing         | ❌                | ❌（降级路径，优先级低） |

跨服务传播：`api/utils/tracePropagation.ts` → `callService` traceparent 头。
